...
static uint32_t lz_decompress(uint8_t *out, uint32_t outcap, const uint8_t *src, uint32_t srclen, int32_t stride) {
  int32_t off[8];
  for (int i = 0; i < 8; i++) {
    off[i] = OFF_BASE[i] + 2 * (OFF_FACTOR[i] * stride);
  }
  uint32_t si = 0, oi = 0;
  while (si < srclen) {
    uint8_t ctrl = src[si++];
    ...
    if (mode != 0) { // back-reference into the output so far
      ...
      for (uint32_t k = 0; k < len; k++) {
        if (oi >= outcap || bi < 0 || bi >= (int64_t)outcap) {
          return oi;
        }
        out[oi++] = out[bi++];
      }
...
int main(int argc, char **argv) {
  ...
  const char *cmp = NULL;
  int nargs = argc;
  if (argc >= 3 && !strcmp(argv[argc - 2], "--cmp")) {
    cmp = argv[argc - 1];
    nargs = argc - 2;
  }
  const char *out = argv[nargs - 1];
  for (int a = 1; a < nargs - 1; a++) {
    if (apply_archive(argv[a])) {
      return 1;
    }
  }
  ...
  if (cmp) {
    ...
    if (fread(live, 2, VRAM_W * VRAM_H, lf) != (size_t)(VRAM_W * VRAM_H)) {
      fprintf(stderr, "short live\n");
      return 1;
    }
    fclose(lf);
    long tot = 0, mism = 0;
    ...
    fprintf(stderr,
            "[recon] CMP vs %s: %ld touched VRAM words, %ld mismatched (%.4f%% match)\n",
            cmp,
            tot,
            mism,
            tot ? 100.0 * (tot - mism) / tot : 0.0);
    return mism ? 3 : 0;
  }
  return 0;
}
