...
static bool iso_find_file(const char *path, uint32_t *out_lba, uint32_t *out_size) {
  ...
  if (!disc_read_user(16, sec)) {
    return false;
  }
  if (sec[0] != 1) {
    return false; // Primary Volume Descriptor
  }
  ...
  while (*p) {
    ...
    for (uint32_t s = 0; s < nsec && !found; s++) {
      if (!disc_read_user(dir_lba + s, sec)) {
        return false;
      }
      uint32_t o = 0;
      while (o < SECTOR_USER) {
...
static void png_chunk(FILE *f, const char *type, const uint8_t *data, uint32_t len) {
  ...
  if (len) {
    fwrite(data, 1, len, f);
    crc0 = (uint32_t)crc32(crc0, data, len);
  }
  fputc((crc0 >> 24) & 255, f);
  fputc((crc0 >> 16) & 255, f);
  fputc((crc0 >> 8) & 255, f);
...
static bool png_write(const char *path, int w, int h, const uint8_t *rgb24) {
  ...
  if (!f) {
    lucent::error("fmv", "cannot write {}", path);
    return false;
  }
  fwrite("\x89PNG\r\n\x1a\n", 1, 8, f);
  uint8_t ihdr[13] = {0};
...
int main(int argc, char **argv) {
  if (argc < 2) {
    fprintf(
        stderr,
        "usage: %s <out-dir> [movie-path] [disc.chd]\n"
  ...
  if (disc.empty()) {
    disc = lucent::config::text("PSXPORT_DISC");
  }
  if (disc.empty()) {
    disc = dotenv("PSXPORT_TOMBA2_DISC");
  }
  if (disc.empty()) {
    disc = dotenv("PSXPORT_DISC");
  }
  if (disc.empty()) {
    lucent::error("fmv",
                  "no disc image: pass one as argv[3], or set PSXPORT_TOMBA2_DISC / PSXPORT_DISC (env or ./.env)");
    return 2;
  }
  if (!disc_open(disc)) {
    return 1;
  ...
  if (!wav) {
    lucent::error("fmv", "cannot write {}/audio.wav", outdir);
    return 1;
  }
  for (int i = 0; i < 44; i++) {
    fputc(0, wav);
  ...
  if (!idx) {
    lucent::error("fmv", "cannot write {}/index.txt", outdir);
    return 1;
  }
  ...
  for (uint32_t sec = 0; sec < nsectors; sec++) {
    ...
    if (submode & 0x04) { // XA-ADPCM audio sector
      ...
      if (media_frames == 0) {
        ...
        lucent::warn("fmv", "audio rate changed {}Hz -> {}Hz mid-movie (LBA {})", xa_freq, fq, lba + sec);
      }
      for (int i = 0; i < n; i++) {
        wav_u16(wav, (uint16_t)xa_pcm[2 * i]);
        wav_u16(wav, (uint16_t)xa_pcm[2 * i + 1]);
      }
      media_frames += n;
      continue;
    }
    ...
    if (expected_chunks > 0 && got_chunks >= expected_chunks) {
      ...
      lucent::debug("fmv",
                    "frame {}: {}x{}, {} payload bytes, {} codes, {} samples elapsed",
                    framenum,
                    fwidth,
                    fheight,
                    paylen,
      ...
      if (cfg_on("PSXPORT_FMV_DUMPCODES")) {
        ...
        for (int i = 0; i < ncodes && i < 24; i++) {
          fprintf(stderr, " %04x", codes[i]);
        }
        fprintf(stderr, "\n");
        fprintf(stderr, "[fmv] frame %d payload first 32 bytes:", framenum);
        for (int i = 0; i < 32 && i < (int)paylen; i++) {
      ...
      if (ncodes > 0) {
        ...
        if (np > 0) {
          ...
          if (!png_write(pngpath, fwidth, fheight, rgb)) {
            fclose(wav);
            fclose(idx);
            return 1;
          }
      ...
      if (frames % 25 == 0) {
        lucent::info("fmv",
                     "{} frames, {} audio samples ({:.2f}s @ {}Hz)",
                     frames,
                     media_frames,
                     (double)media_frames / (xa_freq ? xa_freq : 37800),
                     xa_freq);
      }
    }
  }
  ...
  fputs("RIFF", wav);
  wav_u32(wav, 36 + (uint32_t)datab);
  fputs("WAVE", wav);
  fputs("fmt ", wav);
  wav_u32(wav, 16);
  wav_u16(wav, 1);
  wav_u16(wav, 2);
  wav_u32(wav, (uint32_t)xa_freq);
  wav_u32(wav, (uint32_t)xa_freq * 4);
  wav_u16(wav, 4);
  wav_u16(wav, 16);
  fputs("data", wav);
  wav_u32(wav, (uint32_t)datab);
  fclose(wav);
  fclose(idx);

  lucent::info("fmv",
               "done: {} video frames -> {}/frames, {} audio sample-pairs -> {}/audio.wav "
