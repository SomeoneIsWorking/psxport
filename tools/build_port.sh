...
# raylib (OpenGL) — the SBS debugger presents through raylib (MoltenVK black-pane workaround). Static lib.
tools/build_raylib.sh
RAYLIB_INC="-Ivendor/raylib/src"
RAYLIB_A="$(find build/raylib -name 'libraylib.a' 2>/dev/null | head -1)"
[ -n "$RAYLIB_A" ] || { echo "raylib static lib not built (tools/build_raylib.sh)" >&2; exit 1; }
case "$(uname -s)" in
...
$CXX -rdynamic $OBJS $CHD_LIBS $RMLUI_LIBS $RAYLIB_LIBS $(pkg-config --libs sdl2 vulkan) -lpthread -lm -o scratch/bin/tomba2_port || { echo "[build] link failed" >&2; exit 1; }
