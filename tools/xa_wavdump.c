...
int main(int argc, char **argv) {
  ...
  uint8_t raw[2352];
  static int16_t pcm[4032 * 2];
  int16_t hist[2][2] = {{0, 0}, {0, 0}};
  long frames = 0, loop_frame = -1;
  int freq = 37800;
  for (uint32_t lba = start; lba <= end + pad; lba++) {
    if (lba == end + 1) {
      loop_frame = frames; // the port loops here (s_lba > end)
    }
    ...
    int fq = freq;
    int n = xa_decode_sector(raw, pcm, hist, &fq);
    freq = fq;
    for (int i = 0; i < n; i++) {
      u16(f, (uint16_t)pcm[2 * i]);
      u16(f, (uint16_t)pcm[2 * i + 1]);
    }
    frames += n;
  }
  long datab = frames * 4;
  ...
  fclose(f);
  fprintf(stderr,
          "[xa_wavdump] %s: chan%u [%u..%u]+%u -> %ld frames @ %dHz (%.2f s). PORT LOOPS at %.2f s (frame %ld)\n",
          out,
          chan,
          start,
          end,
          ...
          freq,
          (double)frames / freq,
          loop_frame < 0 ? -1.0 : (double)loop_frame / freq,
          loop_frame);
  return 0;
}
