...
GAME-AGNOSTIC, AND THIS IS ITS ONE HOME. It is a PPM/PNG reader with no game knowledge in it, so
every port runs this exact file through its own `external/psxport` checkout:

    uv run --frozen python external/psxport/tools/port/present_geometry.py --selftest
...
unless a widescreen mod is deliberately active, which is what --expect is for.
    ...
    uv run --frozen python external/psxport/tools/port/present_geometry.py scratch/shots/present_10000.ppm
    uv run --frozen python external/psxport/tools/port/present_geometry.py shot.ppm --expect 16:9        # widescreen mod active
    uv run --frozen python external/psxport/tools/port/present_geometry.py --selftest                    # prove the tool still fires

THE CONFOUNDER, AND WHAT THIS TOOL DOES ABOUT IT
---------------------------------------------------
...
def selftest():
    ...
    bytes."""
    import os
    # The consuming repository's gitignored scratch/, not this framework checkout's: the tool is run
    # from a port's root through its own external/psxport, and that port owns the run artifacts.
    d = os.path.join(os.getcwd(), 'scratch', 'present_geometry_selftest')
