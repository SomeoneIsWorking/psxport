...
extra build targets; the shared engine owns command ordering, generator,
maintained-Lightning prefix derivation, error propagation, and both execution-boundary checks.
Maintainer invocations use `CC=clang CXX=clang++`; the verifier preserves the caller's toolchain.
The four consumer-verifier tests include an actual CTest positive and a misspelled-filter negative,
which previously returned success after executing no tests.
