...
int bs_decode_frame(const uint8_t *payload,
                    uint32_t payload_size,
                    int width,
                    int height,
                    uint16_t *codes,
                    int max_codes); // native_fmv.c (OURS)
int mdec_decode_to_rgb555(
    const uint16_t *codes, int ncodes, int width, int height, uint16_t *pixels); // native_fmv.c (real MDEC)
...
static void die_stub(const char *w) {
  fprintf(stderr, "[harness] unexpected call: %s\n", w);
  abort();
}
...
int MDFNSS_StateAction(void *a, int b, int c, void *d, const char *e, int f) {
  (void)a;
  (void)b;
  (void)c;
  (void)d;
...
static void rbr_init(RBR *b, const uint8_t *d, uint32_t n) {
  b->d = d;
  b->n = n;
  b->bytep = 0;
  b->buf = 0;
...
static void rbr_fill(RBR *b) {
  while (b->cnt <= 16 && b->bytep + 1 < b->n) {
    ...
    b->bytep += 2;
    b->buf |= (uint32_t)w << (16 - b->cnt);
    b->cnt += 16;
  }
}
static uint32_t rbr_peek(RBR *b, int k) {
  rbr_fill(b);
  if (!k) {
    return 0;
  }
  return (b->buf >> (32 - k)) & ((1u << k) - 1);
}
static void rbr_skip(RBR *b, int k) {
  b->buf <<= k;
  b->cnt -= k;
}
static uint32_t rbr_get(RBR *b, int k) {
  uint32_t v = rbr_peek(b, k);
  rbr_skip(b, k);
  return v;
}
static int rbr_eof(RBR *b) {
  return b->cnt <= 0 && b->bytep + 1 >= b->n;
}
...
static int ref_ac(RBR *b, int *run, int *level) {
  ...
  if (rbr_peek(b, 6) == 0x1) { // escape "000001"
    ...
    int l = (int)rbr_get(b, 10);
    if (l & 0x200) {
      l -= 0x400; // 10-bit signed (PSX)
    }
    *run = r;
    *level = l;
    return 1;
  ...
  for (int i = 0; i < R_N; i++) {
    ...
    uint32_t code = 0;
    for (int k = 0; k < len; k++) {
      code = (code << 1) | (R_TAB[i].bits[k] - '0');
    }
    if (rbr_peek(b, len) == code) {
      rbr_skip(b, len);
      int s = (int)rbr_get(b, 1);
      *run = R_TAB[i].run;
      *level = s ? -R_TAB[i].level : R_TAB[i].level;
      return 1;
    }
  }
  return -1;
...
static int ref_decode(const uint8_t *payload, uint32_t paylen, int w, int h, uint16_t *codes, int maxc) {
  if (paylen < 8) {
    return -1;
  }
  int qscale = (payload[4] | (payload[5] << 8)) & 0x3F;
  if (!qscale) {
    qscale = 1;
  }
  RBR br;
  rbr_init(&br, payload + 8, paylen - 8);
...
// ====================================================================================
static int demux_frame(uint32_t lba, uint32_t size, int want, uint8_t *payload, uint32_t maxpay, int *w, int *h) {
  ...
  for (uint32_t s = 0; s < nsec; s++) {
    if (!disc_read_sector(lba + s, sec)) {
      break;
    }
    if ((uint16_t)(sec[0] | (sec[1] << 8)) != 0x0160) {
      continue; // not a video sector
    }
    ...
    if (paylen + pl <= maxpay) {
      memcpy(payload + paylen, sec + SUBHDR_LEN, pl);
      paylen += pl;
    }
    got++;
...
static int put_block(uint16_t *c, int o, int dc, int run, int level, int have_ac) {
  ...
  if (have_ac) {
    c[o++] = (uint16_t)(((run & 0x3F) << 10) | (level & 0x3FF));
  }
  c[o++] = 0xFE00;
  return o;
}
...
static void idct_test(void) {
  static uint16_t codes[64 * MBN];
  static uint16_t px[16 * 16 * MBN];
  const int H = 16 * MBN;
  // (1) DC-only: every MB = Cr,Cb (DC0), Y0..Y3 (DC=200). Expect a flat block.
  int o = 0;
  for (int m = 0; m < MBN; m++) {
    o = put_block(codes, o, 0, 0, 0, 0);
    o = put_block(codes, o, 0, 0, 0, 0);
    for (int y = 0; y < 4; y++) {
      o = put_block(codes, o, 200, 0, 0, 0);
    }
  }
  ...
  for (int y = 0; y < 8; y++) {
    fprintf(stderr, "  ");
    for (int x = 0; x < 8; x++) {
      int v = px[(8 * 16) * 16 + y * 16 + x];
      fprintf(stderr, "%3d ", ((v >> 5) & 31));
    }
    fprintf(stderr, "\n");
  }
  ...
  static uint16_t codesW[64 * 20];
  static uint16_t pxW[320 * 16];
  o = 0;
  for (int m = 0; m < 20; m++) {
    int dc = 20 + m * 2;
    o = put_block(codesW, o, 0, 0, 0, 0);
    o = put_block(codesW, o, 0, 0, 0, 0);
    for (int y = 0; y < 4; y++) {
      o = put_block(codesW, o, dc, 0, 0, 0);
    }
  }
  ...
  for (int m = 0; m < 20; m++) {
    int v = pxW[(8) * 320 + m * 16 + 8];
    fprintf(stderr, "%d ", ((v >> 5) & 31));
  }
  fprintf(stderr, "\n");
  ...
  static uint16_t codesG[64 * 60];
  static uint16_t pxG[320 * 48];
  o = 0;
  for (int m = 0; m < 60; m++) {
    int dc = 12 + m;
    o = put_block(codesG, o, 0, 0, 0, 0);
    o = put_block(codesG, o, 0, 0, 0, 0);
    for (int y = 0; y < 4; y++) {
      o = put_block(codesG, o, dc, 0, 0, 0);
    }
  }
  ...
  for (int ry = 0; ry < 3; ry++) {
    ...
    for (int rx = 0; rx < 20; rx++) {
      int v = pxG[(ry * 16 + 8) * 320 + rx * 16 + 8];
      fprintf(stderr, "%2d ", ((v >> 5) & 31));
    }
    fprintf(stderr, "\n");
  }
}
...
static void framemap(uint32_t lba, uint32_t size, int want) {
  if (!disc_open()) {
    fprintf(stderr, "disc_open failed\n");
    return;
  }
  ...
  static uint16_t codes[1 << 20];
  static uint16_t px[1024 * 512];
  ...
  for (int cy = 0; cy < h; cy += 8) {
    ...
    for (int cx = 0; cx < w; cx += 8) {
      long s = 0, c = 0;
      for (int y = cy; y < cy + 8 && y < h; y++) {
        for (int x = cx; x < cx + 8 && x < w; x++) {
          int v = px[y * w + x];
          int g = (v >> 5) & 31;
          s += g;
          c++;
...
static int xa_ref(const uint8_t *raw, int16_t *out, int h[2][2]) {
  ...
  int coding = raw[19];
  if (coding & 0x10) {
    return -1; // ref only handles 4-bit
  }
  int stereo = coding & 1;
  int16_t L[2016], R[2016];
  int nl = 0, nr = 0;
  for (int g = 0; g < 18; g++) {
    ...
    for (int u = 0; u < 8; u++) {
      ...
      h[ch][1] = s2;
      if (p != pc) {
        memset(smp, 0, sizeof smp); /* note: history already advanced, matches ours? */
      }
      if (stereo) {
        if (ch) {
          for (int i = 0; i < 28; i++) {
...
int main(int argc, char **argv) {
  if (argc >= 2 && !strcmp(argv[1], "idcttest")) {
    idct_test();
    return 0;
  }
  if (argc >= 4 && !strcmp(argv[1], "xacmp")) { // xacmp <lba> <nsectors>: sound oracle
    ...
    uint8_t raw[2352];
    int16_t ours[2016 * 2], ref[2016 * 2];
    int16_t ho[2][2] = {{0, 0}, {0, 0}};
    int hr[2][2] = {{0, 0}, {0, 0}};
    int asec = 0, maxdiff = 0;
    long diffs = 0, samples = 0;
    for (uint32_t s = 0; s < n; s++) {
      if (!disc_read_raw(lba + s, raw, 2352)) {
        break;
      }
      if (!(raw[18] & 0x04)) {
        continue; // audio sectors only
      }
      int fq;
      int no = xa_decode_sector(raw, ours, ho, &fq);
      int nr = xa_ref(raw, ref, hr);
      asec++;
      if (no != nr) {
        fprintf(stderr, "sector %u: count mismatch ours=%d ref=%d\n", lba + s, no, nr);
        continue;
      }
    ...
    fprintf(
        stderr, "xacmp: %d audio sectors, %ld samples, %ld differing, max|diff|=%d\n", asec, samples, diffs, maxdiff);
    fprintf(stderr,
            diffs ? "MISMATCH (our XA decode diverges from the reference)\n"
                  : "MATCH (XA decode bit-exact vs independent reference)\n");
    return diffs ? 3 : 0;
  }
  if (argc >= 5 && !strcmp(argv[1], "framemap")) {
    framemap((uint32_t)strtoul(argv[2], 0, 0), (uint32_t)strtoul(argv[3], 0, 0), atoi(argv[4]));
    return 0;
  }
  if (argc >= 5 && !strcmp(argv[1], "dumplba")) { // dumplba <lba> <nbytes> <out>: extract a region
    ...
    uint8_t s[2048];
    uint32_t done = 0;
    for (uint32_t k = 0; done < nb; k++) {
      if (!disc_read_sector(lba + k, s)) {
        break;
      }
      uint32_t w = nb - done < 2048 ? nb - done : 2048;
      fwrite(s, 1, w, o);
      done += w;
    }
    fclose(o);
    fprintf(stderr, "wrote %u bytes from LBA %u to %s\n", done, lba, argv[4]);
    return 0;
  }
  if (argc >= 4 && !strcmp(argv[1], "strscan")) { // strscan <lba> <nsectors>: dump CD-XA framing
    ...
    for (uint32_t s = 0; s < n; s++) {
      ...
      if (s < 24) {
        fprintf(stderr,
                "  sec %u: mode%d submode=%02x coding=%02x ch=%d vmagic=%04x -> %s\n",
                lba + s,
                mode,
                sm,
                coding,
    ...
    for (uint32_t s = 0; s < n; s++) {
      ...
      if (raw[18] & 0x04) {
        int c = raw[19];
        fprintf(stderr,
                "first AUDIO sec %u: coding=%02x -> %s, %s, %d-bit\n",
                lba + s,
                c,
                (c & 1) ? "stereo" : "mono",
                (c & 4) ? "18900Hz" : "37800Hz",
  ...
  if (argc < 4) {
    fprintf(stderr,
            "usage: %s <lba> <size> <frame#> | %s idcttest | %s framemap <lba> <size> <frame#>\n",
            argv[0],
            argv[0],
            argv[0]);
    return 2;
  ...
  if (!disc_open()) {
    fprintf(stderr, "disc_open failed (set PSXPORT_TOMBA2_DISC/.env)\n");
    return 1;
  }
  ...
  fprintf(stderr,
          "frame %d: %dx%d, payload %d bytes, qscale=%d\n",
          want,
          w,
          h,
          paylen,
          (payload[4] | (payload[5] << 8)) & 0x3F);
  ...
  for (int i = 0; i < n; i++) {
    if (ours[i] != ref[i]) {
      if (diffs == 0) {
        fprintf(stderr,
                "FIRST DIVERGENCE @code %d (block %d, coeff %d): ours=%04x ref=%04x\n"
                ...
                ref[i],
                ours[i] >> 10,
                (ours[i] & 0x3FF) >= 0x200 ? (ours[i] & 0x3FF) - 0x400 : (ours[i] & 0x3FF),
                ref[i] >> 10,
                (ref[i] & 0x3FF) >= 0x200 ? (ref[i] & 0x3FF) - 0x400 : (ref[i] & 0x3FF));
      }
      diffs++;
    }
    if (ours[i] == 0xFE00) {
  ...
  if (diffs == 0 && no == nr) {
    fprintf(stderr, "MATCH: %d codes identical (VLC is correct)\n", no);
  } else {
    fprintf(stderr, "TOTAL: %d differing codes (of %d compared); ours=%d ref=%d\n", diffs, n, no, nr);
  }
  return diffs ? 3 : 0;
}
