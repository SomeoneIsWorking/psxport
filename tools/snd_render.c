...
static uint16_t rd16(long o) {
  return g_buf[o] | (g_buf[o + 1] << 8);
}
static uint16_t rd16be(long o) {
  return (g_buf[o] << 8) | g_buf[o + 1];
}
static uint32_t rd24be(long o) {
  return (g_buf[o] << 16) | (g_buf[o + 1] << 8) | g_buf[o + 2];
}
static long vab_file_off(int idx) {
  return (long)rd16(idx * 2) * 0x800;
}
...
static void wav_write(const char *path, const int16_t *pcm, int frames, int ch, int rate) {
  ...
  fputc(riff & 0xff, f);
  fputc((riff >> 8) & 0xff, f);
  fputc((riff >> 16) & 0xff, f);
  fputc((riff >> 24) & 0xff, f);
...
static int render_raw(long seqOff, long vabOff, const char *out, int seconds) {
  NaVab v;
  if (na_vab_open_at(&v, g_buf, vabOff)) {
    fprintf(stderr, "bad VAB @0x%lx\n", vabOff);
    return 1;
  }
  NaSeq s;
  if (na_seq_open(&s, g_buf, &v, seqOff)) {
    fprintf(stderr, "bad seq @0x%lx\n", seqOff);
    return 1;
  }
  fprintf(stderr,
          "[snd_render] seq@0x%lx vab@0x%lx(ps=%d ts=%d vs=%d) ppqn=%d tempo=%.0f\n",
          seqOff,
          vabOff,
          v.ps,
          v.ts,
...
int main(int argc, char **argv) {
  if (argc < 3) {
    fprintf(stderr, "usage: %s <TOMBA2.SND> info|vab|seqs|vag|tone|song|songid|raw ...\n", argv[0]);
    return 1;
  }
  ...
  if (!strcmp(cmd, "info")) {
    ...
    for (int i = 0; i < 24; i++) {
      long o = vab_file_off(i);
      if (o <= 0 || o + 4 > g_size) {
        break;
      }
      if (memcmp(g_buf + o, "pBAV", 4)) {
        continue;
      }
      NaVab v;
      na_vab_open_at(&v, g_buf, o);
  ...
  if (!strcmp(cmd, "vab")) {
    int idx = atoi(argv[3]);
    NaVab v;
    na_vab_open_at(&v, g_buf, vab_file_off(idx));
  ...
  if (!strcmp(cmd, "vag")) { // raw-decode one VAG (inspection)
    int vi = atoi(argv[3]), gi = atoi(argv[4]);
    const char *out = argv[5];
    NaVab v;
    na_vab_open_at(&v, g_buf, vab_file_off(vi));
    static int16_t pcm[8 * 1024 * 1024];
    int ls, lp;
    int n = na_vab_decode_vag(&v, gi, pcm, sizeof(pcm) / 2, &ls, &lp);
    fprintf(stderr, "[snd_render] VAB%d VAG%d -> %d samples (loop_start=%d loops=%d)\n", vi, gi, n, ls, lp);
    wav_write(out, pcm, n, 1, 44100);
    return 0;
  }
  if (!strcmp(cmd, "song")) {
    int si = atoi(argv[3]), vi = atoi(argv[4]);
    const char *out = argv[5];
  ...
  if (!strcmp(cmd, "songid")) {
    if (argc < 5) {
      fprintf(stderr, "usage: %s <SND> songid <id> <out> [seconds]\n", argv[0]);
      return 1;
    }
    int id = atoi(argv[3]);
    ...
    static const struct {
      int seq, vab;
        ...
        {2, 13},
        {1, 13},
        {0, 13}, // songs 0..3: bank 14 -> vab idx 13
        {4, 7},
        {5, 7},
        {6, 7}, // songs 4..6 (dialog): bank 8 -> vab idx 7
        {7, 13},
        {8, 13},
        {9, 13}, // songs 7..9: bank 14 -> vab idx 13
    };
    if (id < 0 || id >= 10) {
      fprintf(stderr, "songid %d out of range 0..9\n", id);
      return 1;
    }
    fprintf(
        stderr, "[snd_render] songid %d -> seq%d vab%d (bank %d)\n", id, S2SV[id].seq, S2SV[id].vab, S2SV[id].vab + 1);
    return render_song(S2SV[id].seq, S2SV[id].vab, out, seconds);
  }
  if (!strcmp(cmd, "raw")) {
    if (argc < 6) {
      fprintf(stderr, "usage: %s <file> raw <seqOff_hex> <vabOff_hex> <out> [seconds]\n", argv[0]);
      return 1;
    }
