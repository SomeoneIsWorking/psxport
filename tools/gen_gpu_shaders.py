...
def generate(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="gen_gpu_shaders-", dir=scratch) as temp_name:
    ...
    if announce:
        if header_changed:
            print(f"[gen_gpu_shaders] wrote {output.relative_to(root)}")
        else:
...
def run_selftest(root: Path) -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix="gen_gpu_shaders-selftest-", dir=scratch) as temp_name:
        ...
        stamp = fixture / "build/gen_gpu_shaders.stamp"
        ...
        try:
            first_changed = generate(
                fixture, str(fake_glslc), tool_path=tool_path, stamp=stamp, announce=False
            )
            output = fixture / OUTPUT_REL
...
def main() -> int:
    ...
    try:
        ...
        generate(root, find_glslc(), stamp=stamp)
    except GeneratorError as error:
