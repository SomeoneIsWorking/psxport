...
RUNTIME="$RT/mem.c $RT/stubs.c $RT/hle.c $RT/threads.c $RT/interp.c $RT/gpu_native.c $MED_SRC"
CD="$RT/disc.c $RT/cd_override.c $RT/timing.c $RT/games_tomba2.c"
echo "[3/3] compile core + runtime, run leaf test + boot"
$CC $CFLAGS $MED_INC $SDL_CFLAGS "$GEN" $RUNTIME "$RT/test_leaf.c" -o scratch/bin/test_leaf $SDL_LIBS
./scratch/bin/test_leaf
echo "--- boot (6s cap) ---"
$CC $CFLAGS $CHD_INC $MED_INC $SDL_CFLAGS "$GEN" $RUNTIME $CD "$RT/boot.c" $CHD_LIBS $SDL_LIBS -lpthread -o scratch/bin/boot
