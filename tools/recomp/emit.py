...
def main():
    ...
    # Forward declarations: per function a pure recomp body gen_func_X and a wrapper func_X
    # (the wrapper checks a runtime override slot, then calls the body — recomp-overrides:
    # body stays alive for A/B + diffing; overrides are runtime wiring, no regen).
    for a in funcs:
        out.append(f"void gen_func_{a:08X}(R3000*); void func_{a:08X}(R3000*);")
    out.append("")
    ...
    for a in funcs:
        emit_func(exe, a, nxt_of[a], funcset, out, name=f"gen_func_{a:08X}")

    # Override slots + wrappers. Call sites and the dispatch table call func_X; the wrapper
    # runs a registered override (if any) else the recomp body. Overrides super-call
    # gen_func_X directly to invoke the original body without re-entering the wrapper.
    idx = {a: k for k, a in enumerate(funcs)}
    out.append(f"OverrideFn g_override[{len(funcs)}];")
    for a in funcs:
        i = idx[a]
        out.append(f"void func_{a:08X}(R3000* c) {{ if (g_override[{i}]) {{ g_override[{i}](c); return; }} gen_func_{a:08X}(c); }}")
    out.append("")
    # address -> dense index (for rec_set_override) and the dispatch table.
    out.append("int rec_func_index(uint32_t addr) {")
    out.append("  switch (addr & 0x1FFFFFFFu) {")
    for a in funcs:
        out.append(f"    case 0x{a & 0x1FFFFFFF:08X}u: return {idx[a]};")
    out.append("    default: return -1;")
    out.append("  }")
    out.append("}")
    out.append("void rec_set_override(uint32_t addr, OverrideFn fn) {")
    out.append("  int i = rec_func_index(addr); if (i >= 0) g_override[i] = fn;")
    out.append("}")
