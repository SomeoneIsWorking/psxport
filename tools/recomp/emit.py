...
def main():
    ...
    # GENERATED SUBSTRATE IDENTITY. The app/framework git IDs cannot describe generated/ because it
    # is deliberately ignored. Hash the exact emitted translation units plus the routing metadata
    ...
    open(os.path.join(out_dir, ".recomp_identity"), "w").write(substrate_id + "\n")
    print(f"[recomp] version {RECOMP_VERSION}")
    print(f"[recomp] substrate identity {substrate_id}")
...
def generated_substrate_identity(out_dir, src_files, main_exe, overlays, whole_program):
    ...
    Fixed/relocatable routing metadata is hashed separately because it is emitted into that carrier.
    File names are part of the identity so a different shard/module topology cannot collide with the
    same concatenated bytes.
    """
    digest = hashlib.sha256()
    digest.update((RECOMP_VERSION + "\n").encode())
    for name in sorted(set(src_files)):
