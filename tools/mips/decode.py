"""Typed R3000A (MIPS I) instruction decoding for binary-analysis tools.

This module describes instruction encodings only. It does not emit source,
execute guest instructions, or participate in the gameplay runtime.
"""

from __future__ import annotations

from dataclasses import dataclass, field

REG = [
    "zero", "at", "v0", "v1", "a0", "a1", "a2", "a3",
    "t0", "t1", "t2", "t3", "t4", "t5", "t6", "t7",
    "s0", "s1", "s2", "s3", "s4", "s5", "s6", "s7",
    "t8", "t9", "k0", "k1", "gp", "sp", "fp", "ra",
]

ALU_RRR = "alu_rrr"
ALU_RRI = "alu_rri"
SHIFT_I = "shift_i"
SHIFT_V = "shift_v"
LUI = "lui"
MULDIV = "muldiv"
HILO = "hilo"
LOAD = "load"
STORE = "store"
BRANCH = "branch"
JUMP = "jump"
JUMPR = "jumpr"
SYSCALL = "syscall"
BREAK = "break_"
COP0 = "cop0"
GTE_MOVE = "gte_move"
GTE_OP = "gte_op"
GTE_LOAD = "gte_load"
GTE_STORE = "gte_store"
NOP = "nop"
UNKNOWN = "unknown"


@dataclass
class Instr:
    addr: int
    raw: int
    kind: str
    op: str = ""
    rs: int = 0
    rt: int = 0
    rd: int = 0
    shamt: int = 0
    imm: int = 0
    target: int = 0
    cop2: int = 0
    fields: dict = field(default_factory=dict)

    @property
    def simm(self) -> int:
        return self.imm - 0x10000 if self.imm & 0x8000 else self.imm


SPECIAL = {
    0x00: ("sll", SHIFT_I), 0x02: ("srl", SHIFT_I), 0x03: ("sra", SHIFT_I),
    0x04: ("sllv", SHIFT_V), 0x06: ("srlv", SHIFT_V), 0x07: ("srav", SHIFT_V),
    0x08: ("jr", JUMPR), 0x09: ("jalr", JUMPR),
    0x0C: ("syscall", SYSCALL), 0x0D: ("break", BREAK),
    0x10: ("mfhi", HILO), 0x11: ("mthi", HILO),
    0x12: ("mflo", HILO), 0x13: ("mtlo", HILO),
    0x18: ("mult", MULDIV), 0x19: ("multu", MULDIV),
    0x1A: ("div", MULDIV), 0x1B: ("divu", MULDIV),
    0x20: ("add", ALU_RRR), 0x21: ("addu", ALU_RRR),
    0x22: ("sub", ALU_RRR), 0x23: ("subu", ALU_RRR),
    0x24: ("and", ALU_RRR), 0x25: ("or", ALU_RRR),
    0x26: ("xor", ALU_RRR), 0x27: ("nor", ALU_RRR),
    0x2A: ("slt", ALU_RRR), 0x2B: ("sltu", ALU_RRR),
}

REGIMM = {0x00: "bltz", 0x01: "bgez", 0x10: "bltzal", 0x11: "bgezal"}

PRIMARY = {
    0x02: ("j", JUMP), 0x03: ("jal", JUMP),
    0x04: ("beq", BRANCH), 0x05: ("bne", BRANCH),
    0x06: ("blez", BRANCH), 0x07: ("bgtz", BRANCH),
    0x08: ("addi", ALU_RRI), 0x09: ("addiu", ALU_RRI),
    0x0A: ("slti", ALU_RRI), 0x0B: ("sltiu", ALU_RRI),
    0x0C: ("andi", ALU_RRI), 0x0D: ("ori", ALU_RRI), 0x0E: ("xori", ALU_RRI),
    0x0F: ("lui", LUI),
    0x20: ("lb", LOAD), 0x21: ("lh", LOAD), 0x22: ("lwl", LOAD),
    0x23: ("lw", LOAD), 0x24: ("lbu", LOAD), 0x25: ("lhu", LOAD),
    0x26: ("lwr", LOAD),
    0x28: ("sb", STORE), 0x29: ("sh", STORE), 0x2A: ("swl", STORE),
    0x2B: ("sw", STORE), 0x2E: ("swr", STORE),
    0x32: ("lwc2", GTE_LOAD), 0x3A: ("swc2", GTE_STORE),
}

COP2_MOVE = {0x00: "mfc2", 0x02: "cfc2", 0x04: "mtc2", 0x06: "ctc2"}
COP0_MOVE = {0x00: "mfc0", 0x04: "mtc0"}


def decode(addr: int, raw: int) -> Instr:
    if raw == 0:
        return Instr(addr, raw, NOP, "nop")
    op = (raw >> 26) & 0x3F
    rs = (raw >> 21) & 0x1F
    rt = (raw >> 16) & 0x1F
    rd = (raw >> 11) & 0x1F
    shamt = (raw >> 6) & 0x1F
    funct = raw & 0x3F
    imm = raw & 0xFFFF

    if op == 0x00:
        entry = SPECIAL.get(funct)
        if entry is None:
            return Instr(addr, raw, UNKNOWN, f"special:0x{funct:02X}", rs=rs, rt=rt, rd=rd, shamt=shamt)
        name, kind = entry
        return Instr(addr, raw, kind, name, rs=rs, rt=rt, rd=rd, shamt=shamt)

    if op == 0x01:
        name = REGIMM.get(rt)
        if name is None:
            return Instr(addr, raw, UNKNOWN, f"regimm:0x{rt:02X}", rs=rs, imm=imm)
        target = addr + 4 + (((imm - 0x10000) if imm & 0x8000 else imm) << 2)
        return Instr(addr, raw, BRANCH, name, rs=rs, imm=imm, target=target)

    if op == 0x10:
        if rs == 0x10 and funct == 0x10:
            return Instr(addr, raw, COP0, "rfe")
        name = COP0_MOVE.get(rs)
        if name is None:
            return Instr(addr, raw, UNKNOWN, f"cop0:0x{rs:02X}", rt=rt, rd=rd)
        return Instr(addr, raw, COP0, name, rt=rt, rd=rd)

    if op == 0x12:
        if raw & (1 << 25):
            return Instr(addr, raw, GTE_OP, "cop2", cop2=raw)
        name = COP2_MOVE.get(rs)
        if name is None:
            return Instr(addr, raw, UNKNOWN, f"cop2:0x{rs:02X}", rt=rt, rd=rd)
        if raw & 0x7FF:
            return Instr(addr, raw, UNKNOWN, f"cop2-move-reserved:0x{raw & 0x7FF:03X}", rt=rt, rd=rd)
        return Instr(addr, raw, GTE_MOVE, name, rt=rt, rd=rd)

    entry = PRIMARY.get(op)
    if entry is None:
        return Instr(addr, raw, UNKNOWN, f"op:0x{op:02X}", rs=rs, rt=rt, imm=imm)
    name, kind = entry
    if kind == JUMP:
        target = ((addr + 4) & 0xF0000000) | ((raw & 0x03FFFFFF) << 2)
        return Instr(addr, raw, kind, name, target=target)
    if kind == BRANCH:
        target = addr + 4 + (((imm - 0x10000) if imm & 0x8000 else imm) << 2)
        return Instr(addr, raw, kind, name, rs=rs, rt=rt, imm=imm, target=target)
    return Instr(addr, raw, kind, name, rs=rs, rt=rt, imm=imm)


def fmt(ins: Instr) -> str:
    """Return a compact human-readable instruction description."""
    register = lambda number: REG[number]
    kind, operation = ins.kind, ins.op
    if kind == NOP:
        return "nop"
    if kind == LUI:
        return f"lui {register(ins.rt)}, 0x{ins.imm:04X}"
    if kind == ALU_RRR:
        return f"{operation} {register(ins.rd)}, {register(ins.rs)}, {register(ins.rt)}"
    if kind == ALU_RRI:
        return f"{operation} {register(ins.rt)}, {register(ins.rs)}, {ins.simm}"
    if kind == SHIFT_I:
        return f"{operation} {register(ins.rd)}, {register(ins.rt)}, {ins.shamt}"
    if kind == SHIFT_V:
        return f"{operation} {register(ins.rd)}, {register(ins.rt)}, {register(ins.rs)}"
    if kind == MULDIV:
        return f"{operation} {register(ins.rs)}, {register(ins.rt)}"
    if kind == HILO:
        return f"{operation} {register(ins.rd or ins.rs)}"
    if kind in (LOAD, STORE, GTE_LOAD, GTE_STORE):
        return f"{operation} {register(ins.rt)}, {ins.simm}({register(ins.rs)})"
    if kind == BRANCH:
        if operation in ("beq", "bne"):
            return f"{operation} {register(ins.rs)}, {register(ins.rt)}, 0x{ins.target:08X}"
        return f"{operation} {register(ins.rs)}, 0x{ins.target:08X}"
    if kind == JUMP:
        return f"{operation} 0x{ins.target:08X}"
    if kind == JUMPR:
        return f"jalr {register(ins.rd)}, {register(ins.rs)}" if operation == "jalr" else f"jr {register(ins.rs)}"
    if kind == GTE_MOVE:
        return f"{operation} {register(ins.rt)}, ${ins.rd}"
    if kind == GTE_OP:
        return f"cop2 0x{ins.cop2 & 0x01FFFFFF:07X}"
    if kind == COP0:
        return operation if operation == "rfe" else f"{operation} {register(ins.rt)}, ${ins.rd}"
    if kind in (SYSCALL, BREAK):
        return operation
    return f"<{operation}>"
