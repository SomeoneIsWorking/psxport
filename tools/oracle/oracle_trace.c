...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "--steps") && i + 1 < argc)        steps = strtol(argv[++i], NULL, 0);
    else if (!strcmp(argv[i], "--out") && i + 1 < argc)     out_path = argv[++i];
    else if (!strcmp(argv[i], "--entry") && i + 1 < argc)   entry_override = (uint32_t)strtoul(argv[++i], NULL, 0);
    else if (!strcmp(argv[i], "--summary-only"))            summary_only = 1;
    else if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) { usage(); return 0; }
    else if (argv[i][0] == '-') { fprintf(stderr, "oracle_trace: unknown option %s\n", argv[i]); usage(); return 2; }
    else if (!path) path = argv[i];
    else { fprintf(stderr, "oracle_trace: unexpected extra argument %s\n", argv[i]); usage(); return 2; }
  }
  if (!path) { usage(); return 2; }
  if (steps <= 0) {
    fprintf(stderr, "oracle_trace: REFUSING — --steps %ld traces nothing. An empty trace would read as a\n"
  ...
  if (!f) { fprintf(stderr, "oracle_trace: REFUSING — cannot open %s. No trace written.\n", path); return 2; }
  fseek(f, 0, SEEK_END);
  long fsz = ftell(f);
  fseek(f, 0, SEEK_SET);
  if (fsz < PSX_EXE_HEADER_BYTES) {
    fprintf(stderr, "oracle_trace: REFUSING — %s is %ld byte(s), shorter than the 0x800 PS-X EXE header.\n"
                    "  No trace written.\n", path, fsz);
    fclose(f);
    return 2;
  }
  ...
  if (memcmp(img, "PS-X EXE", 8) != 0) {
    fprintf(stderr, "oracle_trace: REFUSING — %s does not begin with the PS-X EXE magic (got %.8s).\n"
  ...
  fprintf(out, "# header: pc0=0x%08X gp0=0x%08X sp=0x%08X text=0x%08X..0x%08X (0x%X of 0x%X byte(s) present)\n",
          pc0, gp0, sp0, t_addr, t_addr + present, present, t_size);
  ...
  for (n = 0; n < steps; n++) {
    ...
    if (!summary_only) {
      ...
      for (int r = 1; r < 32; r++)               // r0 is hardwired 0; a change there would be a core bug
        if (now.gpr[r] != prev.gpr[r]) fprintf(out, " %s=0x%08X", kReg[r], now.gpr[r]);
      if (now.lo != prev.lo) fprintf(out, " lo=0x%08X", now.lo);
      if (now.hi != prev.hi) fprintf(out, " hi=0x%08X", now.hi);
      fputc('\n', out);
    }
    ...
    if (left_text_step < 0 && (now.pc < t_addr || now.pc >= t_addr + present)) {
      left_text_at   = now.pc;
      left_text_step = n;
      fprintf(out, "# LEFT THE MAPPED TEXT at step %ld: pc=0x%08X is outside 0x%08X..0x%08X\n",
              n, now.pc, t_addr, t_addr + present);
      ...
      if (last_jal_step >= 0)
        fprintf(out, "# BOUNDARY-LAST-JAL target=0x%08X ra=0x%08X step=%ld\n",
                last_jal_target, last_jal_ra, last_jal_step);
      else
        fprintf(out, "# BOUNDARY-LAST-JAL none — execution left the text without a jal having been\n"
    ...
    if (now.gpr[31] != prev.gpr[31]) { jal_pending = 1; jal_pending_ra = now.gpr[31]; }

    if (stop != ORACLE_STOP_BUDGET) break;
    prev = now;
  }
  ...
  fprintf(out, "# traced %ld of %ld requested step(s), %d cycle(s), ended pc=0x%08X\n",
          traced, steps, fin.timestamp, fin.pc);
  fprintf(out, "# stop reason: %s\n", oracle_stop_name(stop));
  if (stop == ORACLE_STOP_HARDWARE)
    fprintf(out, "# hardware address: 0x%08X\n", fin.stop_addr);
  if (left_text_step >= 0)
    fprintf(out, "# left mapped text at step %ld (pc=0x%08X); everything after that is NOT game code from\n"
                 "#   this image, and no compare should treat it as such\n", left_text_step, left_text_at);
  else
    fprintf(out, "# NEVER LEFT the mapped text in %ld traced step(s) — no boundary was reached, so this\n"
                 "#   trace contains NO BOUNDARY-REG block and a cross-check against the boot group cannot\n"
                 "#   be performed from it. Raise --steps.\n", traced);
  if (summary_only)
    fprintf(out, "# --summary-only: per-step lines were omitted deliberately. This trace CANNOT be used\n"
  ...
  fprintf(stderr, "oracle_trace: %ld of %ld step(s) traced, %d cycle(s), ended pc=0x%08X (%s)\n",
          traced, steps, fin.timestamp, fin.pc, oracle_stop_name(stop));
  if (left_text_step >= 0)
    fprintf(stderr, "  left the mapped text at step %ld -> pc=0x%08X. That is the end of the straight-line\n"
                    "  window; see the plan's BIOS-call boundary (milestone 3).\n", left_text_step, left_text_at);
  if (out_path) fprintf(stderr, "  trace: %s\n", out_path);
