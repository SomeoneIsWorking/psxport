...
static int rd(uint32_t lba, uint8_t *o) {
  uint32_t hk = lba / fph, off = (lba % fph) * RAW;
  if (hk >= hc) {
    return 0;
  }
  if (hk != cached) {
...
int main(int argc, char **argv) {
  ...
  uint32_t cnt[256] = {0}, lo[256], hi[256];
  int code[256];
  for (int i = 0; i < 256; i++) {
    lo[i] = 0xFFFFFFFF;
    hi[i] = 0;
  ...
  for (uint32_t lba = 0; lba < total; lba++) {
    ...
    uint8_t chan = raw[17], sub = raw[18];
    int cd = raw[19];
    if (!(sub & 0x04)) {
      continue; // audio only
    }
