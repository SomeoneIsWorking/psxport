...
def main():
    ...
    with open(args[0]) as f:
        for ln in f:
            ...
            if section == "time" and len(parts) >= 2:
                a = int(parts[0], 16); n = int(parts[1])
                f0 = enclosing(addrs, a)
                key = f0 if (f0 is not None and a - f0 <= 0x8000) else a  # group overlay by raw addr
    ...
    print(f"=== {header} ===\n")
    print(f"--- TIME by function (top {top}; % of {total_time} sampled insns) ---")
