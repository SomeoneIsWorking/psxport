...
static void begin(const char *name) {
  printf("  [%d/%d] %-34s ... ", ++g_run, PLAN, name);
}
...
#define SMOKE_EQ(name, got, want)                                                                                      \
  do {                                                                                                                 \
    ...
    if (g_ == w_)                                                                                                      \
      ...
      fail("%s: got 0x%08X want 0x%08X", #got, g_, w_);                                                                \
  } while (0)
...
int main() {
  ...
  SMOKE_TRUE("main-ram-kuseg-kseg1-mirror", c->mem_r32(0x00010000u) == val && c->mem_r32(0xA0010000u) == val);
  ...
  // RAN with no Game bound, not merely that the store did not crash.
  {
    ...
    if (slot < 0) {
      ...
      fail("watchLookup did not report the region as watched (before=%d after=%d)", (int)had_before, (int)had_after);
    } else if (before.frame != 0xFFFFFFFFu) {
      fail("fresh watch word was not 'never written' (frame 0x%08X) — no denominator for the result", before.frame);
