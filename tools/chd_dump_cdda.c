...
#define RAW 2448u
static chd_file *C;
static uint32_t fph, hc, hb, cached = 0xFFFFFFFFu;
static uint8_t *hbuf;
static int openc(const char *p) {
  if (chd_open(p, CHD_OPEN_READ, 0, &C) != CHDERR_NONE) {
    return 0;
  }
  const chd_header *h = chd_get_header(C);
  hb = h->hunkbytes;
  fph = hb / RAW;
  hc = h->totalhunks;
  hbuf = malloc(hb);
...
static int rd(uint32_t frame, uint8_t *o) {
  uint32_t hk = frame / fph, off = (frame % fph) * RAW;
  if (hk >= hc) {
    return 0;
  }
  if (hk != cached) {
    if (chd_read(C, hk, hbuf) != CHDERR_NONE) {
      return 0;
    }
    cached = hk;
  }
  memcpy(o, hbuf + off, 2352);
  return 1;
}
...
int main(int argc, char **argv) {
  ...
  for (uint32_t ti = 0; ti < 99; ti++) {
    ...
    printf("TRACK %d  TYPE=%s  FRAMES=%d  chd_frame_base=%u  %s\n",
           ...
           frames,
           frame_base,
           isaudio ? "-> dumping CDDA" : "(data)");
    if (isaudio) {
      char path[600];
      snprintf(path, sizeof path, "%s/track%02d.wav", outdir, trk);
      FILE *f = fopen(path, "wb");
      if (!f) {
        fprintf(stderr, "open %s (mkdir -p %s)\n", path, outdir);
        return 1;
      }
