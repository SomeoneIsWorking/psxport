...
def ignored_paths(repo: str, roots: tuple[str, ...]) -> tuple[set[str], bool]:
    ...
    Source-tree tools can leave gitignored generated artifacts under the scanned roots. Including
    those artifacts makes the denominator depend on which tools ran locally, and a reported line
    would name output no human can edit. Scope is SOURCE, so the ignored set is subtracted."""
    try:
        import subprocess
        r = subprocess.run(
