...
static int disc_open(const char *p) {
  if (chd_open(p, CHD_OPEN_READ, 0, &s_chd) != CHDERR_NONE) {
    return 0;
  }
...
static int disc_read_raw(uint32_t lba, uint8_t *o, uint32_t n) {
  if (n > 2352u) {
    n = 2352u;
  }
  uint32_t hk = lba / s_fph, off = (lba % s_fph) * RAW_FRAME;
  if (hk >= s_hcount) {
    return 0;
  }
  if (hk != s_cached) {
    if (chd_read(s_chd, hk, s_hbuf) != CHDERR_NONE) {
      return 0;
    }
    s_cached = hk;
  }
  memcpy(o, s_hbuf + off, n);
  return 1;
}
...
int main(int argc, char **argv) {
  ...
  long frames[NCH] = {0};
  int freq[NCH] = {0};
  int16_t hist[NCH][2][2];
  memset(hist, 0, sizeof hist);
  char path[512];
  for (int c = 0; c < NCH; c++) {
    snprintf(path, sizeof path, "%s/ch%02d.wav", outdir, c);
    f[c] = fopen(path, "wb");
    if (!f[c]) {
      fprintf(stderr, "mkdir -p %s first\n", outdir);
      return 1;
    }
    for (int i = 0; i < 44; i++) {
      fputc(0, f[c]);
  ...
  for (uint32_t lba = 0; lba < total; lba++) {
    ...
    int fr;
    int n = xa_decode_sector(raw, pcm, hist[c], &fr);
    freq[c] = fr;
    fwrite(pcm, sizeof(int16_t) * 2, n, f[c]);
    frames[c] += n;
  }
  printf("# channel : seconds : freq\n");
  for (int c = 0; c < NCH; c++) {
    uint32_t datasz = (uint32_t)frames[c] * 4, riff = 36 + datasz, srate = freq[c] ? freq[c] : 37800,
             byterate = srate * 4, fmtlen = 16;
    ...
    fwrite("WAVE", 1, 4, f[c]);
    fwrite("fmt ", 1, 4, f[c]);
    fwrite(&fmtlen, 4, 1, f[c]);
    fwrite(&fmt, 2, 1, f[c]);
    fwrite(&ch, 2, 1, f[c]);
    fwrite(&srate, 4, 1, f[c]);
    fwrite(&byterate, 4, 1, f[c]);
    fwrite(&blk, 2, 1, f[c]);
    fwrite(&bps, 2, 1, f[c]);
    fwrite("data", 1, 4, f[c]);
    fwrite(&datasz, 4, 1, f[c]);
    fclose(f[c]);
    if (frames[c]) {
      printf("  ch%02d : %6.1fs : %dHz\n", c, frames[c] / (double)(freq[c] ? freq[c] : 37800), freq[c]);
    }
  }
  return 0;
}
