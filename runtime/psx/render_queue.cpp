...
static inline int objid_quads_on(Core *c) {
  const Mods &m = c->game->mods;
  return m.debug_quads || m.debug_ids || g_objid_ch.enabled();
}
static inline int objid_objects_on(Core *c) {
  const Mods &m = c->game->mods;
  return m.debug_objects || m.debug_ids || g_objid_ch.enabled();
}
static inline int objid_on(Core *c) {
  return objid_quads_on(c) || objid_objects_on(c);
}
...
bool RenderQueue::objidSolid(
  ...
  *it = RqItem{}; // zero every field (fps_*, sh_cast, has_xyf, depth, ...)
  ...
  it->tp_x = ref->tp_x;
  it->tp_y = ref->tp_y;
  it->clut_x = ref->clut_x;
  it->clut_y = ref->clut_y;
  it->tw_mx = ref->tw_mx;
  it->tw_my = ref->tw_my;
  it->tw_ox = ref->tw_ox;
  it->tw_oy = ref->tw_oy;
...
void RenderQueue::objidStr(Core *core,
                           const RqItem *ref,
                           int x,
                           int y,
                           int s,
...
void RenderQueue::objidOverlay(Core *core) {
  ...
  // object's world position to screen. (Same data native_terrain published; terrain is orphaned now.)
  {
    ...
    float R[3][3] = {{(int16_t)k0 / 4096.0f, (int16_t)(k0 >> 16) / 4096.0f, (int16_t)k1 / 4096.0f},
    ...
    float T[3] = {(float)(int32_t)core->mem_r32(S + 0x10C),
                  (float)(int32_t)core->mem_r32(S + 0x110),
                  (float)(int32_t)core->mem_r32(S + 0x114)};
    camview_publish(R, T);
  }
  ...
  int dolog = objid_on(core) && ((s_logframe++ % 120) == 0);
  ...
  for (int li = 0; li < 3; li++) {
    for (uint32_t n = core->mem_r32(HEADS[li]), g = 0; n >= 0x80000000u && n < 0x80200000u && g < 512;
      ...
      // render dispatcher (guest_8003C048) routes render types 0x10..0x14 to the SPRITE/BILLBOARD
      ...
      if (quad ? !objid_quads_on(core) : !objid_objects_on(core)) {
        continue; // class toggled off
      }
      float sx = 0, sy = 0;
      if (!proj_camview_world_screen((float)wx, (float)wy, (float)wz, &sx, &sy)) {
        continue; // behind camera
      }
      int cx = (int)(sx + 0.5f), cy = (int)(sy + 0.5f);
      if (cx < -60 || cx > 420 || cy < -40 || cy > 280) {
        continue; // off-screen
      }
      if (quad) {
        nquad++;
      ...
      if (dolog && quad) {
        lucent::info("objid-q",
                     "node={:08X} rtype=0x{:02X} scr=({},{}) world=({},{},{}) +0xC={:02X} +0xD={:02X}",
                     n,
                     rtype,
                     (int)(sx + 0.5f),
                     (int)(sy + 0.5f),
  ...
  if (dolog) {
    lucent::info("objid", "=== {} live; {} quads + {} 3D boxed ===", nlive, nquad, nobj);
  }
}

// The render queue is THE render path — one behavior, the PC game. No env gate (user directive
// 2026-06-20: "have only one behavior that is PC game"). The lone exception is the differential test dual-channel
...
void RenderQueue::mark_consumed() {
  if (n) {
    consumed = 1;
  }
}
...
void RenderQueue::histogram() {
  ...
  for (int i = 0; i < n; i++) {
    int L = items[i].layer & 3, sm = items[i].semi ? 1 : 0;
    c[L][sm]++;
  }
  static int lf = 0;
  if ((lf++ % 30) == 0) {
    lucent::debug(rqhist_ch,
                  "n={}  bg(op/semi)={}/{}  WORLD={}/{}  ovl={}/{}  hud={}/{}",
                  n,
                  c[0][0],
                  c[0][1],
                  c[1][0],
...
void RenderQueue::zfightScan(Core *core) {
  static float eps = -1.f;
  static int scan_from = -1;
  if (eps < 0.f) {
    const char *e = cfg_str("PSXPORT_ZFIGHT");
    if (!e) {
      eps = -2.f;
      ...
      const char *f = cfg_str("PSXPORT_ZFIGHT_FRAME");
      scan_from = f ? atoi(f) : 0;
    }
  }
  if (eps < 0.f) {
    return; // -2 = disabled
  ...
  if (boxset < 0) {
    boxset = 0;
    const char *bb = cfg_str("PSXPORT_ZFIGHT_BOX");
    if (bb && sscanf(bb, "%d,%d,%d,%d", &bx0, &by0, &bx1, &by1) == 4) {
      boxset = 1;
    }
  }
  ...
  auto edge = [](float ax, float ay, float x0, float y0, float x1, float y1) {
  ...
  for (int k = 0; k < W * H; k++) {
    ...
    if (gap < eps) {
      ...
      for (int u = 0; u < 4; u++) {
        if (d_later + later_idx * Usw[u] >= d_earlier + earlier_idx * Usw[u]) {
          ps_b[u]++;
          if (tie) {
            ptie_b[u]++;
          }
      ...
      nfight++;
      heat[k * 3] = 255;
      heat[k * 3 + 1] = (unsigned char)fminf(255.f, gap / eps * 255.f);
      ...
      if (found < 0) {
        pairs.push_back({a, b, 1, inv, gap});
      } else {
        pairs[found].cnt++;
        pairs[found].inv += inv;
        pairs[found].gap = fminf(pairs[found].gap, gap);
      }
    }
  }
  std::sort(pairs.begin(), pairs.end(), [](const Pair &a, const Pair &b) {
    return a.inv != b.inv ? a.inv > b.inv : a.cnt > b.cnt;
  });
  auto pc = [](int a, int b) {
    return b ? 100.f * a / b : 0.f;
  };
  ...
  if (!candidates) {
    lucent::info("zfight",
                 "f{} REFUSED: scanned 0 depth candidate(s) of {} queue item(s) — this says "
  ...
  lucent::info(
      "zfight",
      "f{} scanned={} of {} item(s) eps={:.6g} fight={} ties(<1e-5)={} | ALL paint-stable raw={:.0f}% U4e7={:.0f}% "
      "U1e6={:.0f}% U4e6={:.0f}% U1e5={:.0f}% | TIES raw={:.0f}% U4e7={:.0f}% U1e6={:.0f}% U4e6={:.0f}% U1e5={:.0f}%",
      s.s_frame,
      candidates,
      n,
      eps,
      ...
      pc(ptie_b[2], ntie),
      pc(ptie_b[3], ntie));
  auto vd = [](const RqItem &P, int i) {
    return P.depth ? P.depth[i] : -1.f;
  };
  for (size_t i = 0; i < pairs.size() && i < 10; i++) {
    const RqItem &A = items[pairs[i].a];
    const RqItem &B = items[pairs[i].b];
    int an = A.nv ? A.nv : 4, bn = B.nv ? B.nv : 4;
    lucent::info("zfight", "  pair px={} order-inverted={} gap>={:.7f}", pairs[i].cnt, pairs[i].inv, pairs[i].gap);
    lucent::info("zfight",
                 "    A node={:08X} key={} key_ord={:.6f} col=({},{},{}) seq={} nv={} xyf={} vdepth=[{:.6f} {:.6f} "
                 "{:.6f} {:.6f}] xy=[({},{})({},{})({},{})({},{})]",
                 A.dbg_node,
                 A.sort_key,
                 (double)A.key_ord,
                 ...
                 an == 4 ? A.ys[3] : 0);
    lucent::info("zfight",
                 "    B node={:08X} key={} key_ord={:.6f} col=({},{},{}) seq={} nv={} xyf={} vdepth=[{:.6f} {:.6f} "
                 "{:.6f} {:.6f}] xy=[({},{})({},{})({},{})({},{})]",
                 B.dbg_node,
                 B.sort_key,
                 (double)B.key_ord,
  ...
  if (nfight > 0) {
    char path[128];
    snprintf(path, sizeof path, "scratch/screenshots/zfight/heat_f%d.ppm", s.s_frame);
    FILE *fp = fopen(path, "wb");
    if (fp) {
      fprintf(fp, "P6\n%d %d\n255\n", W, H);
      fwrite(heat.data(), 3, W * H, fp);
      fclose(fp);
...
void RenderQueue::flush(Core *core) {
  // The queue payload is retained after a flush so the next producer can reset it lazily in push().
  // Retained storage is not a pending submission: a later empty DrawOTag must not sort, diagnose,
  // ledger-count, capture, or emit the same items again. Checking this lifecycle state before every
  // flush side effect keeps one physical draw boundary from being replayed as another.
  if (consumed) {
    return;
  }
  // `debug rqflush`: what this flush is ACTUALLY emitting. The `reemit` field is retained for diagnostic
...
void RenderQueue::emitItem(Core *core, const RqItem *it) {
  ...
  // this instrument doubles as the interp pass's quality debugger (docs/fps60-rework.md).
  {
    ...
    if (pi >= 0) {
      ...
      lucent::debug("preseqobj",
                    "p{:04} key={:08X} layer={} x={} y={} scene={}",
                    pi,
                    it->dbg_node,
                    it->layer,
                    it->xs[0],
  ...
  RqItem pw;
  {
    static int p = -2;
    if (p == -2) {
      const char *e = cfg_str("PSXPORT_PAINTWORLD");
      p = e ? atoi(e) : 0;
    }
    if (p && it->layer == RQ_WORLD && !it->semi) {
      pw = *it;
      pw.mode = 3;
      pw.raw = 0;
      for (int i = 0; i < 4; i++) {
  ...
  // answer to "is the world built but occluded, or is it not landing on-screen?" (diag, 2026-06-26; OPEN #1)
  {
    static int o = -2;
    if (o == -2) {
      const char *e = cfg_str("PSXPORT_ONLYWORLD");
      o = e ? atoi(e) : 0;
    }
    if (o && it->layer != RQ_WORLD) {
      return;
    }
  }
  ...
  // world becomes visible, the backdrop is the occluder (despite its far 2D-BG ord). (diag, 2026-06-26)
  {
    static int nb = -2;
    if (nb == -2) {
      const char *e = cfg_str("PSXPORT_NOBG");
      nb = e ? atoi(e) : 0;
    }
    if (nb && it->layer == RQ_BACKGROUND) {
      return;
    }
  }
  ...
  // is being MIS-CLASSIFIED as HUD (nearest band) and occluding the world. (diag, 2026-06-26; OPEN #1)
  {
    static int nh = -2;
    if (nh == -2) {
      const char *e = cfg_str("PSXPORT_NOHUD");
      nh = e ? atoi(e) : 0;
    }
    if (nh && it->layer == RQ_HUD) {
      return;
    }
  }
  ...
  if (it->sh_cast) {
    ...
    if ((it->nv ? it->nv : 4) == 4) {
      gpu_vk_shadow_push_tri(core, v[1], v[2], v[3]);
    }
  }
  const int *xs = it->xs;
  const int *ys = it->ys;
  const int *us = it->us;
  const int *vs = it->vs;
  const unsigned char *rs = it->rs;
  const unsigned char *gs = it->gs;
  ...
  if (om == RQ_OM_2D_BG) {
    gpu_vk_set_order_2d_bg(core, ord);
  } else if (om == RQ_OM_2D_FG) {
    gpu_vk_set_order_2d(core, ord);
  }
#define RQ_SETVD(p)                                                                                                    \
  do {                                                                                                                 \
    if (om == RQ_OM_DEPTH)                                                                                             \
...
#define RQ_SETXYF(o)                                                                                                   \
  do {                                                                                                                 \
    gpu_vk_set_xyf(core, xsf ? xsf + (o) : nullptr, ysf ? ysf + (o) : nullptr);                                        \
  } while (0)
  if (it->semi) {
...
Rq2dXform rq_2d_xform(int ww, int native_w, Rq2dSpace space, int layer, bool flat, bool untextured) {
  Rq2dXform t;
  if (space == RQ_2D_WIDE_FINAL) {
    return t; // already in the wide frame — finished
  }
  if (native_w <= 0) {
    native_w = 320;
  }
  ...
  if (margin <= 0) {
    return t; // 4:3, or a "wide" width that is not wider
  }
  if (layer == RQ_BACKGROUND && flat && untextured) {
    t.stretch = true;
    t.num = ww;
    t.den = native_w;
  } else {
...
static inline float rq_vx(const RqItem &it, int k) {
  return it.has_xyf ? it.xsf[k] : (float)it.xs[k];
}
static inline float rq_vy(const RqItem &it, int k) {
  return it.has_xyf ? it.ysf[k] : (float)it.ys[k];
}
...
static bool rq_faces_coincident(const RqItem &A, const RqItem &B) {
  ...
  for (int i = 0; i < nv; i++) {
    ...
    for (int j = 0; j < nv && m < 0; j++) {
      if (!used[j] && rq_vx(A, i) == rq_vx(B, j) && rq_vy(A, i) == rq_vy(B, j) && A.depth[i] == B.depth[j]) {
        m = j;
      }
    }
    if (m < 0) {
...
static RqFaceExtent rq_face_extent(const RqItem &it) {
  ...
  for (int k = 1; k < nv; k++) {
    ...
    if (it.depth[k] < e.omin) {
      e.omin = it.depth[k];
    }
    if (it.depth[k] > e.omax) {
      e.omax = it.depth[k];
    }
  }
  return e;
...
void RenderQueue::resolveKeyOrderFaces(uint32_t frame, const char *who, int faceOrder) {
  ...
  std::stable_sort(faces.begin(), faces.end(), [](const KeyedFace &a, const KeyedFace &b) {
...
void RenderQueue::drawWorldQuad(Core *core,
                                const float *px,
                                const float *py,
                                const float *depth,
                                const int *u,
  ...
  core->rsub.stats.dbgWorldQuads++; // PSXPORT_GPU_TRACE: world quads this frame (differential test diag)
  ...
  {
    static const lucent::Channel silbbox{"silbbox"};
    static int once = 0;
    if (silbbox && !once++) {
      lucent::debug("silbbox", "s_off=({},{})", core->game->gpu.s_off_x, core->game->gpu.s_off_y);
    }
  }
  GpuState &s = core->game->gpu;
...
void RenderQueue::push2dQuad(int layer,
                             int order_2d_fg,
                             const int *xs,
                             const int *ys,
                             const int *us,
