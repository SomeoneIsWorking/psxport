...
struct GameConfig {
  ...
  // Guest MAIN .text range, masked to physical addresses. Adapter input only: the live overlay
  ...
  // gap: a port whose boot runs through the guest executor has its movies played by the guest, and
  ...
  struct PlatformHleCfg {
    ...
    // the guest bodies run, which leaves ProjParams unset and makes the native camera's
