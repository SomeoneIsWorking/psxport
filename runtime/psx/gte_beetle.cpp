...
// guest GTE interface (r3000.h) to Beetle's GTE_* API and provides faithful-first stubs for
...
// run as guest PSX and call gte_op). The GTE/Beetle dependency goes away by PORTING ITS CALLERS to
...
// The executor routes only projected screen-coordinate stores here; every other COP2 store keeps
// the ordinary memory path, so this seam costs nothing on them.
//
...
// Native depth, mfc2 form. The runtime translator pairs `mfc2 rX, DR12..15` with the
// `sw rX, off(base)` that writes it into the primitive packet and invokes this seam immediately
// after the store. `addr` is what was written; `zreg` is the Z_FIFO register proven to pair with it.
//
// The pairing is accepted only when nothing between the mfc2 and the store could invalidate it — no
...
//
// Keyed by GPR because the executor proved the pairing: between the hold and the consume the GPR is
...
//
// THAT ASYMMETRY IS THE WHOLE SAFETY PROPERTY. This runs on every `lw`->`sw` pair the executor
