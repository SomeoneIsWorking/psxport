// Native memory-card backend. Memcard owns the host card image, descriptor table, and directory
// operations; BIOS libcard/libmcrd handlers preserve the guest-visible synchronous frame and event
// contracts while routing storage through that owner.
...
char *Memcard::resolvePath(const GameConfig *cfg) {
  ...
  if (cfg && cfg->cardDefaultPath && *cfg->cardDefaultPath) {
    return mc_dup_trim(cfg->cardDefaultPath);
  }
...
bool Memcard::readFrame(uint32_t frame, uint8_t *out128) {
  ...
  if (!mCard) {
    lucent::error("card", "read frame {}: no card image is open ({})", frame, mPath);
    return false;
  }
  if (frame >= kFrames) {
...
bool Memcard::writeFrame(uint32_t frame, const uint8_t *in128) {
  ...
  if (!mCard) {
    lucent::error("card", "write frame {}: no card image is open ({})", frame, mPath);
    return false;
  }
  if (frame >= kFrames) {
...
int Memcard::dirFind(const char *name) {
  ...
  for (uint32_t blk = 1; blk < kBlocks; blk++) {
    ...
    readFrame(blk, e);
    if (e[0] != kDirUsedFirst) {
      continue;
    }
    char fn[0x80];
    size_t k = 0;
    for (uint32_t j = 0x0A; j < kFrameSize && e[j] && k + 1 < sizeof fn; j++) {
      fn[k++] = (char)e[j];
    }
    fn[k] = 0;
    if (strcmp(fn, want) == 0) {
...
int Memcard::dirCreate(const char *name, uint32_t size) {
  ...
  for (uint32_t blk = 1; blk < kBlocks; blk++) {
    ...
    memset(e, 0, sizeof e);
    e[0] = kDirUsedFirst;
    e[4] = (uint8_t)(size & 0xFF);
    e[5] = (uint8_t)((size >> 8) & 0xFF);
    e[6] = (uint8_t)((size >> 16) & 0xFF);
    e[7] = (uint8_t)((size >> 24) & 0xFF);
    e[8] = 0xFF;
    e[9] = 0xFF; // no next-block link
    size_t k = 0;
    for (; bare[k] && (0x0A + k) < kFrameSize - 1; k++) {
      e[0x0A + k] = (uint8_t)bare[k];
    }
    uint8_t x = 0;
    for (uint32_t i = 0; i < 0x7F; i++) {
      x ^= e[i];
...
bool Memcard::dirScanNext(Core *c, uint32_t direntVa) {
  ...
  for (; mScanBlk < kBlocks; mScanBlk++) {
    uint8_t e[kFrameSize];
    if (!readFrame(mScanBlk, e)) {
      continue;
    }
    if (e[0] != kDirUsedFirst) {
      continue; // free, or a continuation block of a longer file
    }
    char fn[0x80];
    size_t k = 0;
    for (uint32_t j = 0x0A; j < kFrameSize && e[j] && k + 1 < sizeof fn; j++) {
    ...
    fn[k] = 0;
    if (!mc_glob(mScanPat, fn)) {
      continue;
    }
    const uint32_t size = (uint32_t)e[4] | ((uint32_t)e[5] << 8) | ((uint32_t)e[6] << 16) | ((uint32_t)e[7] << 24);
    for (uint32_t i = 0; i < kDirEntBytes; i++) {
      c->mem_w8(direntVa + i, 0);
    }
    for (uint32_t i = 0; i < kDirEntNameLen - 1 && fn[i]; i++) {
      c->mem_w8(direntVa + i, (uint8_t)fn[i]);
    }
...
static void card_status(Core *c) {
  c->r[V0] = 1;
}
...
static void file_open(Core *c) {
  ...
  if (blk < 0 && (mode & 0x0200u)) {
    uint32_t nblocks = (mode >> 16) & 0xFFFFu;
    if (!nblocks) {
      nblocks = 1;
    }
...
static void file_lseek(Core *c) {
  Memcard &m = c->game->memcard;
  int fd = (int)c->r[A0];
  int32_t off = (int32_t)c->r[A1];
  uint32_t whence = c->r[A2];
  if (fd >= 0 && fd <= 2) {
    c->r[V0] = 0;
    return;
  } // console device fds: no-op
  auto *f = m.fdAt(fd);
  if (!f) {
    c->r[V0] = 0xFFFFFFFFu;
    return;
...
// Spyro's read op state machine at retail address 0x80066F34 is
//
//     clear_card_events(); do { v0 = read(fd, buf, len); } while (v0 != 0); state = wait_for_event;
//
// and its write op (guest_800671F0) is the same shape. So 0 means "accepted", non-zero means
...
static void file_read(Core *c) {
  Memcard &m = c->game->memcard;
  int fd = (int)c->r[A0];
  uint32_t buf = c->r[A1], len = c->r[A2];
  if (fd >= 0 && fd <= 2) {
    c->r[V0] = 0;
    return;
  }
  ...
  uint8_t fr[Memcard::kFrameSize];
  uint32_t loaded_frame = 0xFFFFFFFFu;
  bool ok = true;
  for (uint32_t i = 0; i < len; i++) {
    uint32_t off = f->pos + i;
    uint32_t gframe = mc_data_frame(f->block, off), boff = off % Memcard::kFrameSize;
    if (gframe != loaded_frame) {
      ok = m.readFrame(gframe, fr) && ok;
      loaded_frame = gframe;
    }
  ...
  if (ok) {
    ...
    Memcard::deliverError(c);
  }
}

// B0:0x35 write(fd, buf, len).
static void log_guest_file_output(Core &core, int fd, uint32_t guestBuffer, uint32_t length) {
  std::string text;
  text.reserve(length);
  for (uint32_t index = 0; index < length; ++index) {
    text.push_back(static_cast<char>(core.mem_r8(guestBuffer + index)));
  }
  lucent::log(fd == 2 ? lucent::Level::Error : lucent::Level::Info, fd == 2 ? "guest-stderr" : "guest-stdout", text);
}
...
static void file_write(Core *c) {
  ...
  if (fd == 1 || fd == 2) { // stdout/stderr
    log_guest_file_output(*c, fd, buf, len);
    c->r[V0] = len;
    return;
  }
  auto *f = m.fdAt(fd);
  ...
  uint8_t fr[Memcard::kFrameSize];
  uint32_t cur_frame = 0xFFFFFFFFu;
  bool ok = true;
  for (uint32_t i = 0; i < len; i++) {
    uint32_t off = f->pos + i;
    uint32_t gframe = mc_data_frame(f->block, off), boff = off % Memcard::kFrameSize;
    if (gframe != cur_frame) { // read-modify-write frames
      if (cur_frame != 0xFFFFFFFFu) {
        ok = m.writeFrame(cur_frame, fr) && ok;
      }
      ok = m.readFrame(gframe, fr) && ok;
      cur_frame = gframe;
    }
  ...
  if (cur_frame != 0xFFFFFFFFu) {
    ok = m.writeFrame(cur_frame, fr) && ok; // flush the last partial frame
  }
  f->pos += len;
...
extern "C" int card_hle_b0(uint32_t fn, Core *c) {
  ...
  case 0x32u:
    file_open(c);
    return 1; // open(name, mode)
  case 0x33u:
    file_lseek(c);
    return 1; // lseek(fd, off, whence)
  case 0x34u:
    file_read(c);
    return 1; // read(fd, buf, len)
  case 0x35u:
    file_write(c);
  ...
  case 0x42u:
    file_firstfile(c);
    return 1; // firstfile(path, dirent)
  case 0x43u:
    file_nextfile(c);
    return 1; // nextfile(dirent)
  case 0x45u:
    file_erase(c);
    return 1; // erase(name)
  case 0x4Cu:
