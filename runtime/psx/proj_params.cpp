...
void ProjParams::requireGeom(const char *who, float &ofx, float &ofy, float &H) const {
  if (!geomValid()) {
    lucent::error("proj",
                  ...
                  who,
                  mGeomOffsetSet ? "ran" : "NEVER RAN",
                  mGeomScreenSet ? "ran" : "NEVER RAN");
    void *bt[32];
    int nbt = backtrace(bt, 32);
    psxport::host::emitBacktrace(lucent::Level::Error, "proj", bt, nbt, 1);
    abort();
  }
  ofx = mGeomOfx;
  ofy = mGeomOfy;
...
float proj_near_pz(void) {
  auto *pp = ProjParams::current();
  return pp ? pp->projNearPz() : 1.0f;
}
float proj_plane_h(void) {
  auto *pp = ProjParams::current();
  return pp ? pp->projPlaneH() : 1.0f;
}
void proj_screen_center(float *cx, float *cy) {
  auto *pp = ProjParams::current();
  if (pp) {
    pp->projScreenCenter(cx, cy);
  } else {
    if (cx) {
      *cx = 160.0f;
    }
