...
void OtAttr::sampleChain(Core *c) {
  mChainSeen++;
  const int depth = c->callAttribution.depth();
  const int vis = c->callAttribution.visibleDepth();
  if (vis == 0) {
    mChainBlind++;
    return;
  } // over OTATTR_CAP: the chain is not knowable, and is COUNTED
  const uint32_t top = c->callAttribution.frameFromTop(0);
  ...
  for (int i = 0; i < s.nframes; i++) {
    s.frames[i] = c->callAttribution.frameFromTop(i);
  }
}
...
void OtAttr::reportChains(const uint32_t *claims, int nclaims) const {
  ...
  lucent::info("otchain",
               ...
               CHAIN_SLOTS,
               mChainBlind,
               psx::cpu::GuestCallAttribution::Capacity,
               nclaims,
               CHAIN_FRAMES);
  if (nclaims == 0) {
    lucent::warn("otchain",
