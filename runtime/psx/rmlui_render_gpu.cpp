...
bool RmlRenderInterfaceGpu::Init(SDL_GPUDevice *dev, SDL_GPUTextureFormat swap_fmt) {
  ...
  vci.code = (const Uint8 *)spv_g_rml_vert;
  vci.code_size = spv_g_rml_vert_len;
  vci.entrypoint = "main";
  ...
  fci.code = (const Uint8 *)spv_g_rml_frag;
  fci.code_size = spv_g_rml_frag_len;
  fci.entrypoint = "main";
  ...
  if (!vs || !fs) {
    lucent::error("rmlgpu", "shader create failed: {}", SDL_GetError());
    return false;
  }

  SDL_GPUVertexBufferDescription vbd = {};
  vbd.slot = 0;
  vbd.pitch = (Uint32)sizeof(Rml::Vertex);
  vbd.input_rate = SDL_GPU_VERTEXINPUTRATE_VERTEX;
  SDL_GPUVertexAttribute attrs[3] = {};
  attrs[0].location = 0;
  attrs[0].buffer_slot = 0;
  attrs[0].format = SDL_GPU_VERTEXELEMENTFORMAT_FLOAT2;
  attrs[0].offset = (Uint32)offsetof(Rml::Vertex, position);
  attrs[1].location = 1;
  ...
  gp.vertex_input_state.num_vertex_attributes = 3;
  ...
  SDL_ReleaseGPUShader(mDev, vs);
  SDL_ReleaseGPUShader(mDev, fs);
  if (!mPipe) {
    lucent::error("rmlgpu", "pipeline create failed: {}", SDL_GetError());
    return false;
  }
  ...
  if (!mSampler) {
    lucent::error("rmlgpu", "sampler create failed: {}", SDL_GetError());
    return false;
  }
  ...
  if (mReady) {
    lucent::info("rmlgpu", "render interface ready (swap fmt {})", (int)mSwapFmt);
  }
  return mReady;
}
...
Rml::CompiledGeometryHandle RmlRenderInterfaceGpu::CompileGeometry(Rml::Span<const Rml::Vertex> vertices,
  ...
  if (!g.vbo || !g.ibo) {
    if (g.vbo) {
      SDL_ReleaseGPUBuffer(mDev, g.vbo);
    }
    if (g.ibo) {
      SDL_ReleaseGPUBuffer(mDev, g.ibo);
    }
    return 0;
  }
...
void RmlRenderInterfaceGpu::RenderGeometry(Rml::CompiledGeometryHandle geometry,
  ...
  ubo.uViewport[0] = (float)mWinW;
  ubo.uViewport[1] = (float)mWinH;
...
void RmlRenderInterfaceGpu::ReleaseGeometry(Rml::CompiledGeometryHandle geometry) {
  ...
  if (it->second.vbo) {
    SDL_ReleaseGPUBuffer(mDev, it->second.vbo);
  }
  if (it->second.ibo) {
    SDL_ReleaseGPUBuffer(mDev, it->second.ibo);
  }
...
Rml::TextureHandle RmlRenderInterfaceGpu::LoadTexture(Rml::Vector2i &texture_dimensions, const Rml::String &source) {
  ...
  (void)source; // the menu uses only generated (font) textures, no <img> files
  return 0;
}

Rml::TextureHandle RmlRenderInterfaceGpu::GenerateTexture(Rml::Span<const Rml::byte> source,
...
void RmlRenderInterfaceGpu::ReleaseTexture(Rml::TextureHandle texture) {
  ...
  if (it->second.tex) {
    SDL_ReleaseGPUTexture(mDev, it->second.tex);
  }
  mTextures.erase(it);
}

void RmlRenderInterfaceGpu::EnableScissorRegion(bool enable) {
...
void RmlRenderInterfaceGpu::Shutdown() {
  ...
  for (auto &[h, t] : mTextures) {
    if (t.tex) {
      SDL_ReleaseGPUTexture(mDev, t.tex);
    }
  }
  mTextures.clear();
  if (mWhiteTex) {
    SDL_ReleaseGPUTexture(mDev, mWhiteTex);
    mWhiteTex = nullptr;
  }
  if (mSampler) {
    SDL_ReleaseGPUSampler(mDev, mSampler);
    mSampler = nullptr;
  }
  if (mPipe) {
    SDL_ReleaseGPUGraphicsPipeline(mDev, mPipe);
    mPipe = nullptr;
  }
  mReady = false;
  mDev = nullptr;
