...
// PSXPORT_WWATCH=lo,hi — log the guest PC of any store landing in [lo,hi). Also fires
...
static void warn_unmapped_ram(Core *gc, uint32_t a, uint32_t bytes, const char *how) {
  ...
  // a bespoke probe.
  if (gc) {
    // `pc` is the architectural executor state at the most recent bounded exit. If a native
    // service reports a later host-side fault, the instruction that originated that service may
    // already have completed; keep the label explicit instead of claiming this is a host fault PC.
    lucent::error("mem",
                  "  guest: executor-pc=0x{:08X} (not necessarily the later host fault site) "
...
uint32_t Core::io_read(uint32_t a, uint32_t bytes) {
  ...
  if (p >= 0x1F801800 && p <= 0x1F801803) { // CD controller registers
    ...
    // PC/RA are architectural guest state; use cdcbt as additional host-call attribution when the
    // service has already crossed into native code.
    lucent::debug("cdcr",
...
void Core::io_write(uint32_t a, uint32_t v, uint32_t bytes) {
  ...
  if (p >= 0x1F801800 && p <= 0x1F801803) { // CD controller
    ...
    // PC/RA are the executor's architectural state at this native boundary. `PSXPORT_DEBUG=cdcbt`
    // adds host-call attribution when the native service path itself is the question.
    lucent::debug("cdcw",
