...
// batches themselves can be per-Core (differential test's two cores keep separate per-frame geometry).
...
struct GpuVkState {
  ...
  // Under differential test the two cores previously rendered through ONE shared set on the device singleton — a
  ...
  // sprite blits, readback, differential test) stays on the fixed, pixel-exact VRAM texture untouched. i==1 never
  // touches these fields (render_geom stays on the direct s_vram_tex path — no extra blit). Lazily created; torn down +
  // rebuilt by ensure_ires_targets() whenever the live scale changes (RmlUi overlay can flip mods.ires mid-run).
  ...
  // (NULL = fall back to the per-prim OT-order s_cur_ord/s_cur_ordn). ordn = the differential test native channel.
  ...
  // memory so per-Core / differential test isolation comes for free. Native present path reads ScreenFade::get(core).)
  ...
  // by render_geom at grab_pane/present time, and RESET by frame_end. Two differential test cores each keep their
  ...
  void shot_b(const char *path); // differential test: capture target 1 (core B / right pane)
