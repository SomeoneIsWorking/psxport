...
// deliver CD IRQs; instead the native-overrides layer swaps in native bodies for the two
// chokepoints, keeping the original guest bodies alive (A/B + diffable) per the skill.
//
...
static void cd_command(Core *c) {
  ...
  case 0x1B: // ReadN / ReadS
    ...
    // continuous stream. Runtimes without that native owner retain the guest callback burst; the
    // typed ownership query below distinguishes those contracts.
...
static void cd_loadfile(Core *c) {
  ...
  // Keep the guest position tracker consistent with the asynchronous reader: both paths expose the
  // final LBA after a successful read.
  if (nsec) {
  ...
  psx::cpu::notifyExecutableWrite(
      *c, {dest & 0x1fffffffu, (dest & 0x1fffffffu) + done}, psx::cpu::ExecutableWriteSource::ModuleLoad);
  c->r[V0] = size;
}
...
// black. FIX (native-overrides, mirrors cd_loadfile): do the read natively & synchronously.
...
// 0x8001D2A8 FUN_8001d2a8(chan, start_lba, end_lba, flags): native voice/BGM clip owner. It plays
// through xa_stream and owns task slot 2 until playback finishes; the scheduler then clears the
// guest-visible task state. All by-index voice APIs funnel through this operation. flags bit0 = loop.
...
void Cd::toSpuMix(int on) {
  Core *c = &game->core;
  c->r[A0] = on ? 1 : 0;
  psx::cpu::dispatchGuestToReturn0(*c, 0x8001cf00u, psx::cpu::ExecutionBudget::currentTurn(*c), "Cd::toSpuMix");
}
...
static void voice_stop(Core *c) {
  ...
  psx::cpu::dispatchGuestToReturn0(
      *c, 0x8001cf00u, psx::cpu::ExecutionBudget::currentTurn(*c), "voice_stop CD-to-SPU mix");
}
...
//
// The guest libcd init busy-waits: FUN_800898a0 retries FUN_80089930 (CdInit) up to 5 times, and
...
void Cd::pumpStream(Core *c, int sectors) {
  ...
  // drive catches up. The host still gets its turn at every guest function entry, so this
  // cannot wedge.
  {
