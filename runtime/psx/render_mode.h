...
// the geometry, and who rasterized it.
//
// It is ONE enum rather than three switches because the three switches it replaces (mPsxRender, the
// GpuState soft_gpu flag, and an independent enhancement toggle) could spell combinations that are
...
public:
  ...
  // of teeing prims to the VK backend. Was GpuState::soft_gpu, which only the differential test oracle leg and the
