...
//      INDIRECT/jalr guest dispatch — see interp_diag.h), caller fn = one frame below that, node =
...
//      gte_op's existing RTPS/RTPT branch (runtime/psx/gte_beetle.cpp, alongside rtpcaller_record).
//
...
// is that playing populates it. So the hot-path gate opens for
...
public:
  ...
  // `pc` = the guest fn most recently ENTERED when this store happened. Every guest wrapper opens
