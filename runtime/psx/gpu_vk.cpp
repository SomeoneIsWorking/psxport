...
static const char *gpu_fault_marker_path() {
  ...
  built = true;
  const char *xdg = cfg_str("XDG_STATE_HOME");
  const char *home = cfg_str("HOME");
  if (xdg && *xdg) {
  ...
  // Best effort here; gpu_fault_persist reports a precise refusal if the marker cannot be opened.
...
// Add SDL's depth range to the platform-neutral viewport rectangle.
