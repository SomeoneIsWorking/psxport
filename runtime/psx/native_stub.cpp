...
static void scea_dump_ppm(const uint8_t *rgba, float fade01, const char *path) {
  ...
  for (int i = 0; i < SCEA_DISP_W * SCEA_DISP_H; i++) {
    ...
    unsigned char o[3] = {
        (unsigned char)(p[0] * fade01), (unsigned char)(p[1] * fade01), (unsigned char)(p[2] * fade01)};
    fwrite(o, 1, 3, f);
  }
  fclose(f);
...
static void native_scea_splash(Core *c) {
  ...
  for (int f = 0; f < SCEA_FRAMES; f++) {
    ...
    if (f < SCEA_FADE_IN) {
      fade = f * 128 / SCEA_FADE_IN;
    } else if (f < SCEA_FADE_IN + SCEA_HOLD) {
      fade = 128;
    } else {
      fade = (SCEA_FRAMES - 1 - f) * 128 / SCEA_FADE_OUT;
    }
...
void BootStub::run(const char *main_exe_path) {
  ...
  load_exe(main_path, c); // load MAIN.EXE image + initial registers into the Core
