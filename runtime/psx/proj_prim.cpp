...
void ProjPrim::reset() {
  ...
  for (int i = 0; i < mN; i++) {
    if (mEntries[i].gen < mGen - (kGens - 1)) {
      continue; // beyond the retained generations
    }
    Ent e = mEntries[i];
    uint32_t h = hashOf(e.addr);
    mEntries[n] = e;
    mEntries[n].next = mHead[h];
    mHead[h] = n;
    n++;
  }
  mN = n;
...
static inline uint32_t pz_key(uint32_t addr) {
  return addr & 0x1FFFFFFC;
}
...
static inline uint32_t word_at(Core *c, uint32_t addr) {
  return c->mem_r32(addr);
}
...
void ProjPrim::setPz(Core *c, uint32_t addr, float pz) {
  ...
  if (s_pz_dbg_set < 12) {
    s_pz_dbg_set++;
    lucent::debug("pzaddr", "RECORD [{:06X}] pz={:.1f}", addr, (double)pz);
  }
  ...
  for (int i = mHead[hashOf(addr)]; i >= 0; i = mEntries[i].next) {
    if (mEntries[i].addr == addr) {
      if (mEntries[i].word != word_at(c, real)) {
        return false; // the vertex it described is gone
      }
      if (pz) {
        *pz = mEntries[i].pz;
      }
...
bool ProjPrim::lookupPz(Core *c, uint32_t addr, float *pz) {
  ...
  if (s_pz_dbg_miss < 12) {
    s_pz_dbg_miss++;
    lucent::debug("pzaddr", "MISS   [{:06X}]", addr);
  }
  ...
  if (lucent::channel_on("pznear")) {
    ...
    for (int k = 0; k < (int)(sizeof kOff / sizeof kOff[0]); k++) {
      if (peekPz(c, (uint32_t)((int32_t)real + kOff[k]), nullptr)) {
        mNear[k]++;
      }
    }
    mNearMiss++;
...
void ProjPrim::nearReport(const char *tag) {
  ...
  if (!mNearMiss) {
    lucent::debug("pznear", "{}: no misses probed yet", tag ? tag : "(null)");
    return;
  }
  if (!tot) {
    lucent::debug("pznear",
                  "{}: {} misses probed, NONE had a recorded depth within +/-32 bytes — the "
  ...
  for (int k = 0; k < 16 && n < (int)sizeof line - 24; k++) {
    if (mNear[k]) {
      n += snprintf(line + n, sizeof line - n, " %+d:%ld", kOff[k], mNear[k]);
    }
  }
  lucent::debug(
      "pznear", "{}: {} misses probed, {} had a recorded depth nearby —{}", tag ? tag : "(null)", mNearMiss, tot, line);
  for (int k = 0; k < 16; k++) {
    mNear[k] = 0;
  }
  mNearMiss = 0;
