...
// CLEAN guest interface (spu_*) to Beetle's SPU_* API and provides definitions for the
...
// ---------------------------------------------------------------------------
// Clean guest SPU interface -> Beetle SPU.
//
...
void spu_write(uint32_t addr, uint32_t val) {
  ...
  // dispwatch misses static gen-to-gen calls; the host stack names the guest_*/native chain.
  {
    ...
    if (bt && (addr & 0x3FF) == (uint32_t)strtoul(bt, 0, 16)) {
      ...
      psxport::host::emitBacktrace(lucent::Level::Info, "spubt", frames, n, 1);
      lucent::info("spubt", "----");
    }
  }
