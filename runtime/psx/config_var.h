...
//
// The split between this read-only header and config.h keeps the mutation surface small and
// greppable. Runtime commands outrank launch-time overrides because they are later and more
// specific. One optional slot per layer makes `layer()` derive its answer from the actual values and
// lets introspection report every layer at once. CVars self-register in their constructors because
// they are file-scope objects in several translation units; a separate registration phase could be
// skipped and silently disable diagnostics (see tests/test_lucent_channel_env.cpp).
//
...
// Declared here, before the extern-template lines below: declaring an inventory CVar in
