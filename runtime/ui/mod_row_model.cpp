...
const ToggleRow kToggleRows[] = {
    { "aspect",        [](Mods& m) { return &m.aspect; },        kAspect, 4, 0, true  },
    { "ires",          [](Mods& m) { return &m.ires; },          kIres,   5, 1, true  },
    { "fps60",         [](Mods& m) { return &m.fps60; },         kOnOff,  2, 0, true  },
    { "ssao",          [](Mods& m) { return &m.ssao; },          kOnOff,  2, 0, true  },
    { "light",         [](Mods& m) { return &m.light; },         kOnOff,  2, 0, true  },
    { "shadows",       [](Mods& m) { return &m.shadows; },       kOnOff,  2, 0, true  },
    { "debug_ids",     [](Mods& m) { return &m.debug_ids; },     kOnOff,  2, 0, false },
    { "debug_quads",   [](Mods& m) { return &m.debug_quads; },   kOnOff,  2, 0, false },
    { "debug_objects", [](Mods& m) { return &m.debug_objects; }, kOnOff,  2, 0, false },
};
...
const AdjustRow kAdjustRows[] = {
    { "ssao_strength",   [](Mods& m) { return &m.ssao_strength; },   0.05f,   0.0f,  2.0f,  2 },
    { "ssao_radius",     [](Mods& m) { return &m.ssao_radius; },     0.5f,    1.0f, 20.0f,  1 },
    { "ssao_bias",       [](Mods& m) { return &m.ssao_bias; },       0.002f,  0.0f,  0.1f,  3 },
    { "ssao_range",      [](Mods& m) { return &m.ssao_range; },      0.01f,   0.02f, 0.6f,  3 },
    { "light_dir_x",     [](Mods& m) { return &m.light_dir[0]; },    0.05f,  -1.0f,  1.0f,  2 },
    { "light_dir_y",     [](Mods& m) { return &m.light_dir[1]; },    0.05f,  -1.0f,  1.0f,  2 },
    { "light_dir_z",     [](Mods& m) { return &m.light_dir[2]; },    0.05f,  -1.0f,  1.0f,  2 },
    { "light_ambient",   [](Mods& m) { return &m.light_ambient; },   0.05f,   0.0f,  1.5f,  2 },
    { "light_diffuse",   [](Mods& m) { return &m.light_diffuse; },   0.05f,   0.0f,  1.5f,  2 },
    { "shadow_strength", [](Mods& m) { return &m.shadow_strength; }, 0.05f,   0.0f,  1.0f,  2 },
};
...
bool ModRowModel::knows(RowKind kind, std::string_view id) {
    if (kind == RowKind::Toggle) return find_toggle(id) != nullptr;
    if (kind == RowKind::Adjust) return find_adjust(id) != nullptr;
    return false;
}
...
bool ModRowModel::value_text(const Mods& m, RowKind kind, std::string_view id, std::string& out) {
    ...
    if (kind == RowKind::Toggle) {
        ...
        if (!r) return false;
        int v = *r->field(mm);
        if (v < 0 || v >= r->states) v = r->fallback;
        out = r->labels[v];
        return true;
    }
...
int ModRowModel::toggle_count() { return (int)(sizeof(kToggleRows) / sizeof(kToggleRows[0])); }
int ModRowModel::adjust_count() { return (int)(sizeof(kAdjustRows) / sizeof(kAdjustRows[0])); }
