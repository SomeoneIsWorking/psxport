...
void Component::set_selected(bool value) {
  ...
  if (!mRoot || Component::selected() == value) {
    return;
  }
...
void Component::set_disabled(bool value) {
  if (!mRoot || Component::disabled() == value) {
    return;
  }
  if (value) {
...
void Component::listen(Rml::Element *element,
                       Rml::EventId event,
                       ScopedEventListener::Callback callback,
                       bool capture) {
  if (!element) {
    element = mRoot;
  }
