...
public:
    ...
    void step(int) override { if (mMods) ModRowModel::toggle(*mMods, mId); }
private:
    Mods*       mMods;
    std::string mId;
};
...
public:
    ...
    void step(int dir) override { if (mMods) ModRowModel::adjust(*mMods, mId, dir); }
    bool steps_with_arrows() const override { return true; }
private:
    Mods*       mMods;
    std::string mId;
};
...
public:
    ...
    bool text(std::string& out) const override {
        if (!mWarp) return false;
        out = mWarp->currentLabel();
        return true;
    }
    void step(int dir) override { if (mWarp) mWarp->adjust(dir); }
    bool steps_with_arrows() const override { return true; }
private:
    WarpControl* mWarp;
};
...
public:
    ...
    void step(int) override { if (mAction) mAction(); }
private:
...
MenuRow::MenuRow(Rml::Element* root, std::unique_ptr<RowBinding> binding,
...
std::string MenuRow::describe() const {
    ...
    return kind + "=\"" + id + "\" \"" + label + "\" = \"" + value + "\"" +
           (mBinding ? "" : "   <-- NO BINDING (inert)");
}
