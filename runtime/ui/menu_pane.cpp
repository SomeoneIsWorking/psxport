...
MenuPane::MenuPane(Rml::Element* root, RowBuilder& builder) : Component(root) {
    ...
    for (Rml::Element* r : rows) {
        MenuRow& row = adopt<MenuRow>(r, builder.bind_row(r),
                                      [&builder](MenuRow& clicked) { builder.on_row_clicked(clicked); });
        mRows.push_back(&row);
    }
}
...
void MenuPane::set_shown(bool shown) {
    if (!mRoot || mRoot->IsPseudoClassSet("shown") == shown) return;
    mRoot->SetPseudoClass("shown", shown);
}

bool MenuPane::focus_first_row() {
    for (MenuRow* r : mRows)
        if (r->focus()) return true;
    return false;
}
...
MenuRow* MenuPane::row_containing(Rml::Element* element) const {
    ...
    for (MenuRow* r : mRows)
        if (r->contains(element)) return r;
    return nullptr;
}
