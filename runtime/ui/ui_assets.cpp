...
bool AssetSet::open() {
    ...
    if (!mBaseOk) {
        ...
        lucent::error("rmlui",
                      ...
                      mDir, base.empty() ? "<unset, so paths are cwd-relative>" : base,
                      cwd_or_unknown());
    }
    return mBaseOk;
}
...
std::string AssetSet::resolve(const char* rel) const {
    ...
    if (!mOpened) {
        const std::string& base = psx::config::cv_asset_dir.get();
        return absolutize((base.empty() ? std::string(kAssetSubdir) : base + "/" + kAssetSubdir) +
                          "/" + rel);
    }
    return mDir + "/" + rel;
}
...
bool AssetSet::require(const char* rel, std::string& out_path) {
    ...
    if (mBaseOk) lucent::error("rmlui", "asset MISSING: {}", out_path);
    return false;
}
...
bool AssetSet::report() const {
    ...
    lucent::error("rmlui", "assets: {} of {} found in {} — {} MISSING", mFound, mWanted, mDir,
                  mBaseOk ? (int)mMissing.size() : mWanted);
    for (const std::string& m : mMissing) lucent::error("rmlui", "  missing: {}", m);
    return false;
}
