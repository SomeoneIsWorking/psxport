...
MenuTab::MenuTab(Rml::Element* root, std::function<void()> on_click) : Component(root) {
    listen(mRoot, Rml::EventId::Click,
           [cb = std::move(on_click)](Rml::Event&) { if (cb) cb(); });
}
...
MenuTabBar::MenuTabBar(Rml::Element* root, std::function<void(int)> on_select)
    ...
    for (int i = 0; i < (int)tabs.size(); i++) {
        MenuTab& tab = adopt<MenuTab>(tabs[i], [this, i] { select(i); });
        mTabs.push_back(&tab);
    }
}
...
void MenuTabBar::select(int index) {
    ...
    for (int i = 0; i < n; i++) mTabs[i]->set_selected(i == index);
    if (mOnSelect) mOnSelect(index);
}
