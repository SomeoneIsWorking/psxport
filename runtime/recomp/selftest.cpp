...
bool stage_cfg(const char *which, StageCfg *out) {
  ...
  if (!s.entryAddr || !s.game) {
    lucent::error("selftest",
                  ...
                  "entry PCs and fill those fields.",
                  which,
                  s.entryAddr,
                  s.game);
    return false;
...
static int run_startgame(const char *path) {
  ...
  auto stage = [&] {
    return c->mem_r32(sc.entryAddr);
  };
  auto sm48 = [&] {
    return (uint32_t)c->mem_r16(sc.smAddr);
  };
  auto log_change = [&] {
  ...
  if (stage() != sc.game) {
    lucent::info("selftest",
                 "FAIL: never reached GAME stage after {} frames of mashing Start (stuck stage=0x{:08X} sm[0x48]={})",
                 f,
                 stage(),
                 sm48());
    return 1;
  ...
  for (uint32_t g = 0; g < SETTLE_CAP; g++, f++) {
    ...
    if (stage() == sc.game && sm48() == 2) {
      ...
      for (uint32_t w = 0; w < MAX_WINS && !clip_ended; w++) {
        for (uint32_t k = 0; k < WIN; k++, f++) {
          dc_step_frame(c, f);
        }
        if (xa_stream_is_active(&game->xa)) {
          saw_clip = 1;
        } else if (saw_clip) {
          clip_ended = 1;
        }
        if (verbose) {
          lucent::info("selftest",
                       "  window {}: loop raw={} xa_active={}",
                       w,
                       c->mem_r16(0x1F800198u),
                       xa_stream_is_active(&game->xa));
        }
      }
      ...
      if (loop_ran && saw_clip && clip_ended) {
        ...
        if (adv < 50) {
          lucent::error("selftest",
                        "FAIL: field FROZE after the intro clip — GAME loop counter *(0x1F800198) advanced only {} "
                        "over {} frames (stuck={}). The full-PSX field is not progressing (recomp-coro "
                        "callee-saved-register corruption — see docs/findings/sbs.md / FUN_8003c048 jump-table emit).",
                        adv,
                        RUN,
                        c->mem_r16(0x1F800198u));
          return 1;
        ...
        lucent::info("selftest",
                     "PASS: GAME loop RAN, the intro XA clip PLAYED then COMPLETED headless, AND the field KEEPS "
                     "running (counter +{} over {} frames).",
                     adv,
                     RUN);
        return 0;
      }
      lucent::info("selftest",
                   "FAIL: loop_ran={} (counter +{}) saw_clip={} clip_ended={} (xa_active={} sm[0x48]={}).",
                   loop_ran,
                   loop_adv,
                   saw_clip,
                   clip_ended,
  ...
  lucent::info("selftest",
               "FAIL: GAME stage entered at f{} but field never reached free-roam (stuck sm[0x48]={} after {} frames) "
               "— the start-game sequence FROZE.",
               reached_at,
               sm48(),
               SETTLE_CAP);
  return 1;
...
static int run_narration(const char *path) {
  ...
  lucent::info("selftest",
               "reached GAME at frame {} — now playing the intro narration (no input) through the field transition",
               f);
  ...
  if (adv < 50) {
    lucent::info("selftest",
                 "FAIL: GAME loop did not progress through the narration (counter advanced only {} over {} frames; "
                 "sm[0x48]={}). The narration->field transition stalled.",
                 adv,
                 RUN,
                 sm48());
    return 1;
  ...
  lucent::info("selftest",
               "PASS: the un-skipped intro narration played through the field transition WITHOUT a render-queue "
               "overflow, and the GAME loop kept running (counter +{} over {} frames).",
               adv,
               RUN);
  return 0;
}
...
static int run_oracle(const char *path) {
  ...
  auto loopc = [&] {
    return c->mem_r16(0x1F800198u);
  };
  auto scene = [&] {
    return c->mem_r8(0x800bf9b4u);
  };
  auto ovsig = [&] {
    return c->mem_r32(0x80109450u);
  }; // MODE overlay: 0x3C021F80=SOP narration, 0x801138A4=walkable field
  ...
  if (stage() != sc.game) {
    lucent::info("selftest",
                 "FAIL(oracle): interpreter core never reached GAME after {} frames (stuck stage=0x{:08X} sm[0x48]={})",
                 f,
                 stage(),
                 sm48());
    return 1;
  ...
  for (uint32_t k = 0; k < RUN; k++, f++) {
    ...
    if (ovsig() == 0x801138A4u && sc == 0) {
      if (freeroam_since < 0) {
        freeroam_since = (int)k;
        lucent::info("selftest", "  oracle: FREE-ROAM field reached at f={}", f);
      }
      int held = (int)k - freeroam_since;
      if (held == 30 || held == 120 || held == 300 || held == 600 || held == 1200 || held == 1800 || held == 2400 ||
          held == 2550 || held == 2600 || held == 2700) {
        char p[160];
        snprintf(p, sizeof p, "scratch/screenshots/oracle_field_h%d_f%u.ppm", held, f);
        gpu_native_shot(c, p);
    ...
    if (sc && !shot_done[sc]) {
      shot_done[sc] = 1;
      char p[160];
      snprintf(p, sizeof p, "scratch/screenshots/oracle_scene%u_f%u.ppm", sc, f);
      gpu_native_shot(c, p);
    }
    ...
    if (sc && (k % 40) == 0) {
      char p[160];
      snprintf(p, sizeof p, "scratch/screenshots/oracle_s%u_k%u.ppm", sc, k);
      gpu_native_shot(c, p);
    }
    if (verbose && (k % 250) == 0) {
      lucent::info("selftest",
                   "  oracle f={} loop={} sm[0x48]={} scene(bf9b4)={} ovsig=0x{:08X}",
                   f,
                   loopc(),
                   sm48(),
                   sc,
  ...
  lucent::info("selftest",
               "  oracle END-OF-CUTSCENE: f={} scene={} ovsig=0x{:08X} (0x801138A4=free-roam field)",
               f,
               scene(),
               ovsig());
  ...
  if (adv < 100) {
    lucent::info("selftest",
                 "FAIL(oracle): GAME loop did not advance under interpretation (counter +{} over {} frames; "
                 "sm[0x48]={}, max scene={}) — the interpreter oracle froze.",
                 adv,
                 RUN,
                 sm48(),
                 max_scene);
  ...
  lucent::info("selftest",
               "PASS(oracle): interpreter core RAN the cutscene without freeze/MISS (GAME loop +{} over {} frames; "
               "furthest SOP scene id={}).",
               adv,
               RUN,
               max_scene);
  return 0;
...
// bugs — e.g. scene state the native side fails to set up (the void "effect" we are missing).
// PSXPORT_SELFTEST=oraclediff. The render/timing regions that differ BY DESIGN. This was a VERBATIM copy of
// dualcore.cpp's Tomba!2 literals; both now derive from GameConfig (render_noise.h). The amplification that makes it
...
static uint32_t od_advance_to_ovsig(Core *c, uint32_t &f, uint32_t target, uint32_t cap, bool *reached) {
  void dc_step_frame(Core *, uint32_t);
  uint32_t n = 0;
  while (n < cap && c->mem_r32(0x80109450u) != target) {
    dc_step_frame(c, f);
    f++;
    n++;
  }
...
static uint32_t od_advance_to_scene(Core *c, uint32_t &f, uint8_t target, uint32_t cap, bool *reached) {
  void dc_step_frame(Core *, uint32_t);
  uint32_t n = 0;
  while (n < cap && c->mem_r8(0x800bf9b4u) != target) {
    dc_step_frame(c, f);
    f++;
    n++;
  }
...
static int run_oraclediff(const char *path) {
  ...
  void gpu_native_shot(Core *, const char *);
  load_exe(path, &A->core);
  dc_boot_init(&A->core);
  load_exe(path, &B->core);
  dc_boot_init(&B->core);
  auto stageA = [&] {
    return A->core.mem_r32(sc.entryAddr);
  };
  auto stageB = [&] {
    return B->core.mem_r32(sc.entryAddr);
  };

  // Drive both to the GAME stage (mash Start), each on its own frame clock.
  uint32_t fa = 0, fb = 0;
  for (; fa < 4000 && stageA() != sc.game; fa++) {
    bool on = (fa % 16u) < 8u;
    A->pad.driveHold(on ? ((fa & 16u) ? PAD_CROSS : PAD_START) : PAD_NONE);
    dc_step_frame(&A->core, fa);
  }
  for (; fb < 4000 && stageB() != sc.game; fb++) {
    bool on = (fb % 16u) < 8u;
    B->pad.driveHold(on ? ((fb & 16u) ? PAD_CROSS : PAD_START) : PAD_NONE);
    dc_step_frame(&B->core, fb);
  }
  if (stageA() != sc.game || stageB() != sc.game) {
    lucent::info(
        "oraclediff", "FAIL: reach GAME — A stage=0x{:08X} (f{}), B stage=0x{:08X} (f{})", stageA(), fa, stageB(), fb);
    return 1;
  }
  A->pad.driveHold(PAD_NONE);
  B->pad.driveHold(PAD_NONE);
  lucent::info("oraclediff", "both at GAME (A f{}, B f{}). Checkpoint-diffing the narration beats.", fa, fb);

  int total_div = 0,
      summary_div = 0; // summary_div = narration+free-roam only (excludes the render-noisy interactive walk)
  ...
  auto diff_band = [&](const char *label) -> int {
    ...
    if (label) {
      lucent::info("oraclediff", "=== {} (native f{}, oracle f{}) ===", label, fa, fb);
    }
    for (uint32_t i = 0; i < HI - LO;) {
      if (a[i] == b[i] || od_is_render(LO + i) || od_is_cd_cache(LO + i)) {
        i++;
        continue;
      }
      uint32_t start = i, bytes = 0;
      while (i < HI - LO && a[i] != b[i] && !od_is_render(LO + i) && !od_is_cd_cache(LO + i)) {
        i++;
        bytes++;
      }
      total_bytes += bytes;
      ...
      if (ranges++ < 48 && label) {
        lucent::info("oraclediff",
                     "  diff @0x{:08X}..0x{:08X} ({} B)  nat[0]={:02X} ora[0]={:02X}",
                     LO + start,
                     LO + start + bytes,
                     bytes,
                     a[start],
    ...
    if (label) {
      lucent::info("oraclediff",
                   "  {}: {} diverging ranges, {} bytes total{} (CD-cache excluded)",
                   label,
                   ranges,
                   total_bytes,
                   ranges > 48 ? " (first 48 shown)" : "");
      for (uint32_t p = 0; p < 0x60; p++) {
        if (pagehist[p]) {
          lucent::info("oraclediff", "    page 0x{:08X}: {} diverging bytes", 0x800B0000u + (p << 12), pagehist[p]);
        }
      }
    }
    return ranges;
  ...
  for (uint8_t ck : CKPTS) {
    ...
    if (!ra || !rb) {
      lucent::info("oraclediff",
                   "CHECKPOINT scene={} UNREACHED: native={}(f{}) oracle={}(f{}) — one core never reaches this beat (a "
    ...
    char lbl[32];
    snprintf(lbl, sizeof lbl, "scene %u", ck);
    diff_band(lbl);
    (void)verbose;
  }
  ...
  // point to diff native gameplay against the PSX oracle beyond the scripted narration.
  {
    ...
    if (!ra || !rb) {
      ...
      lucent::info("oraclediff",
                   "ONSET area id: A(native) bf870={} bf9b4={} sm48={} | B(oracle) bf870={} bf9b4={} sm48={}",
                   A->core.mem_r8(0x800bf870u),
                   A->core.mem_r8(0x800bf9b4u),
                   A->core.mem_r16(0x801fe048u),
                   B->core.mem_r8(0x800bf870u),
                   B->core.mem_r8(0x800bf9b4u),
                   B->core.mem_r16(0x801fe048u));
      ...
      // 0x800F24xx) DIFFERENTLY — so hundreds of "divergences" here are expected and are NOT gameplay bugs.
      {
        ...
        lucent::info("oraclediff",
                     "=== interactive-play scan: hold RIGHT {} frames from onset (baseline={} benign ranges) ===",
                     WALK_FRAMES,
                     onset_ranges);
        for (int k = 0; k < WALK_FRAMES; k++) {
          A->pad.driveHold(PAD_RIGHT);
          dc_step_frame(&A->core, fa);
          fa++;
          B->pad.driveHold(PAD_RIGHT);
          dc_step_frame(&B->core, fb);
          fb++;
        }
        ...
        lucent::info(
            "oraclediff",
            "  post-walk RAM: {} ranges (render scene-graph noise). NOTE: Tomba is NOT controllable at this checkpoint "
            "(scripted caught pose) — this re-confirms still-convergence, it is NOT yet an interactive-MOVEMENT test.",
            nd);
      }
      A->pad.driveHold(PAD_NONE);
      B->pad.driveHold(PAD_NONE);
      ...
      lucent::info("oraclediff",
                   "post-walk framebuffers: oraclediff_freeroam_{{native,oracle}}.ppm (native VK vs PSX soft-GPU "
                   "content match — still-convergent; Tomba stays in the scripted caught pose, so this is not yet an "
      ...
      // gross progression check, not a frame-exact diff.
      {
        ...
        for (int k = 0; k < PROG_FRAMES; k++) {
          ...
          if (mi < (int)(sizeof marks / sizeof marks[0]) && k == marks[mi]) {
            ...
            gpu_native_shot(&A->core, pa);
            gpu_native_shot(&B->core, pb);
            lucent::info(
                "oraclediff",
                "  progression h{}: native scene={} ovsig=0x{:08X} sm48={} | oracle scene={} ovsig=0x{:08X} sm48={}",
                marks[mi],
                A->core.mem_r8(0x800bf9b4u),
                A->core.mem_r32(0x80109450u),
                A->core.mem_r16(0x801fe048u),
                B->core.mem_r8(0x800bf9b4u),
                B->core.mem_r32(0x80109450u),
                B->core.mem_r16(0x801fe048u));
            mi++;
          }
        }
  ...
  lucent::info("oraclediff",
               "DONE: {} total diverging ranges across narration + free-roam (interactive-play walk excluded — "
               "render-noise-dominated).",
...
static int run_mdecpump(const char *) {
  ...
  // (a) Reference decode — native_fmv's verified interleave into a flat frame buffer.
  {
    ...
    for (;;) {
      ...
      if (got >= OUT_WORDS && in_pos >= IN_WORDS) {
        break;
      }
    }
    ...
    if (distinct < 16) {
      lucent::info("selftest",
                   "FAIL: reference frame degenerate (only {} distinct words) — the "
                   "comparison could not see a placement bug; fix the test stream",
                   distinct);
      return 1;
    }
  }
  ...
  for (int i = 0; i < IN_WORDS; i++) {
    c->mem_w32(SRC + (uint32_t)i * 4, in_words[i]);
  }
  for (int i = 0; i < OUT_WORDS; i++) {
    c->mem_w32(DST + (uint32_t)i * 4, 0xDEADBEEFu);
  }
  ...
  if (!(c->mem_r32(0x1F801088u) & 0x01000000u)) {
    lucent::info("selftest",
                 "FAIL: DMA0 busy bit clear right after start — the transfer completed "
  ...
  if (c->mem_r32(0x1F801098u) & 0x01000000u) {
    lucent::info(
        "selftest", "FAIL: DMA1 busy bit still set — the ping-pong did not complete its {} words", (int)OUT_WORDS);
    return 1;
  }
  if (c->mem_r32(0x1F801088u) & 0x01000000u) {
    lucent::info("selftest",
                 "FAIL: DMA0 busy bit still set after DMA1 completed — pending input never "
                 "finished feeding");
    return 1;
  }
  ...
  for (int i = 0; i < OUT_WORDS; i++) {
    ...
    if (got_w != ref[i] && bad++ == 0) {
      first_bad = (uint32_t)i;
    }
  }
  if (bad) {
    lucent::info("selftest",
                 "FAIL: pump output differs from reference in {} of {} words (first at "
                 "word {}: got {:08X} want {:08X}) — the voffs placement contract is broken",
                 bad,
                 (int)OUT_WORDS,
                 first_bad,
                 c->mem_r32(DST + first_bad * 4),
                 ref[first_bad]);
    return 1;
  }
  lucent::info("selftest",
               "PASS: pending-DMA pump decode == reference ({} words bit-identical; "
               "DMA0 deferred at start, both busy bits cleared on completion)",
               (int)OUT_WORDS);
  return 0;
}
...
static int run_spuirq(const char *) {
  ...
  auto check = [&](bool ok, const char *what) {
    ...
    fails++;
    lucent::error("selftest",
                  "spuirq FAIL: {} (I_STAT=0x{:03X} pending_work=0x{:X})",
                  what,
                  hle.i_stat,
                  (unsigned)c->pending_work);
  };
  ...
  lucent::info("selftest",
               "PASS: spuirq — 5 checks (negative control, raise, PW_IRQ armed, "
...
int selftest_run(const char *path) {
  ...
  if (which && !strcmp(which, "startgame")) {
    return run_startgame(path);
  }
  if (which && !strcmp(which, "narration")) {
    return run_narration(path);
  }
  if (which && !strcmp(which, "oracle")) {
    return run_oracle(path);
  }
  if (which && !strcmp(which, "oraclediff")) {
    return run_oraclediff(path);
  }
  if (which && !strcmp(which, "mdecpump")) {
    return run_mdecpump(path);
  }
  if (which && !strcmp(which, "spuirq")) {
    return run_spuirq(path);
  }
  ...
  lucent::info("selftest",
               "unknown PSXPORT_SELFTEST='{}' (framework: startgame, narration, oracle, oraclediff, "
               "mdecpump, spuirq; the game may define more)",
               which ? which : "");
  return 2;
}
