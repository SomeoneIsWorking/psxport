...
struct VramDirtyRect { int x, y, w, h; };
...
class VramDirty {
 public:
  ...
  void add(int x, int y, int w, int h) {
    ++adds_;
    if (cw_ <= 0 || ch_ <= 0) { markAll(); return; }   // no canvas: cannot clip, so cannot narrow
    if (w <= 0 || h <= 0) { ++dropped_; return; }
    int x0 = x, y0 = y, x1 = x + w, y1 = y + h;
    if (x0 < 0) x0 = 0;
    if (y0 < 0) y0 = 0;
    if (x1 > cw_) x1 = cw_;
    if (y1 > ch_) y1 = ch_;
    if (x0 >= x1 || y0 >= y1) { ++dropped_; return; }
    if (all_) return;                                  // already uploading everything
    VramDirtyRect r{ x0, y0, x1 - x0, y1 - y0 };
    for (int i = 0; i < n_; i++) if (contains(r_[i], r)) return;   // already covered
    if (n_ >= CAP) { mergeCheapestPair(); ++merges_; }
    r_[n_++] = r;
  }

  bool all() const { return all_; }
  bool empty() const { return !all_ && n_ == 0; }
  int  count() const { return all_ ? 0 : n_; }
  VramDirtyRect at(int i) const { return r_[i]; }
  // The whole canvas as one rect — what a caller uploads when all() is set.
  VramDirtyRect whole() const { return VramDirtyRect{ 0, 0, cw_, ch_ }; }

  // Called once the composite has been brought up to date with everything recorded so far.
  void clear() { all_ = false; n_ = 0; }
 ...
 private:
  ...
  static long area(const VramDirtyRect& a) { return (long)a.w * (long)a.h; }
...
static inline int vram_upload_regions(const VramDirty& d, VramDirtyRect* out, int cap) {
  if (cap <= 0) return 0;
  if (d.all()) { out[0] = d.whole(); return 1; }
  int n = d.count() < cap ? d.count() : cap;
  for (int i = 0; i < n; i++) out[i] = d.at(i);
  return n;
}
