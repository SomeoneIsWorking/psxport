...
uint32_t divide_unr(uint16_t dividend, uint16_t divisor) {
  ...
  const uint32_t result =
      (uint32_t)(((uint64_t)numerator *
                      reciprocal((uint16_t)(denominator | 0x8000)) +
                  32768) >>
                 16);
...
NativeProjectedVertex
detail::project_gte_mode(const FixedAffine &affine,
                         const ProjectionParams &projection, ModelVertex vertex,
                         unsigned shift, bool limit_mode) {
  ...
  for (unsigned row = 0; row < 3; ++row) {
    ...
    for (unsigned column = 0; column < 3; ++column)
      accumulator =
          wrap44(accumulator + (int64_t)affine.m[row][column] * v[column]);
    out.raw_view_fixed[row] = accumulator;
    out.raw_view[row] = (float)accumulator / 4096.0f;
    out.ir[row] =
        clampi((int32_t)(accumulator >> shift), limit_mode ? 0 : -32768, 32767);
  }
  out.sz = (uint16_t)clampi((int32_t)(out.raw_view_fixed[2] >> 12), 0, 65535);
  const uint32_t ratio = divide_unr(projection.h, out.sz);
  out.sx = (int16_t)clampi(
      (int32_t)(((int64_t)projection.ofx + (int64_t)out.ir[0] * ratio) >> 16),
      -1024, 1023);
  out.sy = (int16_t)clampi(
      (int32_t)(((int64_t)projection.ofy + (int64_t)out.ir[1] * ratio) >> 16),
      -1024, 1023);
