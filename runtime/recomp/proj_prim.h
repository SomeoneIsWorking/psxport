...
public:
  ...
  struct Stats { long set = 0; long hit = 0; long miss = 0; long stale = 0; };
  ...
  Stats stats()     const { return {mSetCt, mHitCt, mMissCt, mStaleCt}; }
  void  statsReset()      { mSetCt = mHitCt = mMissCt = mStaleCt = 0; }
  ...
  Stats totals()    const { return {mSetTot, mHitTot, mMissTot, mStaleTot}; }

private:
  struct Ent { uint32_t addr; float pz; int next; int gen; uint32_t word; };
  Ent  mEntries[kMax];
  int  mHead[kHashSize];
