...
PainterObjectPlan RenderQueue::buildPainterObjectPlan(PainterObjectLimits limits) const {
  ...
  if (limits.max_objects == 0 || limits.max_faces == 0 || limits.max_objects > 256) {
    out.refusal = limits.max_objects > 256 ? PainterObjectRefusal::TooManyObjects : PainterObjectRefusal::TooManyFaces;
    return refuse(out.refusal);
  }
  for (int i = 0; i < n; ++i) {
    const RqItem &it = items[i];
    ++out.items_scanned;
    if (i && (items[i - 1].layer > it.layer || (items[i - 1].layer == it.layer && items[i - 1].seq > it.seq))) {
    ...
    if (it.semi) {
      return refuse(PainterObjectRefusal::SemiTransparent, i);
    }
    if (it.layer != RQ_WORLD) {
      return refuse(PainterObjectRefusal::NonWorld, i);
    }
    if (it.order_mode != RQ_OM_DEPTH) {
      return refuse(PainterObjectRefusal::NonDepth, i);
    }
    if (it.mode < 0 || it.mode > 3) {
      return refuse(PainterObjectRefusal::UnsupportedMaterial, i);
    }
  }
  ...
  for (PainterObjectId id : ids) {
    ...
    for (size_t i : members) {
      ...
      plan.commands.push_back({i,
                               id,
                               it.seq,
                               it.mode == 3 ? PainterMaterial::Untextured : PainterMaterial::Textured,
                               it.shade_gouraud != 0,
                               it.dither != 0});
    }
...
void RenderQueue::flush(Core *core) {
  ...
  if (rqflush_ch) {
    ...
    lucent::debug(
        rqflush_ch, "n={} reemit={} seq={} y=[{}..{}]", n, consumed && n ? 1 : 0, seq, n ? ylo : 0, n ? yhi : 0);
  }
