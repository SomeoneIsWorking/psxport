...
public:
  ...
  bool journalIsArmed() const {
    return mJournalArmed;
  }
  ...
  void skipCheck(uint32_t addr, void (*skipFn)(void *), void *skipCtx, void (*oracleFn)(void *), void *oracleCtx);
...
private:
  ...
  uint32_t mStrictList[32];
  int mStrictN = 0;
  uint8_t *mStrictPreRam = nullptr;
  uint8_t *mStrictNatRam = nullptr; // strictCheckFull only
  ...
  // skip-gate state
  int mSkipMode = -1;
  uint32_t mSkipList[32];
  int mSkipN = 0;
  uint8_t *mSkipSpuA = nullptr;
  uint8_t *mSkipSpuB = nullptr;
  uint8_t *mSkipSpuPre = nullptr;
};
    ...
    if (sv_c->game && sv_c->game->verify.skipArmed(addr)) {                                                            \
      ...
      struct SvS {                                                                                                     \
        static void go(void *p) {                                                                                      \
          (*static_cast<decltype(sv_s) *>(p))();                                                                       \
        }                                                                                                              \
      };                                                                                                               \
      struct SvO {                                                                                                     \
        static void go(void *p) {                                                                                      \
          (*static_cast<decltype(sv_o) *>(p))();                                                                       \
        }                                                                                                              \
      };                                                                                                               \
