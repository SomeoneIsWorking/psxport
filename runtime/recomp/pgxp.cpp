...
int pgxp_lookup(int sx, int sy, float *px, float *py, float *pz) {
  auto *p = Pgxp::current();
  return p && p->lookup(sx, sy, px, py, pz) ? 1 : 0;
}
int pgxp_lookup_view(int sx, int sy, float *vx, float *vy, float *vz) {
  auto *p = Pgxp::current();
  return p && p->lookupView(sx, sy, vx, vy, vz) ? 1 : 0;
}
