...
VerifyHarness::Check &VerifyHarness::check(const char *name) {
  for (int i = 0; i < mNChecks; i++) {
    if (mChecks[i].name == name || strcmp(mChecks[i].name, name) == 0) {
      return mChecks[i];
    }
  }
  if (mNChecks < kMaxChecks) {
    mChecks[mNChecks].name = name;
    return mChecks[mNChecks++];
  }
...
int VerifyHarness::on(const char *chan) {
  Check &k = check(chan);
  if (k.flag < 0) {
    k.flag = lucent::channel_on(chan) ? 1 : 0;
  }
  return k.flag;
}
...
void VerifyHarness::run(uint32_t (*fn)(Core *), uint32_t superAddr, const char *gate, int on) {
  ...
  uint32_t a0 = c->r[4];
  memcpy(r0, c->ram, 0x200000);
  memcpy(spad0, c->scratch, 0x400);
  c->r[2] = fn(c);
  uint32_t v0_n = c->r[2];
  memcpy(rN, c->ram, 0x200000);
  memcpy(spadN, c->scratch, 0x400);
  memcpy(c->ram, r0, 0x200000);
  ...
  if (ro >= 0 || so >= 0 || v0_n != v0_o) {
    if (k.nMismatch++ < 40) {
      lucent::info("verify",
                   "{}: MISMATCH a0={:08x} v0 n={:x} o={:x} ram@{:x} spad@{:x} sp={:x}",
                   gate ? gate : "(null)",
                   a0,
                   v0_n,
                   v0_o,
  ...
  } else if (++k.nMatch % 20 == 0) {
    lucent::info("verify", "{}: {} matches", gate ? gate : "(null)", k.nMatch);
  }
}
...
bool VerifyHarness::strictArmed(uint32_t addr) {
  if (mStrictMode < 0) {
    ...
    if (!e || !*e) {
      mStrictMode = 0;
    } else if (!strcmp(e, "all") || !strcmp(e, "ALL")) {
      mStrictMode = 2;
    } else {
      mStrictMode = 1;
...
void VerifyHarness::journalTrack(uint32_t off, uint8_t origByte) {
  if (!mJournalDirty) {
    mJournalDirty = (uint8_t *)calloc(0x200000 / 8, 1);
  }
  uint32_t byteIdx = off >> 3;
  uint8_t bit = (uint8_t)(1u << (off & 7));
  if (mJournalDirty[byteIdx] & bit) {
    return; // already touched this invocation — orig already recorded
  }
  mJournalDirty[byteIdx] |= bit;
  mJournal.push_back(
      {off, origByte, origByte}); // .nat defaults to .orig (right for a leg-2-only touch — see strictCheck)
}
...
void VerifyHarness::strictCheck(uint32_t addr, void (*fn)(void *), void *ctx) {
  if (mFullMode < 0) {
    mFullMode = cfg_on("PSXPORT_MIRROR_VERIFY_FULL") ? 1 : 0;
  }
  if (mFullMode) {
    strictCheckFull(addr, fn, ctx);
    return;
  }
  Core *c = core;
  uint8_t preSpad[0x400];
  uint32_t preRegs[34];
  inCheck = true;
  memcpy(preSpad, c->scratch, 0x400);
  memcpy(preRegs, c->r, 32 * 4);
  preRegs[32] = c->hi;
  preRegs[33] = c->lo;
  ...
  for (size_t i = 0; i < n1; i++) {
    mJournal[i].nat = c->ram[mJournal[i].addr]; // leg-1-final values
  }
  uint8_t natSpad[0x400];
  memcpy(natSpad, c->scratch, 0x400);
  uint32_t natRegs[kNStrictReg];
  for (int i = 0; i < kNStrictReg; i++) {
    natRegs[i] = c->r[kStrictReg[i]];
  }
  uint32_t natHi = c->hi, natLo = c->lo;

  for (size_t i = 0; i < n1; i++) {
    c->ram[mJournal[i].addr] = mJournal[i].orig; // rewind (touched bytes only)
  }
  memcpy(c->scratch, preSpad, 0x400);
  memcpy(c->r, preRegs, 32 * 4);
  c->hi = preRegs[32];
  c->lo = preRegs[33];
  ...
  auto printHeader = [&]() {
    ...
    lucent::info(
        "mirror-verify",
        "0x{:08X} MISMATCH at invocation #{} entry sp={:08X} ra={:08X} a0={:08X} a1={:08X} a2={:08X} a3={:08X}",
        addr,
  ...
  for (size_t i = 0; i < mJournal.size() && bad < 16; i++) {
    ...
    if (curVal != mJournal[i].nat) {
      printHeader();
      lucent::info("mirror-verify",
                   "0x{:08X} MISMATCH ram 0x{:08X}: native={:02X} substrate={:02X}",
                   addr,
                   0x80000000u + a,
                   mJournal[i].nat,
                   curVal);
  ...
  for (uint32_t i = 0; i < 0x400 && bad < 16; i++) {
    if (c->scratch[i] != natSpad[i]) {
      printHeader();
      lucent::info("mirror-verify",
                   "0x{:08X} MISMATCH spad 0x{:08X}: native={:02X} substrate={:02X}",
                   addr,
                   0x1F800000u + i,
                   natSpad[i],
                   c->scratch[i]);
  ...
  for (int i = 0; i < kNStrictReg; i++) {
    if (c->r[kStrictReg[i]] != natRegs[i]) {
      printHeader();
      lucent::info("mirror-verify",
                   "0x{:08X} MISMATCH reg {}: native={:08X} substrate={:08X}",
                   addr,
                   kStrictName[i],
                   natRegs[i],
                   c->r[kStrictReg[i]]);
  ...
  if (c->hi != natHi || c->lo != natLo) {
    printHeader();
    lucent::info("mirror-verify",
                 "0x{:08X} MISMATCH hi/lo: native={:08X}/{:08X} substrate={:08X}/{:08X}",
                 addr,
                 natHi,
                 natLo,
                 c->hi,
  ...
  if (bad) {
    ...
    k.nMismatch++;
    if (!cont) {
      lucent::error("mirror-verify",
                    "0x{:08X} FAILED ({}+ diffs) — native mirror is NOT byte-exact. Aborting (set "
                    "PSXPORT_MIRROR_VERIFY_CONTINUE=1 to log-and-continue).",
                    addr,
                    bad);
      abort();
    }
    lucent::info("mirror-verify",
                 "0x{:08X} CONTINUING past {}+ diffs (PSXPORT_MIRROR_VERIFY_CONTINUE=1) — execution proceeds from the "
                 "NATIVE result.",
                 addr,
                 bad);
  }
  for (size_t i = 0; i < mJournal.size(); i++) {
    c->ram[mJournal[i].addr] = mJournal[i].nat; // continue-from-native
  }
  memcpy(c->scratch, natSpad, 0x400);
  for (int i = 0; i < kNStrictReg; i++) {
    c->r[kStrictReg[i]] = natRegs[i];
  }
  c->hi = natHi;
  c->lo = natLo;
  inCheck = false;
  if (!bad) {
    Check &k = check("mirror-verify");
    if (++k.nMatch % 64 == 1) {
      lucent::info("mirror-verify", "0x{:08X} OK (pass #{})", addr, k.nMatch);
    }
  }
}
...
void VerifyHarness::strictCheckFull(uint32_t addr, void (*fn)(void *), void *ctx) {
  Core *c = core;
  if (!mStrictPreRam) {
    mStrictPreRam = (uint8_t *)malloc(0x200000);
    mStrictNatRam = (uint8_t *)malloc(0x200000);
  }
  uint8_t preSpad[0x400];
  uint32_t preRegs[34];
  inCheck = true;
  memcpy(mStrictPreRam, c->ram, 0x200000);
  memcpy(preSpad, c->scratch, 0x400);
  memcpy(preRegs, c->r, 32 * 4);
  preRegs[32] = c->hi;
  preRegs[33] = c->lo;
  ...
  memcpy(mStrictNatSpad, c->scratch, 0x400);
  for (int i = 0; i < kNStrictReg; i++) {
    mStrictNatRegs[i] = c->r[kStrictReg[i]];
  }
  mStrictNatRegs[14] = c->hi;
  mStrictNatRegs[15] = c->lo;
  memcpy(c->ram, mStrictPreRam, 0x200000);
  memcpy(c->scratch, preSpad, 0x400);
  memcpy(c->r, preRegs, 32 * 4);
  c->hi = preRegs[32];
  c->lo = preRegs[33];
  ...
  memcpy(c->ram, mStrictNatRam, 0x200000);
  memcpy(c->scratch, mStrictNatSpad, 0x400);
  for (int i = 0; i < kNStrictReg; i++) {
    c->r[kStrictReg[i]] = mStrictNatRegs[i];
  }
  c->hi = mStrictNatRegs[14];
  c->lo = mStrictNatRegs[15];
  inCheck = false;
  if (!bad) {
    Check &k = check("mirror-verify");
    if (++k.nMatch % 64 == 1) {
...
bool VerifyHarness::mirrorSampleGate(uint32_t addr) {
  if (!strictArmed(addr)) {
    return false; // handles mode parsing + inCheck no-nesting guard
  }
  if (mMirrorEvery < 0) {
  ...
  int slot = -1;
  for (int i = 0; i < mMirrorCntN; i++) {
    if (mMirrorCntAddr[i] == k) {
      slot = i;
      break;
    }
  }
  if (slot < 0) {
    if (mMirrorCntN < kMaxMirrorAddrs) {
      slot = mMirrorCntN++;
      mMirrorCntAddr[slot] = k;
      mMirrorCnt[slot] = 0;
    } else {
      slot = kMaxMirrorAddrs - 1; // table full — diagnostics only, degrade to a shared counter
    }
  }
