...
bool RmlRenderInterfaceGpu::Init(SDL_GPUDevice* dev, SDL_GPUTextureFormat swap_fmt) {
  ...
  if (!vs || !fs) { lucent::error("rmlgpu", "shader create failed: {}", SDL_GetError()); return false; }
  ...
  if (!mPipe) { lucent::error("rmlgpu", "pipeline create failed: {}", SDL_GetError()); return false; }
  ...
  if (!mSampler) { lucent::error("rmlgpu", "sampler create failed: {}", SDL_GetError()); return false; }

  const unsigned char white[4] = { 255, 255, 255, 255 };
  mWhiteTex = UploadTexture(white, 1, 1);
  mReady = (mWhiteTex != nullptr);
  if (mReady) lucent::info("rmlgpu", "render interface ready (swap fmt {})", (int)mSwapFmt);
  return mReady;
}
