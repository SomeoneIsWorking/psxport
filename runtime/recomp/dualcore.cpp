...
bool DualCore::navStep(Core *c, Nav &nv, uint32_t f, const char *tag) {
  if ((f % 400u) == 0) {
    lucent::info("dc-nav",
                 ...
                 f,
                 (int)nv.phase,
                 c->mem_r32(mStageEntryAddr),
                 c->mem_r8(CUT_FLAG));
  }
  uint8_t cut = c->mem_r8(CUT_FLAG);
  switch (nv.phase) {
  case REACH_GAME:
    if (c->mem_r32(mStageEntryAddr) == mStageGame) {
      lucent::info("dc", "{} GAME @f{}", tag, f);
      nv.phase = AWAIT_CUT;
    } else if ((f % 12u) == 0) {
  ...
  case AWAIT_CUT:
    if (cut) {
      lucent::info("dc", "{} cutscene up @f{}", tag, f);
      nv.phase = SKIP_CUT;
      nv.idle = 0;
    }
    break;
  case SKIP_CUT:
    if (cut) {
      ...
      lucent::info("dc", "{} gameplay-start @f{}", tag, f);
      nv.phase = DONE;
      return true;
    }
    break;
...
void DualCore::scriptedInput(Core *c, int k) {
  ...
  if ((k % 30) == 10) {
    c->game->pad.driveTap((uint16_t)(BTN_NONE & ~BTN_RIGHT & ~BTN_CROSS), 4);
  }
}
...
int DualCore::runAndRecord(const char *exe,
                           int render_psx,
                           const char *tag,
                           int n,
                           uint32_t lo,
  ...
  for (; f < MAXF && k < n; f++) {
    ...
    dc_step_frame(&g->core, f);
    snaps[k] = (uint8_t *)malloc(rsz);
    memcpy(snaps[k], g->core.ram + lo - 0x80000000u, rsz);
    spads[k] = (uint8_t *)malloc(0x400);
    memcpy(spads[k], g->core.scratch, 0x400);
    k++;
  }
...
bool DualCore::isRenderRegion(uint32_t a) const {
  return mMask.covers(a);
}
...
void DualCore::diffFrameRegion(const char *name, const uint8_t *a, const uint8_t *b, uint32_t n, uint32_t gbase) {
  ...
  while (i < n && shown < 16) {
    if (a[i] == b[i] || (!show_all && isRenderRegion(gbase + i))) {
      i++;
      continue;
    }
    uint32_t s = i, last = i, gap = 0;
    i++;
    while (i < n && gap < GAP) {
      if (a[i] != b[i]) {
        last = i;
    ...
    for (uint32_t k = s; k <= last && k < s + 8; k++) {
      ln.add("{:02X}", a[k]);
    }
    ln.add(" B:");
    for (uint32_t k = s; k <= last && k < s + 8; k++) {
...
void DualCore::run(const char *exe_path) {
  ...
  {
    const char *e = cfg_str("PSXPORT_DC_LO");
    if (e && *e) {
      lo = (uint32_t)strtoul(e, 0, 0);
    }
  }
  {
    const char *e = cfg_str("PSXPORT_DC_HI");
    if (e && *e) {
      hi = (uint32_t)strtoul(e, 0, 0);
    }
  }
  uint32_t rsz = hi - lo;
  lucent::info("dualcore",
               "NATIVE-render vs PSX-render RAM compare: N={} region 0x{:08X}..0x{:08X} ({}KB/frame)",
               n,
               lo,
               hi,
               rsz / 1024);

  uint8_t **snA = (uint8_t **)calloc(n, sizeof(void *));
  uint8_t **spA = (uint8_t **)calloc(n, sizeof(void *));
  uint8_t **snB = (uint8_t **)calloc(n, sizeof(void *));
  uint8_t **spB = (uint8_t **)calloc(n, sizeof(void *));
  ...
  {
    char b[256];
    lucent::info("dc", "  (excluded as legit render-path difference: {})", mMask.describe(b, sizeof b));
  }
  int first = -1;
  for (int k = 0; k < kn; k++) {
    bool ram_d = false;
    for (uint32_t i = 0; i < rsz; i++) {
      if (snA[k][i] != snB[k][i] && !isRenderRegion(lo + i)) {
        ram_d = true;
        break;
      }
    }
    ...
    if (ram_d || spd_d) {
      if (first < 0) {
        first = k;
        lucent::info("dc", "FIRST DIVERGENCE at gameplay-frame {}:", k);
      }
      if (k < first + 6) { // detail the first few divergent frames
        lucent::info("dc", "  frame {}:", k);
        if (ram_d) {
          diffFrameRegion("ram ", snA[k], snB[k], rsz, lo);
        }
        if (spd_d) {
          diffFrameRegion("spad", spA[k], spB[k], 0x400, 0x1F800000u);
        }
      }
    }
  }
  if (first < 0) {
    lucent::info("dc", "NO DIVERGENCE across {} frames — native gameplay == PSX gameplay here.", kn);
  } else {
    lucent::info("dc", "(divergence began at frame {})", first);
  }
  lucent::info("dc", "========================================================================");
}
