...
void xa_stream_setmode(XaState* xs, uint8_t mode) {
  ...
  if (s_active && !(mode & 0x40)) { s_active = 0; lucent::debug("xa", "stop (XA bit cleared, mode={:02X})", mode); }
}
...
static int xa_decode_next_sector(XaState* xs) {
  ...
  if (s_end_lba && s_lba > s_end_lba) {
    if (s_loop) { lucent::debug("xa", "LOOP chan={} back to {} (passed end {}, span={} sectors)", s_clip_chan, s_clip_start, s_end_lba, s_end_lba - s_clip_start);
                  s_lba = s_clip_start; s_hist[0][0]=s_hist[0][1]=s_hist[1][0]=s_hist[1][1]=0; }
    else { lucent::debug("xa", "clip done @ LBA {} (end {})", s_lba, s_end_lba); s_active = 0; return 0; }
  }
  for (int guard = 0; guard < 64; guard++) {     // bounded scan past any interleaved data sectors
    ...
    if (pass) {
      ...
      if (eof && !s_end_lba) { lucent::debug("xa", "EOF @ LBA {}", s_lba - 1); s_active = 0; }
      return n;
    }
    ...
    if (eof && !s_end_lba) { lucent::debug("xa", "EOF (non-audio) @ LBA {}", s_lba - 1); s_active = 0; return 0; }
  }
