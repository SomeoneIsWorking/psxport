...
// listener and piece of UI state lives there, as components on `psx::ui::Component`
// (`runtime/ui/`, modelled on Dusklight's `src/dusk/ui/`, CC0).
//
...
void RmlOverlay::init(SDL_Window* win, SDL_GPUDevice* dev, SDL_GPUTextureFormat target_fmt,
    ...
    for (const char* f : kFonts) {
        std::string path;
        if (!assets.require(f, path)) continue;
        ...
        if (Rml::LoadFontFace(path.c_str())) fonts_loaded++;
        else lucent::error("rmlui", "font present but REJECTED by the font engine: {}", path);
    }
    ...
    if (!d) {
        ...
        lucent::error("rmlui", "MENU UNAVAILABLE — the overlay is up but has NOTHING TO SHOW. {} "
                               "(fonts loaded: {}/{}; assets dir {})",
                      have_doc ? "LoadDocument(" + doc_path + ") FAILED to parse"
                               : "menu.rml was not found",
                      fonts_loaded, (int)(sizeof(kFonts) / sizeof(kFonts[0])), assets.dir());
        return;
    }

    mMenu = std::make_unique<psx::ui::MenuDocument>(c, d, game);

    lucent::info("rmlui", "overlay up ({} sink {}x{}, {}/{} fonts, {} tabs / {} rows, ESC to toggle)",
                 win ? "windowed" : "headless", sink_w, sink_h, fonts_loaded,
                 (int)(sizeof(kFonts) / sizeof(kFonts[0])), mMenu->tab_count(), mMenu->row_count());
}
...
void RmlOverlay::event(const SDL_Event* e) {
    ...
    if (e->type == SDL_EVENT_KEY_DOWN && mMenu->handle_key((int)e->key.key)) return;

    SDL_Event ev = *e;
    RmlSDL::InputEventHandler(c, mWin, ev);
}

bool RmlOverlay::visible() const { return mMenu && mMenu->visible(); }

void RmlOverlay::setVisible(bool v) {
    if (!mMenu) return;
    if (v) mMenu->show(); else mMenu->hide();
}

void RmlOverlay::selectTab(int index)   { if (mMenu) mMenu->select_tab(index); }
bool RmlOverlay::sendKey(int sdl_keycode) { return mMenu && mMenu->send_key(sdl_keycode); }
void RmlOverlay::dumpMenu() const        { if (mMenu) mMenu->dump(); }

void RmlOverlay::setWorld(int x, int y, int z, unsigned stage) {
    if (mMenu) mMenu->set_world(x, y, z, stage);
}
...
void RmlOverlay::newFrame() {
    ...
    if (mMenu) mMenu->update();   // no-op while hidden
    c->Update();
}
