...
inline HostTurnClockState host_turn_clock_field_delivered(HostTurnClockState state, int64_t now_ns, int64_t period_ns) {
  ++state.generation;
...
inline HostTurnClockState host_turn_clock_armed(HostTurnClockState state, int64_t now_ns, int64_t period_ns) {
  state.deadline_ns = now_ns + period_ns;
  return state;
}

inline HostTurnWakeAction
host_turn_wake_action(uint64_t captured_generation, const HostTurnClockState &current, int64_t now_ns, bool stop) {
  if (stop) {
    return HostTurnWakeAction::Stop;
  }
  if (captured_generation != current.generation) {
    return HostTurnWakeAction::Restart;
  }
