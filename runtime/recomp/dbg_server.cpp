...
static void dbg_exec(FILE *out, const char *line) {
  ...
  if (s_ctx && s_ctx->game && s_ctx->game->sbs && s_ctx->game->sbs->dbgCmd(out, line)) {
    return; // SBS divergence-debugger commands
  }
  ...
  if (!strcmp(cmd, "help")) {
    ...
    if (!g) {
      ...
      fprintf(out,
              ...
              g->oracle ? "ORACLE" : "an SBS compare",
