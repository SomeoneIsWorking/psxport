...
class ProducerScopeState {
 public:
  ...
  uint32_t    currentAddr() const {
    return (mDepth > 0 && mStack[mDepth - 1].key.kind == ProducerKey::GUEST) ? mStack[mDepth - 1].key.id
                                                                            : 0u;
  }
  ...
  void push(uint32_t addr, const char* name) { pushKey(ProducerKey::guest(addr), name); }
 ...
 private:
  struct Entry { ProducerKey key; const char* name = ""; };
...
class ProducerScope {
 public:
  ...
  explicit ProducerScope(ProducerScopeState* st, PcProducer pc) : mSt(st) {
    if (mSt) mSt->pushKey(ProducerKey::native(pc.iid), pc.name);
  }
