...
void spu_bind(Core* c) { c->game->spu.bind(c); }
...
extern "C" void spu_irq_raise(void* corev, int asserted) {
  ...
  if (hle.i_stat == before) return;         // level unchanged, or a falling edge: nothing new latched
  c->pending_work |= Core::PW_IRQ;          // arm the per-function-entry delivery gate
  lucent::debug("irq", "SPU raised IRQ9 -> I_STAT=0x{:03X} (mask=0x{:03X}, {})", hle.i_stat, hle.i_mask,
