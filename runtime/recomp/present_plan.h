...
inline bool present_plan_same_picture(const PresentPlan& a, const PresentPlan& b) {
  if (a.build != b.build || a.src_ires != b.src_ires) return false;
  if (a.sink_w != b.sink_w || a.sink_h != b.sink_h) return false;
  if (a.viewport.x != b.viewport.x || a.viewport.y != b.viewport.y ||
      a.viewport.w != b.viewport.w || a.viewport.h != b.viewport.h) return false;
  for (int i = 0; i < 4; i++)
    if (a.disp[i] != b.disp[i] || a.fade[i] != b.fade[i] || a.fmt[i] != b.fmt[i]) return false;
  return true;
}
...
inline PresentPlan plan_present(const PresentInputs& in, bool headless) {
  ...
  if (in.sink_w <= 0 || in.sink_h <= 0) return p;   // build = false, everything zeroed

  p.build = true;
  p.to_swapchain = !headless;   // <-- the ONE place the leg is allowed to appear
  p.sink_w = in.sink_w; p.sink_h = in.sink_h;   // resolved ONCE, here, for everything downstream

  const int scale = in.present_ires > 1 ? in.present_ires : 1;
  p.src_ires = in.present_ires > 1 ? in.present_ires : 0;
  p.disp[0] = in.sx * scale;      p.disp[1] = in.sy * scale;
  p.disp[2] = in.disp_w * scale;  p.disp[3] = in.disp_h * scale;
  p.fade[0] = in.fade_mode; p.fade[1] = in.fade_r; p.fade[2] = in.fade_g; p.fade[3] = in.fade_b;
