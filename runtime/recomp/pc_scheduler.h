...
// runtime/recomp/scheduler.cpp for un-ported tasks. The yield/spawn primitives
...
class Game;
class Coro; // runtime/recomp/coro.h — thread-fiber for full-PSX mid-function
            // resume (later-264)
struct Core;
struct RecOverlay; // generated overlay_table.h — descriptor of one recompiled
                   // overlay image
...
public:
  ...
  int game_coop[3] = {};    // slot runs the GAME COOPERATIVE task loop (a GAME state not yet
                            ...
                            // just a frame boundary: RE-ENTER the recompiled loop at its TOP
  ...
  // resume. The FULL-PSX path (PSXPORT_SBS_MODE=gameplay/full core B) runs pure
  // recompiled task bodies that yield mid-function (switch); the substrate
  ...
  // recompiler-only, no interpreter (USER 2026-06-30). Active ONLY when
  // psx_fallback is on; the native path is untouched. cur_is_coro tells switch
  ...
  // route native_sync=0 to the substrate. native_fiber[i] marks the slot's Coro as
  // a native body (vs a recomp substrate body).
  int native_fiber[3] = {};
  ...
  // register values, task-slot writes) so a strict SBS byte-compare holds.
  ...
  // Wire the five primitives into the global override registry
  // (overrides::install) at their guest addresses so substrate callers on core
  // A (via rec_dispatch or a direct shard call) reach the same implementations.
  // Core B (psx_fallback) always runs the passed gen_func_<addr>. PrimGen — the
  // generated substrate bodies + override setter the GAME supplies for the five
  // scheduler primitives (framework names no generated symbol; the game passes
  // gen_func_<addr> + shard_set_override from register_overrides.cpp, where the
  // generated substrate is linked). Core B (psx_fallback) runs the gen body;
  // the shared thunk installs the native handler on core A + direct callers.
  struct PrimGen {
  ...
  // Resident overlay per OVERLAP SLOT (0x80106228 stage / 0x80108F9C mode /
  // 0x8018A000 area), recorded by overlay_note_load() at LOAD time — when the
  // freshly-written image still matches its raw .BIN signature, BEFORE the game
  // mutates its header pointer table at runtime. The router routes by this
  // IDENTITY (robust), falling back to a content-signature scan only when
  // unset. Fixes the overlay-router miss when GAME's header pointer @+0x08 is
  // swapped at runtime so the raw-bytes signature no longer matches
  // (later-264). Per-instance for SBS (each core has its own resident set).
  ...
  // from native_boot.cpp's `rc0(c, 0x800506d0)` site (still substrate there —
  // this method is a faithful native port, not yet wired in). Byte-shape
  // source: generated/shard_5.c:7522 (gen_func_800506D0) — a leaf, no stack
