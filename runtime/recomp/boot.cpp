...
void load_exe(const char* path, Core* c) {   // non-static: the dual-core harness loads two cores
  ...
  if (fread(buf, 1, n, f) != (size_t)n) { lucent::error("boot", "short read on {}", path ? path : "(null)"); exit(1); }
  fclose(f);
  ...
  lucent::info("boot", "loaded {}: entry 0x{:08X} load 0x{:08X} text 0x{:X} sp 0x{:08X}",
               path ? path : "(null)", entry, load, tsize, c->r[29]);
}
