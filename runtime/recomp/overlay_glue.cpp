...
// (imgui_overlay.cpp). Keeps all overlay-specific orchestration out of the renderer — see
...
void overlay_glue_init(SDL_Window* win, VkInstance inst, VkPhysicalDevice phys, uint32_t qfam,
    ...
    imgui_overlay_init(win, inst, phys, qfam, dev, queue, present_rpass, min_image_count, image_count);
}

void overlay_glue_event(const SDL_Event* e) {
    imgui_overlay_event(e);
}
...
void overlay_glue_record(VkCommandBuffer cmd, uint32_t frame_index, VkExtent2D extent) {
    ...
    imgui_overlay_render_vk(cmd, frame_index, full_vp, full_sc);
}
