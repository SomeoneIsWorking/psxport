// PcScheduler — PC-native cooperative-task stanzas + the per-frame slot
// dispatch (replaces the PSX BIOS scheduler FUN_80051e60 for tasks the port
// owns natively). The stanzas run under BOTH native_sync modes: native_sync=true uses
// shortcut branches, native_sync=false uses faithful byte-exact branches. Un-ported
// tasks fall through to the substrate coro-fiber / generic-dispatch stanzas
...
// -------------------------- Byte-shape source: generated recomp bodies
// gen_func_80051F80 / 80051F14 / 80044BD4 / 80052010 / 80051FB4
// (shard_2/3/5/6.c) + Ghidra decomps (scratch/decomp/task_spawn.c,
// ram_f1000_all.c 31833-31990). Every frame descent, spill offset and task-slot
// write below is that RE; the spill VALUES are the live guest registers, so
// nested callee spills land organically.
...
// (written even on the already-closed early-out, per the recomp prologue).
...
// (spawnPrim's s0 etc.) hold live values too. Native execution always completes
// the task synchronously. The generated/oracle implementation retains the
// original multi-frame wait when the explicit substrate path is selected.
...
// Guest-ABI trampolines: substrate callers reach the ported primitives through
// rec_dispatch (overrides::dispatch, backed by the global override registry),
// args in r4..r7 exactly like the recomp bodies read them.
static void eov_yield(Core *c) {
  c->game->pcSched.yieldPrim((uint16_t)c->r[4]);
}
static void eov_spawn(Core *c) {
  c->game->pcSched.spawnPrim(c->r[4], c->r[5]);
}
static void eov_spawnwait(Core *c) {
  c->game->pcSched.spawnAndWait(c->r[4], c->r[5], c->r[6], c->r[7]);
}
static void eov_forceclose(Core *c) {
  c->game->pcSched.forceClose(c->r[4]);
}
static void eov_selfclose(Core *c) {
  c->game->pcSched.selfClose();
}

// The `shard_set_override` argument to `install()` below wires the RECOMPILER's
// OWN g_override[] table (generated/shard_disp.c), in addition to the
// registry's own rec_dispatch entry. FOUND (2026-07-08, dispatch-blind-spot
// verification): all five of these guest addresses are reached from MANY
// still-substrate call sites as a DIRECT C call `func_<addr>(c)` (grep across
// generated/shard_*.c: 15/7/2/7/8 direct call sites for
// yield/spawn/spawnAndWait/forceClose/ selfClose respectively) — a MIPS jal the
// recompiler emits as an intra-module call, which NEVER goes through
// rec_dispatch. A rec_dispatch-only registration does NOT populate
// g_override[]. So every one of those direct-call sites fell through the
// wrapper's `if (g_override[i]) {...}` to `gen_func_<addr>(c)` — running the
// OLD SUBSTRATE body — even though a byte-exact native port existed and was
// registered for rec_dispatch. A live run confirmed the gap empirically:
// PSXPORT_DEBUG=dispatch,recdep over a real autonav session showed exactly ONE
// `[dispatch]` hit total for these five addresses (PcScheduler::spawnPrim,
// called once from the native boot driver at ra=DEAD0000) and ZERO for
// yieldPrim/spawnAndWait/forceClose/selfClose, despite the game visibly
// scheduling tasks every frame — i.e. every REAL in-game yield/close ran the
// substrate body, not this port. See docs/findings/tooling.md
// "rec_dispatch-only registration is BLIND to a direct substrate call" (the gap
// was documented but left unfixed pending a concrete victim; this is that
// victim) and game/object/actor_sm_reward.cpp (the existing precedent this
// copies). The generated substrate bodies (gen_func_<addr>) + override setter
// are supplied by the GAME via PrimGen (register_overrides.cpp), so this
// framework file names no generated symbol — the psxport_smoke link proves it.
void PcScheduler::registerOverrides(const PrimGen &gen) {
  using overrides::install;
  install(0x80051F80u, "PcScheduler::yieldPrim", eov_yield, gen.yield, gen.setter);
  install(0x80051F14u, "PcScheduler::spawnPrim", eov_spawn, gen.spawn, gen.setter);
  install(0x80044BD4u, "PcScheduler::spawnAndWait", eov_spawnwait, gen.spawnwait, gen.setter);
  install(0x80052010u, "PcScheduler::forceClose", eov_forceclose, gen.forceclose, gen.setter);
  install(0x80051FB4u, "PcScheduler::selfClose", eov_selfclose, gen.selfclose, gen.setter);
}

// Entry PCs the native stanzas handle. Under BOTH native_sync modes native handlers
// run — that's the actual test surface. The two modes differ in what the
// handlers DO:
//   native_sync=true  → shortcut branches (collapsed multi-step init, scratch drift
//   OK) native_sync=false → FAITHFUL native branches — byte-exact reproduction of
//   substrate cadence
//                   (Slip #3 case-0 split, task-1 spawn cadence, RNG stamps,
//                   etc.).
// Routing native_sync=false to substrate/fiber = trivial "match" that tests
// nothing. Reverted 2026-07-04 (and again after a second attempt) per user
// The SBS strict leg uses native calls but remains byte-exact to the recompiled path —
// the ported path RUNS, and its byte-exactness is the gate, not the routing.
// The four entries this used to name as literals (Tomba!2's DEMO 0x801062E4,
// SOP area-load 0x80109164, GAME 0x8010637C, STAGE-0 START.BIN 0x8010649C) now
// come from `cfg->schedEntries`, which the GAME declares. See the SchedEntry
// comment in game_iface.h: P1.7c moved the task BODIES to the hooks seam and
// left this mapping behind, so the framework was scheduling one specific game
// specially.
//
...
// DEMO 0x801062E4 — native per-frame dispatcher (both native_sync modes; see the
// note on hasNativeHandlerForEntry). Fresh: stageMain (prologue) then frame()
// dispatches sm[0x48]==0 substate. Resume: frame(). Slip #1 leave-defer delays
// the sm[0x48]==5 leave-to- GAME dispatch by one tick to match the substrate
// coro's FUN_80051F80 yield cost. Entry rewrite (LEAVE-DEMO -> GAME): drop
// demo_native, task_started=0, state=3 stands so the next tick enters the
// generic path with the new entry.
...
PcScheduler::runDemoStanza(Core *c, int i, uint32_t base, uint32_t st, int native_content, const R3000 &loop) {
  if (!game->native_sync) {
    return STANZA_NOT_MINE; // pc_faithful DEMO runs on the stage fiber
  }
  int demo_fresh =
...
// task done. With psx_fallback on the recomp body runs as a normal cooperative
...
PcScheduler::runSopAreaLoadStanza(Core *c, int i, uint32_t base, uint32_t st, int native_content, const R3000 &loop) {
  ...
  if (!game->native_sync) {
    return STANZA_NOT_MINE; // pc_faithful: the task-1 fiber runs
                            // areaLoadFaithful
  }
  task_ctx[i] = loop;
...
PcScheduler::runGameStanza(Core *c, int i, uint32_t base, uint32_t st, int native_content, const R3000 &loop) {
  ...
  if (!game->native_sync) {
    return STANZA_NOT_MINE; // pc_faithful GAME runs on the stage fiber
  }
...
// Both set done_flag and rec_dispatch 0x80051FB4 (task-end) so the caller of
// FUN_80044BD4's wait-loop sees the completion in the same tick. native_sync=true
// never enters this stanza — Engine::startBinStage closes task-1 preemptively
// there.
PcScheduler::StanzaResult
PcScheduler::runTask1PreloadStanza(Core *c, int i, uint32_t base, uint32_t st, int native_content, const R3000 &loop) {
  ...
  if (fresh) {
    ...
    Core *cc = c;
    co = new Coro();
    // The body kind is the game's declaration. The previous if/else chain ended
    // in a bare `else`, which meant any entry reaching here that was not one of
    // the first three silently became SCHED_CORO_AREALOAD_FAITHFUL — a default
    // that only happened to be right because the guard above admitted exactly
    // four literals. With the table there is no fall-through to be wrong about.
...
// pc_faithful STAGE-0 — the whole ov_start arc as a NATIVE task body on a Coro
// fiber (faithful- execution model). Fresh at entry 0x8010649C: fiber start
// with the task's guest sp and the frame-loop registers, exactly like the
// substrate coro stanza. Resume: one co->resume() per runnable frame; the body
...
PcScheduler::runStage0FiberStanza(Core *c, int i, uint32_t base, uint32_t st, int native_content, const R3000 &loop) {
  if (!native_content || game->native_sync) {
    return STANZA_NOT_MINE;
  }
  Coro *&co = coro[i];
  const int fresh = (st == 3 || (st == 2 && !task_started[i]));
...
// loop is purely slot-iteration + stanza dispatch — PC-native stanzas first,
// then the substrate coro-fiber / generic-dispatch stanzas for un-ported tasks.
void PcScheduler::step() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    // run the (now unused) FUN_8001cfc8 recomp coroutine; reflect clip state
    ...
    // GATE: on `gate` toggle to psx_fallback, clear any stale native-dispatcher
    // mode so the generic path re-enters cleanly (the native DEMO/GAME
    // dispatchers are OFF under psx_fallback).
    ...
    if (!game->native_sync && i == 1 // pc_faithful task-1 body
        && runTask1PreloadStanza(c, i, base, st, native_content, loop) == STANZA_HANDLED) {
      continue;
    }
    if (runStage0FiberStanza(c, i, base, st, native_content, loop) == STANZA_HANDLED) {
      continue;
    }
    if (recomp_run_coro_fiber_stanza(c, i, base, st, native_content, loop)) {
      continue;
    }
...
// pc_scheduler.h for the RE summary. 1:1 with generated/shard_5.c:7522: no
// frame descent (leaf, no sp/ra touched), 3-slot sweep over
...
// still-substrate call site is runtime/recomp/native_boot.cpp:129 (`rc0(c,
// 0x800506d0)`); wiring is a frontier-tier follow-up (swap that call for
// `c->game->pcSched.tickSleepCountdown()` once SBS-gated).
