...
static void repl_wav_write(const char* path, const int16_t* pcm, uint32_t frames, int rate) {
  ...
  if (!fp) { cfg_loge("repl", "wav write: cannot open %s", path); return; }
  ...
  cfg_logi("repl", "xadump -> %s (%u frames @ %d Hz, %.2fs)", path, frames, rate, frames / (double)rate);
}
...
long Repl::read(Core* c, uint32_t f) {
  ...
  char line[256];
  cfg_logi("repl", "frame=%u ready", f); fflush(stderr);
  while (fgets(line, sizeof line, stdin)) {
    ...
    // frozen" (the unknown command just reprinted the prompt, advancing nothing).
    ...
    else if (!strcmp(cmd, "unwatch")) { c->mem_set_watch(0, 0); cfg_logi("repl", "unwatch"); }
    else if (!strcmp(cmd, "hits")) cfg_logi("repl", "watch hits=%d", c->mem_watch_hits());
    else if (!strcmp(cmd, "press") && sscanf(line, "%*s %31s", arg) == 1)   { held &= ~repl_btn(arg); c->game->pad.driveHold(held); cfg_logi("repl", "held=%04X", held); }
    else if (!strcmp(cmd, "release") && sscanf(line, "%*s %31s", arg) == 1) { held |= repl_btn(arg);  c->game->pad.driveHold(held); cfg_logi("repl", "held=%04X", held); }
    else if (!strcmp(cmd, "tap") && sscanf(line, "%*s %31s %u", arg, &a) >= 1) { if (!a) a = 4; c->game->pad.driveTap((uint16_t)(0xFFFF & ~repl_btn(arg)), (int)a); cfg_logi("repl", "tap %s %u", arg, a); }
    else if (!strcmp(cmd, "debug")) { char ch[200] = {0}; sscanf(line, "%*s %199[^\n]", ch); void cfg_dbg_set(const char*); cfg_dbg_set(ch); cfg_logi("repl", "debug channels = %s", ch[0] ? ch : "(none)"); }
      ...
      for (int h = 0; h < 3; h++) {
        ...
        cfg_logi("ents", "-- list %d head=%08X --", h, n);
        for (int g = 0; n && g < 400; g++, n = c->mem_r32(n + 0x24)) {
          ...
          if (bn) owned++;
          cfg_logi("ents", " %08X t=%02X ri=%02X model=%04X h=%08X pos=(%6d,%6d,%6d) rf=%u cmds=%u gb0=%08X  %s%s", n, c->mem_r8(n + 0xc), c->mem_r8(n + 0xb), c->mem_r16(n + 0xe) & 0x3fff, hh,
      ...
      if (sscanf(line, "%*s %d %d %d", &x, &y, &z) == 3) { c->hooks->replCamTeleport(c, x, y, z); cfg_logi("repl", "tp camera -> (%d,%d,%d)", x, y, z); }
      else { c->hooks->replCamTeleportOff(c); cfg_logi("repl", "tp off (camera follows player)"); } }
    else if (!strcmp(cmd, "newgame")) { this->navNewgame = 1; cfg_logi("repl", "newgame: pulsing to GAME prologue"); return 100000; }
    else if (!strcmp(cmd, "skip")) { a = 0; sscanf(line, "%*s %u", &a); if (!a) a = 500; this->skipFrames = (long)a; cfg_logi("repl", "skip %u frames", a); return (long)a; }
      ...
      } else cfg_logi("repl", "warp <area_id> [sub]  (area table @0x800be118, ids 0..0x1f; sub = 0x800BF871 sub-state)");
    }
      ...
      if (sscanf(line, "%*s %u %119s", &n, dir) >= 1 && n > 0) {
        ...
        cfg_logi("repl", "preseq armed: next %u presents -> %s/p%%04d.ppm", n, dir);
      } else cfg_logi("repl", "preseq <N> [dir] — dump the next N PRESENTED frames (incl. fps60 interp)");
    }
      ...
      if (sscanf(line, "%*s %199s", path) == 1) {
        ...
        if (gpu_vk_enabled()) { gpu_vk_shot(c, path); cfg_logi("repl", "shot (VK) -> %s", path); }
      ...
      unsigned n = 0;
      if (sscanf(line, "%*s %u", &n) == 1) { c->game->mods.ires = (int)n; cfg_logi("repl", "mods.ires = %d", c->game->mods.ires); }
      else cfg_logi("repl", "setires <0..4> — live ires toggle (0=Auto)");
    }
      ...
      if (sscanf(line, "%*s %199s", path) == 1) {
        FILE* fp = fopen(path, "wb");
        if (fp) { fwrite(c->ram, 1, 0x200000, fp); fclose(fp); cfg_logi("repl", "dumpram -> %s", path); }
        else cfg_loge("repl", "dumpram: cannot open %s", path);
        ...
        if (sp) { fwrite(c->scratch, 1, sizeof c->scratch, sp); fclose(sp); cfg_logi("repl", "dumpram scratchpad -> %s", spath); }
      }
    }
    ...
    // runs in their place. e.g. `native music off` drops the native field-BGM engine.
      ...
      if (k <= 0 || !strcmp(nm, "list")) c->game->native_gates.list();
             ...
             cfg_logi("repl", "native %s = %s", nm, strcmp(st, "off") ? "on" : "off"); }
    }
    ...
    // that relied on the bare form into a pc-vs-pc compare (kanban #26 was manufactured that way).
      ...
      else cfg_logi("repl", "prof: start | stop | dump <path>");
    }
      ...
      if (sscanf(line, "%*s %199s", path) == 1) { interp_trace_open(c, path); cfg_logi("repl", "trace -> %s", path); }
      else { interp_trace_open(c, 0); cfg_logi("repl", "trace closed"); }
    }
    ...
    // Pipe stderr through tools/symres.py to resolve the raw hex fn/node addresses to FUN_/native names.
      ...
      if (!strcmp(sub, "watch")) {
        ...
        if (sscanf(line, "%*s %*s %x %x", &addr, &len) != 2 || len == 0) {
          cfg_logi("otattr", "usage: otattr watch <addr-hex> <len-hex>");
        } else {
          int slot = oa.watchRegister(addr, len);
          if (slot < 0)
            cfg_logi("otattr", "watch REJECTED (slots=%d/%d wordsUsed=%d/%d overflow=%d) — free a slot or shrink the region", oa.watchSlotCount(), OtAttr::WATCH_SLOTS, oa.watchWordsUsed(), OtAttr::WATCH_CAP_WORDS,
                    oa.watchOverflow());
          else {
            ...
            cfg_logi("otattr", "watch[%d] = [0x%08X,0x%08X) (%u words) — slots %d/%d, %d/%d words used", slot, dlo, dhi, (r->hi - r->lo) / 4,
      ...
      if (!strcmp(sub, "who")) {
        ...
        if (got < 1) { cfg_logi("otattr", "usage: otattr who <addr-hex> [len-hex]"); fflush(stderr); continue; }
        if (got == 1) len = 4;
        cfg_logi("otattr", "who 0x%08X..0x%08X (word-granular, coalesced runs):", addr, addr + len);
      ...
      if (!strcmp(sub, "trace")) {
        uint32_t addr = 0;
        if (sscanf(line, "%*s %*s %x", &addr) != 1) { cfg_logi("otattr", "usage: otattr trace <addr-hex>"); fflush(stderr); continue; }
        OtAttr::WordRec rec{}; uint32_t wa = 0;
        if (!oa.watchLookup(addr, &rec, &wa)) {
          cfg_logi("otattr", "trace 0x%08X: NOT in any watched region — `otattr watch <addr> <len>` first", addr);
          fflush(stderr); continue;
        }
        if (rec.frame == 0xFFFFFFFFu) {
          cfg_logi("otattr", "trace 0x%08X: watched, but NEVER WRITTEN since registration", addr);
          fflush(stderr); continue;
        }
        cfg_logi("otattr", "trace 0x%08X (word 0x%08X): last writer fn=0x%08X caller=0x%08X frame=%u", addr, wa, rec.fn, rec.caller, rec.frame);
      ...
      cfg_logi("otattr", "frame=%u spans=%d(overflow=%d) gteBuckets=%d(overflow=%d) watch: slots=%d/%d words=%d/%d(overflow=%d)", oa.frame(), oa.spanCount(), oa.spanOverflow(), oa.gteCount(), oa.gteOverflow(),
      ...
      if (!madr) {
        cfg_logi("otattr", "GpuState::s_ot_madr == 0 — no OT walked yet this run");
      } else {
