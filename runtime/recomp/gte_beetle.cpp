...
static void ws_sx_record(void) {
  ...
  if (d.sxhist_on < 0) d.sxhist_on = lucent::channel_on("sxhist") ? 1 : 0;
...
void ws_sx_dump(const char* tag) {
  ...
  lucent::info("ws_sxhist", "{} n={}  below0={}({:.1f}%)  atOrAbove320={}({:.1f}%)", tag ? tag : "(null)", d.sx_n, d.sx_oob_lo, 100.0*d.sx_oob_lo/d.sx_n, d.sx_oob_hi, 100.0*d.sx_oob_hi/d.sx_n);
  for (int b = 0; b < 16; b++)
...
void gte_probe_dump(const char* tag) {
  ...
  lucent::info("gteprobe", "  LightMatrix(LLM cr8-12): {:08X} {:08X} {:08X} {:08X} {:08X}", GTE_ReadCR(8),GTE_ReadCR(9),GTE_ReadCR(10),GTE_ReadCR(11),GTE_ReadCR(12));
  lucent::info("gteprobe", "  BackColor(RBK/GBK/BBK cr13-15): {} {} {}", (int32_t)GTE_ReadCR(13),(int32_t)GTE_ReadCR(14),(int32_t)GTE_ReadCR(15));
  lucent::info("gteprobe", "  LightColorMtx(LCM cr16-20): {:08X} {:08X} {:08X} {:08X} {:08X}", GTE_ReadCR(16),GTE_ReadCR(17),GTE_ReadCR(18),GTE_ReadCR(19),GTE_ReadCR(20));
  lucent::info("gteprobe", "  FarColor(RFC/GFC/BFC cr21-23): {} {} {}   [fog target]", (int32_t)GTE_ReadCR(21),(int32_t)GTE_ReadCR(22),(int32_t)GTE_ReadCR(23));
  lucent::info("gteprobe", "  DepthCue DQA(cr27)={} DQB(cr28)={}   [IR0 = DQB + DQA*h/sz -> fog factor]", (int16_t)GTE_ReadCR(27),(int32_t)GTE_ReadCR(28));
}
...
void proj_probe_dump(const char* tag) {
  ...
  lucent::info("projprobe", "{} verts={}  IR diff={}(max{}) SZ diff={}(max{}) SX diff={}(max{}) SY diff={}(max{})  subpix<=1px={}({:.2f}%)", tag ? tag : "(null)", g.pp_verts, g.pp_bad_ir, g.pp_maxd_ir, g.pp_bad_sz, g.pp_maxd_sz,
...
static void rtpcaller_record(uint32_t ra) {
  GteDebug& d = GTE_CurState()->dbg;
  if (d.rtpcaller_on < 0) d.rtpcaller_on = lucent::channel_on("rtpcaller") ? 1 : 0;
...
void rtpcaller_dump(Core* c, const char* tag) {
  ...
  lucent::info("rtpcaller", "{} RA histogram (caller return site -> jal target = projection fn):", tag ? tag : "(null)");
  for (int i = 0; i < 64 && d.rtpcaller[i].n; i++) {
    ...
    lucent::info("rtpcaller", "    RA=0x{:08X}  {:8}   jal[{:08X}]->fn=0x{:08X}", d.rtpcaller[i].ra, d.rtpcaller[i].n, jal, tgt);
  }
}
...
void gte_op(Core* c, uint32_t insn) { gte_op_impl(c, insn, c ? c->pc : 0); }
void gte_op_at(Core* c, uint32_t insn, uint32_t guest_pc) { gte_op_impl(c, insn, guest_pc); }
