...
void ws_sx_dump(const char* tag) {
  ...
  for (int b = 0; b < 16; b++)
    cfg_logi("ws_sxhist", "  [%5d..%5d) %ld", b*64-256, b*64-256+64, d.sx_hist[b]);
...
void gte_probe_dump(const char* tag) {
  ...
  cfg_logi("gteprobe", "%s executed GTE ops:", tag);
  for (unsigned fn = 0; fn < 64; fn++) if (d.gte_hist[fn])
    cfg_logi("gteprobe", "    %-6s(0x%02X) = %ld", gte_name(fn), fn, d.gte_hist[fn]);
  // lighting/fog control-register snapshot (cop2 CR numbering)
  cfg_logi("gteprobe", "  LightMatrix(LLM cr8-12): %08X %08X %08X %08X %08X", GTE_ReadCR(8),GTE_ReadCR(9),GTE_ReadCR(10),GTE_ReadCR(11),GTE_ReadCR(12));
  cfg_logi("gteprobe", "  BackColor(RBK/GBK/BBK cr13-15): %d %d %d", (int32_t)GTE_ReadCR(13),(int32_t)GTE_ReadCR(14),(int32_t)GTE_ReadCR(15));
  cfg_logi("gteprobe", "  LightColorMtx(LCM cr16-20): %08X %08X %08X %08X %08X", GTE_ReadCR(16),GTE_ReadCR(17),GTE_ReadCR(18),GTE_ReadCR(19),GTE_ReadCR(20));
  cfg_logi("gteprobe", "  FarColor(RFC/GFC/BFC cr21-23): %d %d %d   [fog target]", (int32_t)GTE_ReadCR(21),(int32_t)GTE_ReadCR(22),(int32_t)GTE_ReadCR(23));
  cfg_logi("gteprobe", "  DepthCue DQA(cr27)=%d DQB(cr28)=%d   [IR0 = DQB + DQA*h/sz -> fog factor]", (int16_t)GTE_ReadCR(27),(int32_t)GTE_ReadCR(28));
}
...
void proj_probe_dump(const char* tag) {
  ...
  cfg_logi("projprobe", "%s verts=%ld  IR diff=%ld(max%d) SZ diff=%ld(max%d) SX diff=%ld(max%d) SY diff=%ld(max%d)  subpix<=1px=%ld(%.2f%%)", tag, g.pp_verts, g.pp_bad_ir, g.pp_maxd_ir, g.pp_bad_sz, g.pp_maxd_sz,
...
void rtpcaller_dump(Core* c, const char* tag) {
  ...
  cfg_logi("rtpcaller", "%s RA histogram (caller return site -> jal target = projection fn):", tag);
  for (int i = 0; i < 64 && d.rtpcaller[i].n; i++) {
    ...
    cfg_logi("rtpcaller", "    RA=0x%08X  %8ld   jal[%08X]->fn=0x%08X", d.rtpcaller[i].ra, d.rtpcaller[i].n, jal, tgt);
  }
}
