...
void beh_event_record_machine(Core* c) {
  ...
  leaf1(c, nd, 0x800517f8u);                               // FUN_800517F8
  c->mem_w8(nd + 0x2b, 0);
}
...
void ov_beh_event_record_machine(Core* c) {
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[event_record_machineverify] MISMATCH obj=%08x st=%u n5=%u n6=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), c->mem_r8(obj + 6), ro, so);
