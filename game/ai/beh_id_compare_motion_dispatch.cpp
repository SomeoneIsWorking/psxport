...
void ov_beh_id_compare_motion_dispatch(Core* c) {
  ...
  beh_id_compare_motion_dispatch(c);
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[id_compare_motion_dispatchverify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
...
void beh_id_compare_motion_dispatch_register(void) {
}
