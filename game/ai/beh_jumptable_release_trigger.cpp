// engine/beh_jumptable_release_trigger.cpp — PC-native per-object BEHAVIOR handler FUN_80124E74 (OVERLAY).
//
...
void ov_beh_jumptable_release_trigger(Core* c) {
  ...
  beh_jumptable_release_trigger(c);
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[jumptable_release_triggerverify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
