...
void ov_beh_substate_edge_orchestrator(Core* c) {
  ...
  beh_substate_edge_orchestrator(c);
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[substate_edge_orchestratorverify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
...
void beh_substate_edge_orchestrator_register(void) {
}
