...
// ------------------------------------------------------------------------------------------------
// sm24448verify gate (lazy REPL channel `sm24448verify`) — full main-RAM + scratchpad A/B vs the recomp
// body. Run native -> snapshot the result -> roll back RAM/scratch/regs -> rec_super_call -> diff.
// ------------------------------------------------------------------------------------------------
void ov_sm_24448(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("sm24448verify") ? 1 : 0;
  if (!s_v) { sm_24448(c); return; }
  ...
  sm_24448(c);
  uint32_t v0_native = c->r[R_V0];
  memcpy(ramN, c->ram, 0x200000); memcpy(spadN, c->scratch, 0x400);

  memcpy(c->ram, ram0, 0x200000); memcpy(c->scratch, spad0, 0x400); memcpy(c->r, regs0, sizeof regs0);
  rec_super_call(c, SM_FN);
  uint32_t v0_oracle = c->r[R_V0];

  // exclude the dead stack window [sp-0x800, sp) (this fn's own frame + the dispatched leaves' frames).
  uint32_t sp = regs0[29] & 0x1FFFFFu, flo = (sp >= 0x800) ? sp - 0x800 : 0;
  int ro = -1; for (uint32_t a = 0; a < 0x200000; a++) if (c->ram[a] != ramN[a] && !(a >= flo && a < sp)) { ro = (int)a; break; }
  int so = -1; for (uint32_t a = 0; a < 0x400; a++) if (c->scratch[a] != spadN[a]) { so = (int)a; break; }

  static long ng = 0, nb = 0;
  if (ro >= 0 || so >= 0 || v0_native != v0_oracle) {
    if (nb++ < 40)
      fprintf(stderr, "[sm24448verify] MISMATCH obj=%08x v0 n=%x o=%x ram@%x spad@%x sp=%x\n",
              regs0[R_A0], v0_native, v0_oracle, ro, so, sp);
  } else if (++ng % 50 == 0) {
    fprintf(stderr, "[sm24448verify] %ld matches\n", ng);
  }
}
