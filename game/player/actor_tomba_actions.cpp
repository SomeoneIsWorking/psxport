// actor_tomba.cpp — PORT_GEN draft, byte-faithful transcription of gen_func_8005A970.
// ORACLE: gen_func_8005A970 (generated/shard_4.c:7783-7969)
// PORT_GEN: 0x8005A970 generated/shard_4.c:7783-7969
//
// This body is the gen function's guest-visible operations VERBATIM — every c->r[] op,
// mem_r/mem_w call, func_X/rec_dispatch call with its r31 constant, and label/goto is
// preserved unchanged. Faithful by construction; the only allowed next step is RENAMING
// (locals/labels -> named fields/control-flow), verified equivalent by tools/port_check.py.
// WIRED 2026-07-16 via ActorTomba::registerOverrides (engine_set_override_main 0x8005A970); port_check
// PASS + MIRROR_VERIFY-gated. Verbatim body — the RENAMING pass into named fields is a future step.
#include "actor_tomba.h"
#include "core.h"

void rec_dispatch(Core*, uint32_t);  // overlay_router.cpp — shared dispatch choke point
void func_80054D14(Core*);  // generated/shard_disp.c
void func_80055E28(Core*);  // generated/shard_disp.c
void func_80055FBC(Core*);  // generated/shard_disp.c
void func_80056B48(Core*);  // generated/shard_disp.c
void func_80055D5C(Core*);  // generated/shard_disp.c
void func_8005444C(Core*);  // generated/shard_disp.c
void func_800532A0(Core*);  // generated/shard_disp.c
void func_80055390(Core*);  // generated/shard_disp.c
void func_800558B4(Core*);  // generated/shard_disp.c
void func_80076D68(Core*);  // generated/shard_disp.c
void func_8005A39C(Core*);  // generated/shard_disp.c
void func_80054E80(Core*);  // generated/shard_disp.c
void func_800538E0(Core*);  // generated/shard_disp.c
void func_80049280(Core*);  // generated/shard_disp.c
void func_80074590(Core*);  // generated/shard_disp.c
void func_80055C30(Core*);  // generated/shard_disp.c
void func_800559F4(Core*);  // generated/shard_disp.c
void func_800551C4(Core*);  // generated/shard_disp.c
void func_80056C00(Core*);  // generated/shard_disp.c

void ActorTomba::mode0WalkHandler() {
  Core* c = core;
    c->r[29] = c->r[29] + (uint32_t)-32;
    c->mem_w32((c->r[29] + (uint32_t)16), c->r[16]);
    c->r[16] = c->r[4] + c->r[0];
    c->mem_w32((c->r[29] + (uint32_t)28), c->r[31]);
    c->mem_w32((c->r[29] + (uint32_t)24), c->r[18]);
    c->mem_w32((c->r[29] + (uint32_t)20), c->r[17]);
    c->r[2] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)6));
    { int _t = (c->r[2] == c->r[0]); c->r[18] = c->r[0] + c->r[0]; if (_t) goto L_8005A9B0; }
    { int _t = ((int32_t)c->r[2] < 0); c->r[2] = (uint32_t)((int32_t)c->r[2] < 4); if (_t) goto L_8005AC70; }
    { int _t = (c->r[2] == c->r[0]); c->r[4] = c->r[16] + c->r[0]; if (_t) goto L_8005AC70; }
     goto L_8005A9F4;
  L_8005A9B0:;
    c->r[2] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)357));
    { int _t = (c->r[2] == c->r[0]); c->mem_w16((c->r[16] + (uint32_t)68), (uint16_t)c->r[0]); if (_t) goto L_8005A9CC; }
    c->r[4] = c->r[16] + c->r[0];
    c->r[5] = c->r[0] + (uint32_t)17; goto L_8005A9D4;
  L_8005A9CC:;
    c->r[4] = c->r[16] + c->r[0];
    c->r[5] = c->r[0] + (uint32_t)2;
  L_8005A9D4:;
    c->r[31] = 0x8005A9DCu;
    c->r[6] = c->r[0] + (uint32_t)4; func_80054D14(c);
    c->r[2] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)6));
    c->mem_w16((c->r[16] + (uint32_t)334), (uint16_t)c->r[0]);
    c->mem_w8((c->r[16] + (uint32_t)7), (uint8_t)c->r[0]);
    c->r[2] = c->r[2] + (uint32_t)1;
    c->mem_w8((c->r[16] + (uint32_t)6), (uint8_t)c->r[2]);
    c->r[4] = c->r[16] + c->r[0];
  L_8005A9F4:;
    c->r[31] = 0x8005A9FCu;
    c->r[5] = c->r[0] + (uint32_t)1; func_80055E28(c);
    c->r[5] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)330));
    c->r[31] = 0x8005AA08u;
    c->r[4] = c->r[16] + c->r[0]; func_80055FBC(c);
    c->r[4] = c->r[16] + c->r[0];
    c->r[31] = 0x8005AA14u;
    c->r[5] = c->r[0] + c->r[0]; func_80056B48(c);
    c->r[31] = 0x8005AA1Cu;
    c->r[4] = c->r[16] + c->r[0]; func_80055D5C(c);
    c->r[2] = (uint32_t)c->mem_r16((c->r[16] + (uint32_t)50));
    c->r[4] = c->r[16] + c->r[0];
    c->r[2] = c->r[2] + (uint32_t)8;
    c->r[31] = 0x8005AA30u;
    c->mem_w16((c->r[16] + (uint32_t)50), (uint16_t)c->r[2]); func_8005444C(c);
    c->r[31] = 0x8005AA38u;
    c->r[4] = c->r[16] + c->r[0]; func_800532A0(c);
    { int _t = (c->r[2] != c->r[0]);  if (_t) goto L_8005AB48; }
    c->r[31] = 0x8005AA48u;
    c->r[4] = c->r[16] + c->r[0]; func_80055390(c);
    c->r[18] = c->r[2] + c->r[0];
    { int _t = (c->r[18] != c->r[0]);  if (_t) goto L_8005AB48; }
    c->r[31] = 0x8005AA5Cu;
    c->r[4] = c->r[16] + c->r[0]; func_800558B4(c);
    { int _t = (c->r[2] != c->r[0]);  if (_t) goto L_8005AB48; }
    c->r[2] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)330));
    c->r[2] = c->r[2] & 2u;
    { int _t = (c->r[2] == c->r[0]); c->r[2] = (uint32_t)8064u << 16; if (_t) goto L_8005AB04; }
    c->r[2] = (uint32_t)c->mem_r8((c->r[2] + (uint32_t)311));
    { int _t = (c->r[2] != c->r[0]); c->r[4] = c->r[16] + c->r[0]; if (_t) goto L_8005AAE8; }
    c->r[31] = 0x8005AA90u;
    c->r[4] = c->r[16] + c->r[0]; func_80076D68(c);
    c->r[3] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)357));
    { int _t = (c->r[3] == c->r[0]); c->r[17] = c->r[2] + c->r[0]; if (_t) goto L_8005AAB8; }
    c->r[4] = c->r[16] + c->r[0];
    c->r[5] = c->r[0] + (uint32_t)17;
    c->r[31] = 0x8005AAB0u;
    c->r[6] = c->r[0] + (uint32_t)2; func_80054D14(c);
     goto L_8005AB48;
  L_8005AAB8:;
    c->r[3] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)70));
    c->r[2] = c->r[0] + (uint32_t)17;
    { int _t = (c->r[3] != c->r[2]); c->r[4] = c->r[16] + c->r[0]; if (_t) goto L_8005AAD8; }
    c->r[5] = c->r[0] + (uint32_t)2;
    c->r[31] = 0x8005AAD4u;
    c->r[6] = c->r[5] + c->r[0]; func_80054D14(c);
    c->r[4] = c->r[16] + c->r[0];
  L_8005AAD8:;
    c->r[31] = 0x8005AAE0u;
    c->r[5] = c->r[17] + c->r[0]; func_8005A39C(c);
     goto L_8005AB48;
  L_8005AAE8:;
    c->r[5] = c->r[0] + (uint32_t)2;
    c->r[31] = 0x8005AAF4u;
    c->r[6] = c->r[0] + (uint32_t)4; func_80054D14(c);
    c->r[31] = 0x8005AAFCu;
    c->r[4] = c->r[16] + c->r[0]; func_80076D68(c);
     goto L_8005AB48;
  L_8005AB04:;
    c->r[3] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)70));
    c->r[2] = c->r[0] + (uint32_t)1;
    c->mem_w8((c->r[16] + (uint32_t)5), (uint8_t)c->r[2]);
    c->r[2] = c->r[0] + (uint32_t)2;
    { int _t = (c->r[3] != c->r[2]); c->mem_w8((c->r[16] + (uint32_t)6), (uint8_t)c->r[0]); if (_t) goto L_8005AB28; }
    c->r[4] = c->r[16] + c->r[0];
    c->r[5] = c->r[0] + (uint32_t)4; goto L_8005AB30;
  L_8005AB28:;
    c->r[4] = c->r[16] + c->r[0];
    c->r[5] = c->r[0] + (uint32_t)1;
  L_8005AB30:;
    c->r[31] = 0x8005AB38u;
     func_80054E80(c);
    c->r[4] = c->r[16] + c->r[0];
    c->r[5] = c->r[16] + (uint32_t)44;
    c->r[31] = 0x8005AB48u;
    c->r[6] = c->r[0] + c->r[0]; func_800538E0(c);
  L_8005AB48:;
    c->r[2] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)41));
    c->r[2] = c->r[2] & 128u;
    { int _t = (c->r[2] == c->r[0]);  if (_t) goto L_8005AC70; }
    c->r[2] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)363));
    { int _t = (c->r[2] != c->r[0]); c->r[2] = (uint32_t)8064u << 16; if (_t) goto L_8005AC50; }
    c->r[2] = (uint32_t)c->mem_r8((c->r[2] + (uint32_t)311));
    { int _t = (c->r[2] != c->r[0]); c->r[2] = c->r[0] + (uint32_t)1; if (_t) goto L_8005AC50; }
    c->r[3] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)6));
    { int _t = (c->r[3] != c->r[2]); c->r[2] = c->r[0] + (uint32_t)57; if (_t) goto L_8005AC50; }
    c->r[3] = (uint32_t)(int16_t)c->mem_r16((c->r[16] + (uint32_t)74));
    { int64_t _p = (int64_t)(int32_t)c->r[3] * (int64_t)(int32_t)c->r[2]; c->lo = (uint32_t)_p; c->hi = (uint32_t)((uint64_t)_p >> 32); }
    c->r[4] = c->r[16] + c->r[0];
    c->r[5] = c->r[2] + c->r[0];
    c->r[2] = (uint32_t)c->mem_r16((c->r[16] + (uint32_t)98));
    c->r[7] = c->lo;
    c->r[6] = (uint32_t)((int32_t)c->r[7] >> 8);
    c->r[6] = c->r[6] + c->r[2];
    c->r[6] = c->r[6] << 16;
    c->r[31] = 0x8005ABBCu;
    c->r[6] = (uint32_t)((int32_t)c->r[6] >> 16); func_80049280(c);
    { int _t = (c->r[2] != c->r[0]); c->r[2] = (uint32_t)8064u << 16; if (_t) goto L_8005AC44; }
    c->r[2] = (uint32_t)8064u << 16;
    c->r[3] = (uint32_t)(int16_t)c->mem_r16((c->r[16] + (uint32_t)50));
    c->r[2] = (uint32_t)(int16_t)c->mem_r16((c->r[2] + (uint32_t)420));
    c->r[3] = c->r[3] - c->r[2];
    c->r[3] = (uint32_t)((int32_t)c->r[3] < -500);
    { int _t = (c->r[3] == c->r[0]); c->r[4] = (uint32_t)8064u << 16; if (_t) goto L_8005AC40; }
    c->r[2] = (uint32_t)8064u << 16;
    c->r[3] = (uint32_t)(int16_t)c->mem_r16((c->r[2] + (uint32_t)530));
    c->r[2] = c->mem_r32((c->r[4] + (uint32_t)140));
    { int _t = (c->r[2] == c->r[3]); c->r[2] = (uint32_t)8064u << 16; if (_t) goto L_8005AC44; }
    c->r[2] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)5));
    { int _t = (c->r[2] != c->r[0]); c->r[2] = (uint32_t)8064u << 16; if (_t) goto L_8005AC44; }
    c->r[4] = c->r[0] + (uint32_t)37;
    c->r[5] = c->r[0] + c->r[0];
    c->r[6] = c->r[5] + c->r[0];
    c->r[31] = 0x8005AC20u;
    c->mem_w16((c->r[16] + (uint32_t)64), (uint16_t)c->r[0]); func_80074590(c);
    c->r[4] = c->r[16] + c->r[0];
    c->r[5] = c->r[0] + (uint32_t)112;
    c->r[31] = 0x8005AC30u;
    c->r[6] = c->r[0] + c->r[0]; func_80054D14(c);
    c->r[2] = c->r[0] + (uint32_t)2;
    c->mem_w8((c->r[16] + (uint32_t)6), (uint8_t)c->r[2]);
    c->r[2] = c->r[0] + (uint32_t)7;
    c->mem_w8((c->r[16] + (uint32_t)7), (uint8_t)c->r[2]);
  L_8005AC40:;
    c->r[2] = (uint32_t)8064u << 16;
  L_8005AC44:;
    c->r[3] = (uint32_t)c->mem_r16((c->r[2] + (uint32_t)140));
    c->r[2] = (uint32_t)8064u << 16;
    c->mem_w16((c->r[2] + (uint32_t)530), (uint16_t)c->r[3]);
  L_8005AC50:;
    c->r[31] = 0x8005AC58u;
    c->r[4] = c->r[16] + c->r[0]; func_80055C30(c);
    { int _t = (c->r[2] != c->r[0]);  if (_t) goto L_8005AC70; }
    { int _t = (c->r[18] != c->r[0]);  if (_t) goto L_8005AC70; }
    c->r[31] = 0x8005AC70u;
    c->r[4] = c->r[16] + c->r[0]; func_800559F4(c);
  L_8005AC70:;
    c->r[31] = 0x8005AC78u;
    c->r[4] = c->r[16] + c->r[0]; func_800551C4(c);
    c->r[4] = c->r[16] + c->r[0];
    c->r[31] = 0x8005AC84u;
    c->r[5] = c->r[0] + c->r[0]; func_80056C00(c);
    c->r[2] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)5));
    c->r[2] = (uint32_t)(c->r[2] < (uint32_t)2);
    { int _t = (c->r[2] == c->r[0]);  if (_t) goto L_8005ACB0; }
    c->r[2] = (uint32_t)c->mem_r8((c->r[16] + (uint32_t)41));
    { int _t = (c->r[2] == c->r[0]); c->r[3] = (uint32_t)32780u << 16; if (_t) goto L_8005ACB0; }
    c->r[2] = c->r[0] + (uint32_t)1;
    c->mem_w8((c->r[3] + (uint32_t)-2018), (uint8_t)c->r[2]);
  L_8005ACB0:;
    c->r[31] = c->mem_r32((c->r[29] + (uint32_t)28));
    c->r[18] = c->mem_r32((c->r[29] + (uint32_t)24));
    c->r[17] = c->mem_r32((c->r[29] + (uint32_t)20));
    c->r[16] = c->mem_r32((c->r[29] + (uint32_t)16));
    c->r[29] = c->r[29] + (uint32_t)32; return;
    return;
}

