...
void ov_grid_offset_48360(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("gridoffset") ? 1 : 0;
  if (!s_v) { c->r[2] = grid_offset_48360(c); return; }
  // full main-RAM + scratchpad + v0 A/B gate (no dispatched callees -> exact 0-diff, no stack window).
  static uint8_t* ram0 = (uint8_t*)malloc(0x200000);
  static uint8_t* ramN = (uint8_t*)malloc(0x200000);
  uint8_t spad0[0x400], spadN[0x400];
  uint32_t regs0[32]; memcpy(regs0, c->r, sizeof regs0);
  memcpy(ram0, c->ram, 0x200000); memcpy(spad0, c->scratch, 0x400);
  uint32_t myv0 = grid_offset_48360(c);
  memcpy(ramN, c->ram, 0x200000); memcpy(spadN, c->scratch, 0x400);
  memcpy(c->ram, ram0, 0x200000); memcpy(c->scratch, spad0, 0x400); memcpy(c->r, regs0, sizeof regs0);
  rec_super_call(c, 0x80048360u);
  uint32_t recv0 = c->r[2];
  int ro = -1; for (uint32_t a = 0; a < 0x200000; a++) if (c->ram[a] != ramN[a]) { ro = (int)a; break; }
  int so = -1; for (uint32_t a = 0; a < 0x400;   a++) if (c->scratch[a] != spadN[a]) { so = (int)a; break; }
  static long ng = 0, nb = 0;
  if (ro >= 0 || so >= 0 || myv0 != recv0) {
    if (nb++ < 40) fprintf(stderr, "[gridoffset] MISMATCH ram@%x spad@%x v0 mine=%x oracle=%x\n", ro, so, myv0, recv0);
  } else if (++ng % 2000 == 0) fprintf(stderr, "[gridoffset] %ld matches\n", ng);
}
