...
// substrate function dispatched by ALREADY-OWNED field code, now owned as a native free function on the
// override registry (gen_func_<addr> = oracle leg). Generated by tools/port_gen.py, all verified
// byte-equivalent by tools/port_check.py (PASS). Register-literal FAITHFUL drafts (readability/rename is
// a separate follow-up); a call to a still-substrate func_<X> routes to the native override when X is
// owned (unified registry), else the gen body. Grown wave-by-wave: each batch's leaf callees surface the
// next wave of unowned leaves.
#include "core.h"
