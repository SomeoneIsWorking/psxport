...
static const char *const kAllInputs[] = {"PSXPORT_ENH", "PSXPORT_ORACLE", "PSXPORT_SBS", "PSXPORT_SBS_MODE"};
...
static int pre_migration_enh(const std::string &e, const char *name) {
  ...
  if (kNamelessIsRefused && (!name || !*name)) {
    return 0;
  }
  if (e == "all") {
    return 1;
  ...
  while (start <= e.size()) {
    ...
    if (end > start && e.substr(start, end - start) == name) {
      return 1;
    }
...
static void test_enh_is_a_declared_cvar(void) {
  ...
  fprintf(stderr,
          "  kind=%s persistable=%d external=%d\n",
  ...
  for (const std::string &s : a.declared) {
    declared += (s == "PSXPORT_ENH");
  }
  for (const std::string &s : a.unknown) {
    unknown += (s == "PSXPORT_ENH");
  }
  fprintf(stderr,
          "  audit: %zu PSXPORT_* set -> %zu declared, %zu legacy, %zu UNKNOWN; "
...
static void test_value_layer_reaches_the_gate(void) {
  ...
  fprintf(stderr,
          "  settings-file Value=\"faster-transitions\" accepted=%d -> asked-for=%d other=%d\n",
          (int)ok,
          hit,
          miss);
  CHECK(ok);
...
static void test_runtime_layer_outranks_the_environment(void) {
  ...
  fprintf(stderr,
          "  env=\"from-env\" then REPL=\"from-repl\" accepted=%d -> env-name=%d repl-name=%d\n",
          (int)ok,
          env_now,
          repl_now);
  CHECK(ok);
...
static void test_parse_matches_the_pre_migration_implementation(void) {
  clear_all();
  static const char *const values[] = {nullptr,
                                       "",
                                       "all",
                                       "faster-transitions",
  ...
  for (const char *v : values) {
    ...
    for (const char *n : names) {
      ...
      if (got != want) {
        fprintf(stderr,
                "  MISMATCH PSXPORT_ENH=%-42s cfg_enh(\"%s\") = %d, pre-migration = %d\n",
                v ? (*v ? v : "\"\"") : "<unset>",
                n,
                got,
                want);
...
static void test_a_nameless_enhancement_is_refused_loudly(void) {
  ...
  fprintf(stderr,
          "  PSXPORT_ENH=all -> cfg_enh(\"\")=%d cfg_enh(NULL)=%d cfg_enh(\"faster-transitions\")=%d; "
...
static void test_each_compare_run_input_suppresses_and_a_plain_run_does_not(void) {
  ...
  for (const Case &c : cases) {
    ...
    set_env("PSXPORT_ENH", "all");
    if (c.name) {
      set_env(c.name, c.value);
    }
    const int got = cfg_enh("faster-transitions");
    fprintf(stderr,
            "  PSXPORT_ENH=all %-16s=%-6s -> cfg_enh = %d (want %d)\n",
  ...
  fprintf(stderr,
          "  exercised %d configuration(s) in ONE process: %d suppressing, %d honouring "
          "(the table wants %d / %d)\n",
          checked,
          suppressed,
          honoured,
...
static void test_the_oracle_moves_mid_process_with_no_caches_cleared(void) {
  ...
  std::vector<std::string> lines;
  lucent::set_sink([&lines](lucent::Level, std::string_view l) {
  ...
  for (const std::string &l : lines) {
    fprintf(stderr, "  | %s\n", l.c_str());
    suppressed_notices +=
        (l.find("SUPPRESSED") != std::string::npos && l.find("faster-transitions") != std::string::npos);
  }
  fprintf(stderr,
          "  one process, no caches cleared: plain=%d -> REPL oracle=1 (accepted %d) -> %d -> "
...
static void test_migrating_the_sbs_knobs_must_update_compare_run(void) {
  ...
  fprintf(stderr,
          "  checked %d suppression input(s) for CVar declaration -> %d declared\n",
          (int)(sizeof(kSbs) / sizeof(kSbs[0])),
          declared);
  CHECK_EQ(declared, 0);
  clear_all();
}
...
static void test_suppression_warns_once_per_knob_naming_each(void) {
  ...
  for (const std::string &l : lines) {
    fprintf(stderr, "  | %s\n", l.c_str());
    if (l.find("SUPPRESSED") == std::string::npos) {
      continue;
    }
    ++suppressed_lines;
  ...
  fprintf(stderr,
          "  4 gate calls over 2 knobs -> %zu captured line(s), %d SUPPRESSED, "
...
static psx::config::BoolVar
    cv_test_knob("PSXPORT_TEST_ENH_KNOB", false, "test-only: the game-declared enhancement shape (case 7)");
...
static void test_a_game_declared_bool_cvar_reaches_the_same_hook(void) {
  ...
  lucent::set_sink([&on_lines](lucent::Level, std::string_view l) {
    on_lines.emplace_back(l);
  });
  ...
  for (const std::string &l : on_lines) {
    fprintf(stderr, "  | %s\n", l.c_str());
    active_named +=
        (l.find("ENHANCEMENT ACTIVE") != std::string::npos && l.find("PSXPORT_TEST_ENH_KNOB") != std::string::npos);
  }
  fprintf(stderr,
          "  asked-for, plain run -> enh() = %d,%d; %zu line(s), %d ACTIVE notice(s) naming the knob\n",
          (int)on1,
          (int)on2,
          on_lines.size(),
          active_named);
  ...
  lucent::set_sink([&off_lines](lucent::Level, std::string_view l) {
    off_lines.emplace_back(l);
  });
  ...
  for (const std::string &l : off_lines) {
    fprintf(stderr, "  | %s\n", l.c_str());
    suppressed_named +=
        (l.find("SUPPRESSED") != std::string::npos && l.find("PSXPORT_TEST_ENH_KNOB") != std::string::npos);
  }
  fprintf(stderr,
          "  asked-for, SBS_MODE=panes -> enh() = %d,%d; %zu line(s), %d SUPPRESSED notice(s) "
          "naming the knob\n",
          (int)off1,
          (int)off2,
          off_lines.size(),
...
static void test_compare_run_is_the_single_definition_of_a_byte_compare_run(void) {
  ...
  for (int bits = 0; bits < 8; ++bits) {
    ...
    fprintf(stderr,
            "  oracle=%d sbs=%d sbs_mode=%-6s -> framework=%d hand-written=%d why=\"%s\"\n",
            (int)oracle,
            (int)sbs,
            has_mode ? "panes" : "\"\"",
            (int)by_framework,
  ...
  fprintf(stderr,
          "  compared %d of 8 input combination(s); %d suppressing, %d honouring\n",
          compared,
          suppressing,
          compared - suppressing);
  CHECK_EQ(compared, 8);
