...
static uint32_t coord_word(int x, int y) { return (uint32_t)(x & 0x3FF) | ((uint32_t)(y & 0x1FF) << 16); }
static uint32_t size_word(int w, int h)  { return (uint32_t)(w & 0x3FF) | ((uint32_t)(h & 0x1FF) << 16); }

// A recognisable, position-dependent halfword so a mis-addressed read is visible rather than lucky.
static uint16_t pat(int x, int y) { return (uint16_t)(((x * 7 + y * 131) & 0x7FFF) | 0x8000); }

static void fill_vram(Game* g, int x0, int y0, int w, int h) {
  for (int y = 0; y < h; y++)
    for (int x = 0; x < w; x++) *g->gpu.vram(x0 + x, y0 + y) = pat(x0 + x, y0 + y);
}
...
static void test_save_restore_roundtrip(void) {
  ...
  fill_vram(g, X, Y, W, H);
  for (int i = 0; i < words; i++) g->core.mem_w32(BUF + 4u * i, 0x33333333u);   // allocator poison
  ...
  int poison = 0;
  for (int i = 0; i < words; i++) if (g->core.mem_r32(BUF + 4u * i) == 0x33333333u) poison++;
  ...
  for (int y = 0; y < H; y++)
    for (int x = 0; x < W; x++) { compared++; if (*g->gpu.vram(X + x, Y + y) != pat(X + x, Y + y)) diff++; }
...
static void test_wraparound_matches_other_paths(void) {
  ...
  for (int y = 0; y < H; y++)
    for (int x = 0; x < W; x++) *g->gpu.vram(X + x, Y + y) = pat(X + x, Y + y);
  gp0_c0(g, X, Y, W, H);
...
static void test_zero_size_is_max(void) {
  ...
  int nonzero = 0;
  for (int i = 0; i < 600; i++) if (g->core.mem_r32(0x1F801810u) != 0u) nonzero++;
