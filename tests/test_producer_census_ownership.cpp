...
static bool stub_query(uint32_t addr, uint64_t* nativeHits, uint64_t* oracleHits) {
  if (addr == kOwnedAndRan)   { if (nativeHits) *nativeHits = 42; if (oracleHits) *oracleHits = 7; return true; }
  if (addr == kOwnedNeverRan) { if (nativeHits) *nativeHits = 0;  if (oracleHits) *oracleHits = 3; return true; }
...
static void feed_three_rows(ProducerCensus& cx, ProducerScopeState& st) {
  { ProducerScope s(&st, kOwnedAndRan,   "a"); cx.noteNative(st.currentKey(), 3, 10); }
  { ProducerScope s(&st, kOwnedNeverRan, "b"); cx.noteNative(st.currentKey(), 5, 11); }
  { ProducerScope s(&st, kNotInstalled,  "c"); cx.noteNative(st.currentKey(), 7, 12); }
}
