...
static bool logged(const char *needle) {
  for (const auto &l : g_lines) {
    if (l.second.find(needle) != std::string::npos) {
      return true;
    }
  }
  return false;
...
static bool logged_at(lucent::Level lv, const char *needle) {
  for (const auto &l : g_lines) {
    if (l.first == lv && l.second.find(needle) != std::string::npos) {
      return true;
    }
  }
  return false;
...
static void write_file(const char *path, const char *body) {
  ...
  if (!f) {
    fprintf(stderr,
            "test_producer_provenance: CANNOT WRITE the fixture %s in cwd — this suite is "
...
static void test_loaded_and_earned_are_distinguishable(void) {
  write_file(CLAIMS,
             "0x80001000 2026-01-01T00:00:00 build-OLD\n"
...
static void test_bare_address_line_still_loads_with_no_provenance(void) {
  ...
  CHECK_EQ(c.loadClaims(CLAIMS), 2);
  for (int i = 0; i < c.claimCount(); i++) {
    if (c.claimAt(i) == 0x80001000u) {
      CHECK_STREQ(c.claimProv(i), ""); // REAL third state
    }
    if (c.claimAt(i) == 0x80002000u) {
      CHECK_STREQ(c.claimProv(i), "build-OLD");
    }
  }
}
...
static void test_newest_line_supersedes_older_provenance(void) {
  write_file(CLAIMS,
             "0x80001000 2026-01-01T00:00:00 build-ANCIENT\n"
...
static void test_report_names_the_foreign_build_and_not_the_re_earned(void) {
  write_file(CLAIMS,
             "0x80001000 2026-01-01T00:00:00 build-NEW\n" // will be re-earned below
