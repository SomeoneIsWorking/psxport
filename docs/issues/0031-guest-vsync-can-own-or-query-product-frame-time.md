...
guest had reached a primitive without an owned wait or query contract.

## Resolution

`FrameLoopShell::prepareProduct` now runs after title override registration, requires
`Game::frameDriver`, reinstalls and requires the protected VSync handler, and is the one product preflight used
...
measured address. `PlatformHle::initBuiltins` binds it to one framework-private handler that cannot
be replaced through the HLE table. Nonnegative waits produce typed frame boundaries; a negative
libetc query is permitted only with a title-measured counter address and returns that guest word
without advancing a field. A negative query without a measured counter aborts with the missing
contract named. Missing drivers, missing addresses, refused windows, and conflicting/replacement
registrations abort before guest execution can turn the ownership error into a timeout. The successful
...
`test_vsync_ownership` covers direct and adapter initialization, measured negative-query return,
nonnegative mode boundaries, repeated direct initialization without a title override installed,
replacement refusal, missing-counter/address refusal, and out-of-window refusal. `test_hsync_counter`
now covers root-counter MMIO only.
