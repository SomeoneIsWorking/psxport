...
| Resolved-picture announcement | The one `[wide] native picture:` line stating the aspect, wide-engine answer, native width and render width a Core actually resolved, re-emitted whenever they change | `runtime/psx/picture_announce.*` | The only owner; per-Core state lives on `Core::rsub`. Never copy it into a title — Spyro's copy was deleted when this was extracted | `runtime/psx/picture_announce.h` |
