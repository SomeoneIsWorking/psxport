...
| Lightrec lifetime | One dynarec-default Lightrec state and callback context per `Core` | `runtime/cpu/lightrec_executor.*` | Keep backend lifetime and bounded-fallback telemetry per `Core`; maintained-fork work belongs in the pinned dependency | `docs/migration.md` |
...
| Lightrec cache/code memory | Translated-block cache, chaining, executable-memory allocation/publication, and teardown | Direct pinned dependency at `../../shared/lightrec/` | The maintained Lightrec fork; psxport only owns its adapter | `../../shared/lightrec/docs/runtime-contract.md` |
...
| Configuration | CLI/environment/settings/runtime-diagnostic precedence, validation, and typed immutable values | `runtime/psx/config.cpp`, `runtime/psx/config_var.h`, `runtime/psx/config_vars.h` | The configuration owner; no product CPU-engine setting | `docs/config.md` |
...
| Build and hosted verification | Framework library/dependency/test targets plus the canonical asset-free configure/build/gate commands | `CMakeLists.txt`, `cmake/`, `tools/project.py`, `tools/build.py`, `tools/verify.py`, `.github/workflows/ci.yml` | CMake owns targets; Python owns orchestration; workflows only select the proven host/toolchain | `README.md` |
