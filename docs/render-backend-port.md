...
## Why SDL_GPU
**SDL_GPU maps ~1:1 onto the EXISTING `runtime/recomp/gpu_gpu.cpp`** (the renderer that already WORKS on
...
- **Shaders:** reuse `runtime/recomp/shaders_vk/*.{vert,frag}` as-is. `tools/gen_vk_shaders.sh` already
  compiles them GLSL→SPIR-V→`gpu_gpu_shaders.h` (embedded). SDL_GPU's Vulkan backend consumes SPIR-V
...
- **Renderer design:** `gpu_gpu.cpp` is the behavioral reference — VRAM R16_UINT image, the tri/tritex/semi
  pipelines + CLUT decode, depth bands, present (region+letterbox+fade), headless readback, dirty-rects.

## VK → SDL_GPU object mapping
| Vulkan (gpu_gpu.cpp) | SDL3 GPU |
|---|---|
...
1. **Module + build:** new `runtime/recomp/gpu_gpu.cpp` providing EVERY `gpu_gpu_*`/`gpu_*` entry point from
   `gpu_gpu.h` on SDL_GPU. SDL2→SDL3 in CFLAGS/link. Reuse `gpu_gpu_shaders.h` (SPIR-V). Build green.
...
4. **Delete:** `gpu_gpu.cpp` + SDL2 once Pass 2/3 verified.
...
- [x] **Pass 1 — gpu_gpu.cpp on SDL_GPU + SDL3 migration + build green + 2D screens render.**
  - `runtime/recomp/gpu_gpu.cpp`: device/window/swapchain on SDL_GPU; VRAM R16_UINT sampler texture
    ...
    IMAGE present (SCEA/FMV); headless VRAM readback (`shot`). All `gpu_gpu_*` symbols provided.
  ...
  - RmlUi overlay dropped (`rmlui_overlay_stub.cpp`); SBS two-pane present is now SDL_GPU (`sbs_present_sdl.cpp` + `gpu_gpu_present_sbs2`). Build wires
    `sdl3` (no sdl2/vulkan/GL); `ldd` shows only `libSDL3.so`. Verified WINDOWED on Linux/Vulkan-backend:
    the intro FMV renders correctly (scratch/screenshots/window_grab*.png). gpu_gpu.cpp kept for reference.
...
- [ ] Pass 2 — native 3D raster (draw_tri/tritri/semi) on an R16_UINT color target with IN-SHADER semi
  ...
  bands. This is the bulk of gpu_gpu.cpp's tri/tritex/semi pipelines re-expressed; rewrite tri/tritex
...
- [ ] delete VK (gpu_gpu.cpp) + SDL2 once Pass 2/3 verified.
