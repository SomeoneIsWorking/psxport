...
| `game/ai/` | per-object BEHAVIORS (AI/logic) | 49 `beh_*.cpp` — one per object handler. The native beh registry is the `g_native_beh` table in `game/object/engine_tomba2.cpp`; `ents` flags each live object owned/PSX. |
| `game/object/` | object SYSTEM | `engine_tomba2.cpp` (object-list walk `ov_objwalk` + behavior dispatch table + the field-frame object dispatchers), `animation.cpp`, `actor_sm_24448.cpp`. |
| `game/world/` | entity lifecycle | `entity.cpp`, `pool.cpp`, `spawn.cpp`, `placement.cpp`, `graphics_bind.cpp`, `verify_gate.cpp`. |
| `game/render/` | rendering | `engine_submit`, `engine_render`, `engine_render_walk`, `engine_project`, `cull`, `fps60`, `gpu_lib`, `margin_render`, `render_queue`, `lighting`, `native_terrain`; + `render_native.cpp`, `scene/scene_build.cpp`, `mesh/mesh_draw.cpp`. |
| `game/camera/` | per-frame CAMERA follow | `engine_camera.cpp`. |
| `game/scene/` | stage/scene state machines | `engine_stage` (GAME-stage sm), `engine_demo`, `engine_level`, `engine_init`, `bg_scene_transition_sm`, `sop` (intro cutscene). |
| `game/audio/` | sound + music | `sound.cpp`, `sound_voice.cpp`, `native_audio.c`, `native_music.c`, `music_list.c`. |
| `game/input/` | pad input | `input.cpp`. |
| `game/player/` | player physics/collision | `engine_player`, `collision`, `hitbox`, `grid_offset`. |
| `game/ui/` | HUD / menus / text | `hud`, `menu`, `engine_font`, `engine_ui_rect`, `engine_bav`. |
| `game/items/` | inventory / save / script | `inventory.cpp`, `save.cpp`, `script.cpp`. |
| `game/math/` | math + GTE ops | `engine_math`, `mathlib`, `gte`. |
| `game/core/` | libc + assets | `clib.cpp`, `asset.cpp`, `native_misc.cpp` (residual small leaves + disabled reference reimpls). |
...
- **A native belongs in its SUBSYSTEM FOLDER, named for the system** (`game/camera/engine_camera.cpp`), one
