...
## Un-owned FUN_8007E9C8 fade callers — RE'd (2026-07-03), 3 remain across all overlays
- **symptom (from #27 investigation):** After surveying all 36 shard callsites of `func_8007E9C8` and mapping the enclosing fn per overlay, exactly THREE fade-caller SMs remain still-substrate (all their siblings in game.bin / SOP / A0L / A06 are either native or dead code). Any cutscene that reaches one of these three via a still-substrate PARENT will drop the fade rect and can trigger the #27 stuck-black symptom.
- **status:** RE'd (Ghidra 12.0.4 A06 + A08 projects); parent identification for A06 pair PENDING; A08 parent NAMED. Not yet ported — awaiting user's `PSXPORT_DEBUG=fadetrace` capture on the failing cutscene to prioritise which to port first (see #27).
- **The three:**
  - **A06 FUN_80139728** — 8-state additive-white fade SM. States 0/1/3 ramp UP by 0x40 to threshold 0x80, state 4 ramps DOWN and pokes `DAT_800BFA22=0x2B` (external event), state 5 ramps to 0xFF then invokes `FUN_8006CBD0(0x800E8008, &DAT_8014994C)` (BG init) + sets `DAT_800E806C=7`, state 6 holds 30 frames, state 7 ramps DOWN by 0x10, exits with `return 1`. Every non-exit state ends with `FUN_8007E9C8(gray*0x010101, 1, 4)` — additive fade. Not called directly from any shard fn; installed via a cutscene-script table entry at overlay data `@0x80149D34` (32-bit pointer preceded by opcode `0x0000103E`).
  - **A06 FUN_8013B178** — 3-state additive fade SM. State 0 seeds node+0x40=0. State 1 ramps UP by 0x20 to threshold 0x80, then advances. State 2 ramps DOWN by 0x20, exits at 0x20 with `return 1`. Same script-table entry-point convention at `@0x80149D14`.
  - **A08 FUN_80127C58** — 10-state cutscene director for an A08 object (called from `FUN_801280D0` in A08 shard 0, node behaviour tail). States 0..7 = script sequencing + timed sub-object dispatch via `FUN_8013DD48`. **State 9 = fade-out**: sub-state 0 ramps UP by 8 to 0xFF, sub-state 1 waits for external `DAT_800BFA50 == 0x15`, sub-state 2 ramps DOWN by 8; at 0 sets `DAT_800BFA50=0x16` and node[+4]=3 (despawn). Fade is additive-white gray gradient.
...
- **script-table format (partial RE):** A06 data section at `0x80149CDC` onward is a cutscene-script table interleaving small opcode words (like `0x00011030`, `0x0000103E`) with argument bytes and 32-bit handler pointers. Every `0x0000103E` word is followed by a handler pointer (`0x8013xxxx`) — appears to be a "install per-frame handler" opcode. Full opcode inventory not yet decoded; would take a separate arc to reverse the script interpreter.
- **fix path (per BUG-1 = #27):** port each of these three natively as fade sub-machines calling `c->screenFade.applyLeafCall(...)`. But CANNOT port in isolation — top-down rule: their PARENTS (cutscene-script interpreter for A06 pair, `FUN_801280D0` for A08) must be native first so the sub-machine is reached via a plain C call. Track porting order via #27's fadetrace-log evidence: whichever the user's failing cutscene actually reaches goes first.
- **refs:** #27; Ghidra projects `scratch/ghidra/A06` (base 0x80108F9C, 484 fns) + `scratch/ghidra/A08` (base 0x80108F9C, freshly imported 2026-07-03 with Ghidra 12.0.4); `scratch/decomp/*` Ghidra outputs; `game/ai/beh_a06_multi_actor.cpp` (previous port that covered A06 case 10's whiteFlashPhaseRamp/whiteFadeHold — the two OTHER fade SMs from FUN_801189E8).
