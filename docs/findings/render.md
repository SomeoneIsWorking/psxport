...
## Un-owned FUN_8007E9C8 fade callers — 2 of 3 shipped (2026-07-03), A08 remains
- **symptom (from #27 investigation):** After surveying all 36 shard callsites of `func_8007E9C8` and mapping the enclosing fn per overlay, exactly THREE fade-caller SMs remained still-substrate (all their siblings in game.bin / SOP / A0L / A06 either native or dead code). Any cutscene that reaches an un-owned caller via a still-substrate PARENT drops the fade rect and can trigger the #27 stuck-black symptom.
- **status:** 2 SHIPPED (commit c7c2224), 1 REMAINS (A08). Also solved the sibling arc — the cutscene-script INTERPRETER that dispatches the A06 pair is now fully native (dd40602 + 4c331a3 + a70092a), so op-0x03E fnptr routing is live for any A06 fade fn registered as a `beh_*`.
- **The three, current state:**
  ...
  - **A08 FUN_80127C58** — 🔴 STILL SUBSTRATE. 10-state cutscene director. Called from `FUN_801280D0` state-1 case-0x1A (A08 shard 0, node behaviour tail). States 0..7 = script sequencing + timed sub-object dispatch via `FUN_8013DD48`. State 9 = fade-out (sub 0 ramps UP by 8 to 0xFF; sub 1 waits `DAT_800BFA50 == 0x15`; sub 2 ramps DOWN by 8; at 0 sets `DAT_800BFA50=0x16` and node[+4]=3 despawn). Additive-white gray. RE'd (scratch/decomp/a08_cutscene_director.c) but NOT ported — top-down rule requires the parent `FUN_801280D0` (~400 decomp lines, 21 fnptr refs in A08 → is a real per-object behavior) to be native FIRST so its case-0x1A calls native FUN_80127C58 via plain C. That parent port is the next A08 arc.
...
- **NEXT ARC (A08 tail):** port `FUN_801280D0` as `beh_a08_scene_actor` — 3-state outer (0=init variants 0..0x1B, 1=run variants 0..0x1B, 2=hold, 3=despawn); register at 0x801280D0 in kTable (21 objs use it); replace its state-1 case-0x1A `jal 0x80127C58` with a native `FUN_80127C58` port that lives alongside. Substrate leaves it keeps reachable: FUN_80051B70 (music/SFX), FUN_800778E4 (position), FUN_8004BD64 (position sync), FUN_800778C, FUN_8007A624 (despawn), FUN_8004766C / FUN_80048750 / FUN_800517F8, FUN_8003116C (subobj spawn), FUN_80074590 (SFX trigger), FUN_800708B4, plus the child overlay leaves (FUN_80126ad8, FUN_80126c68, FUN_80126e24, FUN_801273b4, FUN_801277e0, FUN_801279dc, FUN_80127b34, FUN_801268d8) — each a small self-contained fn safe to promote incrementally.
- **refs:** #27; c7c2224 (2 A06 fades shipped); commits dd40602/4c331a3/a70092a (script interpreter + caller chain — the NATIVE PATH that reaches the fade fns); Ghidra projects `scratch/ghidra/A06` (base 0x80108F9C, 484 fns) + `scratch/ghidra/A08` (imported 2026-07-03 with Ghidra 12.0.4); `scratch/decomp/a06_fade_fns.c` + `scratch/decomp/a08_cutscene_director.c`; `game/ai/beh_a06_script_fades.cpp` (the 7 script-driven A06 fade fnptrs) + `game/ai/beh_a06_scripted_actor.cpp` (caller chain); `game/ai/beh_a06_multi_actor.cpp` (previous port covering A06 case 10's whiteFlashPhaseRamp/whiteFadeHold — the two OTHER fade SMs from FUN_801189E8, a DIFFERENT family).
