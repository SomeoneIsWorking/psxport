// engine/objbeh_8013c9c0.cpp — PC-native per-object BEHAVIOR handler FUN_8013C9C0.
//
...
void ov_beh_8013c9c0(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj8013c9c0verify") ? 1 : 0;
  if (!s_v) { beh_8013c9c0(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj8013c9c0verify] MISMATCH obj=%08x st=%u n5=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj8013c9c0verify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_8013c9c0_run(Core* c) { ov_beh_8013c9c0(c); }
