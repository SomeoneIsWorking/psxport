// engine/objbeh_8013259c.cpp — PC-native per-object BEHAVIOR handler FUN_8013259C (OVERLAY).
//
...
// SHAPE as the resident siblings (objbeh_739ac / objbeh_73cd8 / objbeh_8012eb54): a state machine on
...
void ov_beh_8013259c(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj8013259cverify") ? 1 : 0;
  if (!s_v) { beh_8013259c(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj8013259cverify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj8013259cverify] %ld matches\n", ng);
}

}  // namespace

void objbeh_8013259c_register(void) {
}

// Exported entry — the verify wrapper ov_beh_8013259c is in the anonymous namespace above (internal
// linkage); the engine's per-object dispatch calls THIS to run the owned behavior.
void ov_beh_8013259c_run(Core* c) { ov_beh_8013259c(c); }
