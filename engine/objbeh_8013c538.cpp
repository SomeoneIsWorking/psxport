// engine/objbeh_8013c538.cpp — PC-native per-object BEHAVIOR handler FUN_8013C538.
//
...
// siblings (objbeh_8006f2d0 / objbeh_8004ce14 / …): a state machine on the node's state byte node[4].
//
...
void ov_beh_8013c538(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj8013c538verify") ? 1 : 0;
  if (!s_v) { beh_8013c538(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj8013c538verify] MISMATCH obj=%08x st=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj8013c538verify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_8013c538_run(Core* c) { ov_beh_8013c538(c); }
