// engine/objbeh_80125e0c.cpp — PC-native per-object BEHAVIOR handler FUN_80125E0C.
//
...
void beh_80125e0c(Core* c) {
  ...
  if (s0 == 2) goto S2;
  if (s0 == 3) { leaf(c, s1, 0x8007A624u); goto Lret; }   // STATE 3
  goto Lret;                                        // s0 >= 4
...
void ov_beh_80125e0c(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80125e0cverify") ? 1 : 0;
  if (!s_v) { beh_80125e0c(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80125e0cverify] MISMATCH obj=%08x st=%u n5=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80125e0cverify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_80125e0c_run(Core* c) { ov_beh_80125e0c(c); }
