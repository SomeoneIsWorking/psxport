// engine/objbeh_8012eb54.cpp — PC-native per-object BEHAVIOR handler FUN_8012EB54 (OVERLAY).
//
// An OVERLAY per-object behavior routine (guest 0x8012xxxx) with the same state-byte shape as the
// resident siblings (objbeh_739ac / objbeh_73cd8 / objbeh_741dc): a state machine on the node's state
...
//
// Ownership model (identical to objbeh_739ac/73cd8/741dc): CONTROL FLOW + node/global memory writes owned
...
void ov_beh_8012eb54(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj8012eb54verify") ? 1 : 0;
  if (!s_v) { beh_8012eb54(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj8012eb54verify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj8012eb54verify] %ld matches\n", ng);
}

}  // namespace

void objbeh_8012eb54_register(void) {
}

// Exported entry — the verify wrapper ov_beh_8012eb54 is in the anonymous namespace above (internal
// linkage); the engine's per-object dispatch calls THIS to run the owned behavior.
void ov_beh_8012eb54_run(Core* c) { ov_beh_8012eb54(c); }
