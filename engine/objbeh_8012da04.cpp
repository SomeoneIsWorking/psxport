// engine/objbeh_8012da04.cpp — PC-native per-object BEHAVIOR handler FUN_8012DA04.
//
...
void ov_beh_8012da04(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj8012da04verify") ? 1 : 0;
  if (!s_v) { beh_8012da04(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj8012da04verify] MISMATCH obj=%08x st=%u n3=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 3), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj8012da04verify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_8012da04_run(Core* c) { ov_beh_8012da04(c); }
