// engine/objbeh_80136158.cpp — PC-native per-object BEHAVIOR handler FUN_80136158.
//
 ...
 L48c: {                                 // POST-MATH common — should-run gate again
  ...
  L538:
   if (v0 < 256) {                       // slti 256 ; beq -> >=256 goto L558
     ...
     c->r[4] = (uint32_t)a0; rec_dispatch(c, 0x80074af0u);  // FUN_80074AF0(node[0x68])
     c->mem_w32(nd + 0x68, 0);
     goto L598;
   }
   // >=256 (L558):
...
void ov_beh_80136158(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80136158verify") ? 1 : 0;
  if (!s_v) { beh_80136158(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80136158verify] MISMATCH obj=%08x st=%u n5=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80136158verify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_80136158_run(Core* c) { ov_beh_80136158(c); }
