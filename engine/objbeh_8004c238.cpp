// engine/objbeh_8004c238.cpp — PC-native per-object BEHAVIOR handler FUN_8004C238.
//
...
// sibling owned behaviors (objbeh_73cd8 / objbeh_739ac / actor_sm_24448): a state machine on the
...
void ov_beh_8004c238(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj8004c238verify") ? 1 : 0;
  if (!s_v) { beh_8004c238(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj8004c238verify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj8004c238verify] %ld matches\n", ng);
}

}  // namespace

void objbeh_8004c238_register(void) {
}

// Exported entry — the verify wrapper ov_beh_8004c238 is in the anonymous namespace above (internal
// linkage); the engine's per-object dispatch calls THIS to run the owned behavior.
void ov_beh_8004c238_run(Core* c) { ov_beh_8004c238(c); }
