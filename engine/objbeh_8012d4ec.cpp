// engine/objbeh_8012d4ec.cpp — PC-native per-object BEHAVIOR handler FUN_8012D4EC (OVERLAY).
//
...
// resident siblings (objbeh_739ac / objbeh_73cd8 / objbeh_8012eb54): a state machine on the node's state
...
//
// Ownership model (identical to objbeh_739ac / objbeh_73cd8 / objbeh_8012eb54): CONTROL FLOW + node/global
...
void ov_beh_8012d4ec(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj8012d4ecverify") ? 1 : 0;
  if (!s_v) { beh_8012d4ec(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj8012d4ecverify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj8012d4ecverify] %ld matches\n", ng);
}

}  // namespace

void objbeh_8012d4ec_register(void) {
}

// Exported entry — the verify wrapper ov_beh_8012d4ec is in the anonymous namespace above (internal linkage);
// the engine's per-object dispatch calls THIS to run the owned behavior.
void ov_beh_8012d4ec_run(Core* c) { ov_beh_8012d4ec(c); }
