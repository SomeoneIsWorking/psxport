// engine/objbeh_8006f2d0.cpp — PC-native per-object BEHAVIOR handler FUN_8006F2D0.
//
...
// siblings (objbeh_8004ce14 / objbeh_80071a3c / …): a state machine on the node's state byte node[4].
//
...
void ov_beh_8006f2d0(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj8006f2d0verify") ? 1 : 0;
  if (!s_v) { beh_8006f2d0(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj8006f2d0verify] MISMATCH obj=%08x st=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj8006f2d0verify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_8006f2d0_run(Core* c) { ov_beh_8006f2d0(c); }
