// engine/objbeh_8013c3f4.cpp — PC-native per-object BEHAVIOR handler FUN_8013C3F4.
//
...
void ov_beh_8013c3f4(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj8013c3f4verify") ? 1 : 0;
  if (!s_v) { beh_8013c3f4(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj8013c3f4verify] MISMATCH obj=%08x st=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj8013c3f4verify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_8013c3f4_run(Core* c) { ov_beh_8013c3f4(c); }
