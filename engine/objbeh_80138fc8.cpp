// engine/objbeh_80138fc8.cpp — PC-native per-object BEHAVIOR handler FUN_80138FC8 (OVERLAY).
//
...
// resident/overlay siblings (objbeh_739ac / objbeh_73cd8 / objbeh_8012eb54 / objbeh_80124e74): a state
...
void ov_beh_80138fc8(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80138fc8verify") ? 1 : 0;
  if (!s_v) { beh_80138fc8(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80138fc8verify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80138fc8verify] %ld matches\n", ng);
}

}  // namespace

void objbeh_80138fc8_register(void) {
}

// Exported entry — the verify wrapper ov_beh_80138fc8 is in the anonymous namespace above (internal
// linkage); the engine's per-object dispatch calls THIS to run the owned behavior.
void ov_beh_80138fc8_run(Core* c) { ov_beh_80138fc8(c); }
