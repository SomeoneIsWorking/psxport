// engine/objbeh_80117658.cpp — PC-native per-object BEHAVIOR handler FUN_80117658.
//
...
void ov_beh_80117658(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80117658verify") ? 1 : 0;
  if (!s_v) { beh_80117658(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80117658verify] MISMATCH obj=%08x st=%u n3=%u n5=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 3), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80117658verify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_80117658_run(Core* c) { ov_beh_80117658(c); }
