// engine/objbeh_8013a900.cpp — PC-native per-object BEHAVIOR handler FUN_8013A900.
//
...
void beh_8013a900(Core* c) {
 ...
 S1: {
    ...
    if (c->mem_r8(nd + 0x29) == 0) goto L8c00;       // node[0x29]
    {
      ...
      uint32_t vB = leafr2(c, (uint32_t)s0, 110, 0x80085690u);  // FUN_80085690(s0, 110)
...
void ov_beh_8013a900(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj8013a900verify") ? 1 : 0;
  if (!s_v) { beh_8013a900(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj8013a900verify] MISMATCH obj=%08x st=%u n3=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 3), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj8013a900verify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_8013a900_run(Core* c) { ov_beh_8013a900(c); }
