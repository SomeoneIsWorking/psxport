// engine/objbeh_801395c0.cpp — PC-native per-object BEHAVIOR handler FUN_801395C0 (OVERLAY).
//
...
// in a0. Same SHAPE as the resident/overlay siblings (objbeh_739ac / objbeh_73cd8 / objbeh_8012eb54 /
// objbeh_80124e74 / objbeh_80138fc8): a state machine on the node's state byte node[4] (0 init / 1 active /
...
void ov_beh_801395c0(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj801395c0verify") ? 1 : 0;
  if (!s_v) { beh_801395c0(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj801395c0verify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj801395c0verify] %ld matches\n", ng);
}

}  // namespace

void objbeh_801395c0_register(void) {
}

// Exported entry — the verify wrapper ov_beh_801395c0 is in the anonymous namespace above (internal
// linkage); the engine's per-object dispatch calls THIS to run the owned behavior.
void ov_beh_801395c0_run(Core* c) { ov_beh_801395c0(c); }
