// engine/objbeh_741dc.cpp — PC-native per-object BEHAVIOR handler FUN_800741DC.
//
...
//
// Ownership model (same as objbeh_739ac/73cd8): CONTROL FLOW + node/global memory writes owned native;
...
// discipline (see beh_741dc wrapper) — the buffer then sits above the sub-call frames exactly as the recomp
...
void beh_741dc(Core* c) {
  c->r[29] -= 0x30;
  beh_741dc_body(c);
  c->r[29] += 0x30;
}

void ov_beh_741dc(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj741dcverify") ? 1 : 0;
  if (!s_v) { beh_741dc(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj741dcverify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj741dcverify] %ld matches\n", ng);
}
...
// Exported entry — the verify wrapper ov_beh_741dc is in the anonymous namespace above (internal linkage);
// the engine's per-object dispatch (engine_tomba2.cpp call_handler) calls THIS to run the owned behavior.
void ov_beh_741dc_run(Core* c) { ov_beh_741dc(c); }
