// engine/objbeh_80132400.cpp — PC-native per-object BEHAVIOR handler FUN_80132400.
//
...
void ov_beh_80132400(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80132400verify") ? 1 : 0;
  if (!s_v) { beh_80132400(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80132400verify] MISMATCH obj=%08x st=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80132400verify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_80132400_run(Core* c) { ov_beh_80132400(c); }
