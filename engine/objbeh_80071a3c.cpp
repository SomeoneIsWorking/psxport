// engine/objbeh_80071a3c.cpp — PC-native per-object BEHAVIOR handler FUN_80071A3C.
//
// A RESIDENT MAIN.EXE per-object behavior routine (prologue 0x80071A3C; `jr ra` at 0x80071B3C).
// Same SHAPE as the sibling owned behaviors (objbeh_739ac / objbeh_73cd8 / objbeh_8004c238): a state
...
void ov_beh_80071a3c(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80071a3cverify") ? 1 : 0;
  if (!s_v) { beh_80071a3c(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80071a3cverify] MISMATCH obj=%08x st=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80071a3cverify] %ld matches\n", ng);
}

}  // namespace

// Exported entry — the engine's per-object dispatch calls THIS to run the owned behavior.
void ov_beh_80071a3c_run(Core* c) { ov_beh_80071a3c(c); }
