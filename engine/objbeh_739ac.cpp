// engine/objbeh_739ac.cpp — PC-native per-object BEHAVIOR handler FUN_800739AC.
//
  ...
  switch (sub) {
    ...
    case 0:                                          // jt[0]=0x80073b20: confirm -> push sub-state + warp
      if (c->mem_r8(obj + 0x2b) == 3) {
        ...
        c->r[4] = c->mem_r8(0x800BF870u); c->r[5] = v1 & 0xffu;
        rec_dispatch(c, 0x800782F0u);                // area/sub transition
      }
      break;
...
void ov_beh_739ac(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj739acverify") ? 1 : 0;
  if (!s_v) { beh_739ac(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj739acverify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj739acverify] %ld matches\n", ng);
}
