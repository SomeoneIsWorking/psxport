// engine/objbeh_8013b2e4.cpp — PC-native per-object BEHAVIOR handler FUN_8013B2E4.
//
...
void ov_beh_8013b2e4(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj8013b2e4verify") ? 1 : 0;
  if (!s_v) { beh_8013b2e4(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj8013b2e4verify] MISMATCH obj=%08x st=%u n3=%u n6=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 3), c->mem_r8(obj + 6), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj8013b2e4verify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_8013b2e4_run(Core* c) { ov_beh_8013b2e4(c); }
