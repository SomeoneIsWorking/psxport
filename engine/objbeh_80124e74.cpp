// engine/objbeh_80124e74.cpp — PC-native per-object BEHAVIOR handler FUN_80124E74 (OVERLAY).
//
...
// resident siblings (objbeh_739ac / objbeh_73cd8): a state machine on the node's state byte node[4]
...
//
// Ownership model (identical to objbeh_739ac / objbeh_73cd8): CONTROL FLOW + node/global memory writes
...
void ov_beh_80124e74(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80124e74verify") ? 1 : 0;
  if (!s_v) { beh_80124e74(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80124e74verify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80124e74verify] %ld matches\n", ng);
}

}  // namespace

void objbeh_80124e74_register(void) {
}

// Exported entry — the verify wrapper ov_beh_80124e74 is in the anonymous namespace above (internal linkage);
// the engine's per-object dispatch calls THIS to run the owned behavior.
void ov_beh_80124e74_run(Core* c) { ov_beh_80124e74(c); }
