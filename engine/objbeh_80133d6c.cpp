// engine/objbeh_80133d6c.cpp — PC-native per-object BEHAVIOR handler FUN_80133D6C.
//
...
void ov_beh_80133d6c(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80133d6cverify") ? 1 : 0;
  if (!s_v) { beh_80133d6c(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80133d6cverify] MISMATCH obj=%08x st=%u n29=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 0x29), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80133d6cverify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_80133d6c_run(Core* c) { ov_beh_80133d6c(c); }
