// engine/objbeh_80121978.cpp — PC-native per-object BEHAVIOR handler FUN_80121978.
//
...
void ov_beh_80121978(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80121978verify") ? 1 : 0;
  if (!s_v) { beh_80121978(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80121978verify] MISMATCH obj=%08x st=%u n3=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 3), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80121978verify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_80121978_run(Core* c) { ov_beh_80121978(c); }
