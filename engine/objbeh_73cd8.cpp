// engine/objbeh_73cd8.cpp — PC-native per-object BEHAVIOR handler FUN_80073CD8.
//
...
// (engine/objbeh_739ac.cpp) — a state machine on the node's state byte node[4] (0 init / 1 active /
...
//
// Ownership model (identical to objbeh_739ac / actor_sm_24448 / script_vm): CONTROL FLOW + node/global
...
void ov_beh_73cd8(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj73cd8verify") ? 1 : 0;
  if (!s_v) { beh_73cd8(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj73cd8verify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj73cd8verify] %ld matches\n", ng);
}
