// engine/objbeh_8012d404.cpp — PC-native per-object BEHAVIOR handler FUN_8012D404 (OVERLAY).
//
...
// resident siblings (objbeh_739ac / objbeh_73cd8) and the overlay siblings (objbeh_8012eb54 /
// objbeh_80124e74): a state machine on the node's state byte node[4] (0 init / 1 active / 2 idle /
...
//
// Ownership model (identical to objbeh_739ac / objbeh_73cd8 / objbeh_8012eb54 / objbeh_80124e74):
...
void beh_8012d404(Core* c) {
  ...
  if (st != 1) {
    if (st >= 2) {                                 // ---- 0x8012D43C: v1 >= 2 ----
      ...
      if (st == 3) {
        c->r[4] = obj; rec_dispatch(c, 0x8007A624u);  // 8012D4D4 jal 0x8007a624 (a0=s0)
      }
...
void ov_beh_8012d404(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj8012d404verify") ? 1 : 0;
  if (!s_v) { beh_8012d404(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj8012d404verify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj8012d404verify] %ld matches\n", ng);
}

}  // namespace

void objbeh_8012d404_register(void) {
}

// Exported entry — the verify wrapper ov_beh_8012d404 is in the anonymous namespace above (internal
// linkage); the engine's per-object dispatch calls THIS to run the owned behavior.
void ov_beh_8012d404_run(Core* c) { ov_beh_8012d404(c); }
