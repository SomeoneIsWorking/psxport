// engine/objbeh_80133c14.cpp — PC-native per-object BEHAVIOR handler FUN_80133C14 (OVERLAY).
//
...
// FUN_800739AC (engine/objbeh_739ac.cpp) and FUN_80073CD8 (engine/objbeh_73cd8.cpp): a small state
...
// table; control flow has no computed jumps. Ownership model identical to objbeh_739ac/73cd8: CONTROL
...
//
// Gated byte-exact (full RAM+scratchpad A/B vs rec_super_call) via channel "obj80133c14verify".
...
void beh_80133c14(Core* c) {
  ...
  if (st != 1) {                              // 80133C34 beq s0,1 -> state 1
    if (st >= 2) {                            // 80133C38 slti v0,s0,2 ; 80133C3C beqz -> >=2  (s0 is lbu 0..255, so signed slti == unsigned compare here)
      ...
      c->r[4] = obj;                          // 80133D50 move a0,s1 (delay slot)
      rec_dispatch(c, 0x8007A624u);           // 80133D4C jal 0x8007a624
      return;                                 // -> epilogue
    }
...
void ov_beh_80133c14(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80133c14verify") ? 1 : 0;
  if (!s_v) { beh_80133c14(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80133c14verify] MISMATCH obj=%08x st=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80133c14verify] %ld matches\n", ng);
}

}  // namespace

void objbeh_80133c14_register(void) {
}

// Exported entry — the verify wrapper ov_beh_80133c14 is in the anonymous namespace above (internal
// linkage); the engine's per-object dispatch (engine_tomba2.cpp call_handler) calls THIS to run the
// owned behavior.
void ov_beh_80133c14_run(Core* c) { ov_beh_80133c14(c); }
