// engine/objbeh_80136d9c.cpp — PC-native per-object BEHAVIOR handler FUN_80136D9C.
//
...
void ov_beh_80136d9c(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80136d9cverify") ? 1 : 0;
  if (!s_v) { beh_80136d9c(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80136d9cverify] MISMATCH obj=%08x st=%u n5=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80136d9cverify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_80136d9c_run(Core* c) { ov_beh_80136d9c(c); }
