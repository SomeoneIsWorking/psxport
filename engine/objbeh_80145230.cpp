// engine/objbeh_80145230.cpp — PC-native per-object BEHAVIOR handler FUN_80145230 (OVERLAY).
//
// An OVERLAY per-object behavior routine (guest 0x8014xxxx) with the same state-byte shape as the
// resident/overlay siblings (objbeh_739ac / objbeh_73cd8 / objbeh_8012eb54): a state machine on the
...
second_cull:;
  ...
  if (c->r[2] == 0) goto after_no_cull;                 // beqz v0 -> 0x8014560c        [0x80145518]
  {
    ...
    c->r[4] = obj; rec_dispatch(c, 0x80076D68u);        // FUN_80076d68                [0x801455C8]
...
void ov_beh_80145230(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80145230verify") ? 1 : 0;
  if (!s_v) { beh_80145230(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80145230verify] MISMATCH obj=%08x st=%u sub=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80145230verify] %ld matches\n", ng);
}

}  // namespace

void objbeh_80145230_register(void) {
}

// Exported entry — the verify wrapper ov_beh_80145230 is in the anonymous namespace above (internal
// linkage); the engine's per-object dispatch calls THIS to run the owned behavior.
void ov_beh_80145230_run(Core* c) { ov_beh_80145230(c); }
