// engine/objbeh_80134fd8.cpp — PC-native per-object BEHAVIOR handler FUN_80134FD8.
//
...
void ov_beh_80134fd8(Core* c) {
  static int s_v = -1; if (s_v < 0) s_v = cfg_dbg("obj80134fd8verify") ? 1 : 0;
  if (!s_v) { beh_80134fd8(c); return; }
  ...
  if (ro >= 0 || so >= 0) {
    if (nb++ < 40) fprintf(stderr, "[obj80134fd8verify] MISMATCH obj=%08x st=%u n5=%u ram@%x spad@%x\n",
                           obj, c->mem_r8(obj + 4), c->mem_r8(obj + 5), ro, so);
  } else if (++ng % 50 == 0) fprintf(stderr, "[obj80134fd8verify] %ld matches\n", ng);
}

}  // namespace

void ov_beh_80134fd8_run(Core* c) { ov_beh_80134fd8(c); }
