...
**This file is the canonical, AGENT-AGNOSTIC authority** for the framework, port architecture, build,
runtime, rendering, diagnostics, and registries. `CLAUDE.md` is a symlink to it and exists only
because some tools look for that name — never edit or add content there.

Before concurrent framework work, also read
[`docs/workspace/PROTOCOL.md`](docs/workspace/PROTOCOL.md) completely.
...
### NEVER forward-declare across an ownership boundary — include the header

USER, 2026-09-01: *"I don't like those forward-declarations also, ban it, agents must always use
headers"*.

Declaring someone else's function at your call site (`int gpu_seen3d_this_frame(Core *);` inside a
`.cpp`) is the duplicate-declaration defect above, with an extra failure mode: **it is invisible to a
caller audit.** The owning repo greps its headers, finds no caller, and concludes the function is
dead.

That is not hypothetical — it happened here on 2026-09-01. The GpuState de-globalization deleted
`gpu_seen3d_this_frame` as having "no external callers" while Tomba2Engine's `render_walk.cpp` was
calling it through a local declaration. The framework tree could no longer link its own reference
consumer, and nothing caught it because psxport's hermetic suite is game-agnostic by design and
`psxport_smoke` only proves the framework exports no GAME symbols — never that a game still resolves
the framework's.

So:

- **A function you call belongs in a header owned by whoever defines it.** If no header declares it,
  add it to the right one; do not declare it locally, not even "just for a debug line".
- **This binds across the framework/game seam in both directions**, and inside psxport too: a `.cpp`
  that already includes its own header must not re-declare what that header declares.
- **Removing a public function requires checking the GAME trees, not just psxport.** `$PSX/*/game/`
  is part of the caller set.
- The narrow exception is a genuine circular-include break, which is a design smell to fix rather
  than paper over — and it is a declaration in a HEADER, never at a call site.
